...
fn dispatch(cfg: &Config, key: &str) -> Result<()> {
    match key {
        ...
        "lazygit" => commands::tool::run(cfg, "lazygit", None),
        "yazi" => commands::tool::run(cfg, "yazi", None),
        "editor" => commands::tool::run(cfg, "editor", None),
        "diff" => commands::tool::run(cfg, "diff", None),
