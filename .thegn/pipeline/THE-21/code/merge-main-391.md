# THE-21 — merge main and re-gate (Lead work order)
...
while this one was in review. Reported conflicts / failure:
    ...
    conflicts with main: crates/thegn-core/src/completion/catalog.rs, crates/thegn-core/src/control.rs — NOTE: a previous attempt (row 391) died before completing 'git merge main'; its edits were preserved by the supervisor but the merge itself never happened. Start by confirming 'git merge-base --is-ancestor main HEAD' fails, then do the merge.
