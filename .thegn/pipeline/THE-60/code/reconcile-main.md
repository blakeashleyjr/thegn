# THE-60 reconcile — merge current main into the lane
...
- `agent.rs`: main added `extend_reserving` / `reserve_sandbox_overrides` and
  `provider_home_mounts` so an `[[agents]].env` key beats the bundle env and
  keeps its credential-home mount. Preserve that behaviour exactly while
  keeping your lane's changes.
