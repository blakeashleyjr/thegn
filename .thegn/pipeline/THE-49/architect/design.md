...
read across worktrees and sessions: dispatch state and handoffs, committed
...
| Dispatch report (`THE-88`)       | `thegn dispatch report <id>` stores the bounded report through [`db_dispatch.rs`](../../../../crates/thegn-core/src/db_dispatch.rs#L11-L25); the row model says the report is what the Lead reads and the artifact remains in git [`issue.rs`](../../../../crates/thegn-core/src/issue.rs#L252-L281).                                                                                                                                                      | A structured handoff summary (verdict, commits, unverified items, findings, next hints), capped at 16 KiB and last-write-wins. `dispatches.report` is currently CLI-only [`capability.rs`](../../../../crates/thegn-core/src/capability.rs#L614-L619).                                                        |
| Dispatch note                    | `thegn dispatch note <id>` appends to `agent_dispatch_notes` and reads with `since`/limit [`db_dispatch.rs`](../../../../crates/thegn-core/src/db_dispatch.rs#L28-L74).                                                                                                                                                                                                                                                                                    | Short progress context, capped at 4 KiB. It is deliberately separate from the daemon retry note [`issue.rs`](../../../../crates/thegn-core/src/issue.rs#L258-L265) and is currently CLI-only [`capability.rs`](../../../../crates/thegn-core/src/capability.rs#L620-L630).                                    |
| Pipeline artifacts               | Files under `.thegn/pipeline/<ISSUE>/...` are referenced by roster rows; the row explicitly stores a pointer, never the payload, and git is the source of truth [`issue.rs`](../../../../crates/thegn-core/src/issue.rs#L252-L257). `dispatch verify` requires the artifact to exist and be git-tracked [`pipeline_run.rs`](../../../../crates/thegn-core/src/pipeline_run.rs#L115-L155).                                                                  | The full architect/coder/reviewer handoff, inspectable and reviewable in git across worktrees. There is no global artifact search/read capability today; callers must resolve the recorded worktree and path.                                                                                                 |
...
| Harness session summaries        | `thegn agent sessions` is the CLI read path [`cmd/agent.rs`](../../../../crates/thegn-host/src/cmd/agent.rs#L1-L13), backed by a bounded, read-on-demand scan of configured local harness stores [`sessions.rs`](../../../../crates/thegn-svc/src/sessions.rs#L1-L11). The `agent.sessions` catalog row describes harness/id/worktree/mtime/summary [`capability.rs`](../../../../crates/thegn-core/src/capability.rs#L349-L355).                          | A credential-free one-line first-turn summary and worktree link; at most 500 newest files per discovery pass [`sessions.rs`](../../../../crates/thegn-svc/src/sessions.rs#L21-L24) [`sessions.rs`](../../../../crates/thegn-svc/src/sessions.rs#L78-L114). It deliberately does not return transcript bodies. |
...
optional harness tool: its README describes local BM25 transcript search,
optional local embeddings, and a shared skill workflow. It indexes agent CLI
transcripts, not thegn's git artifacts, Linear detail, or thegn SQLite rows.
Its optional continuous index service and SSH multi-machine mode also make it
the wrong thing to embed behind a no-network, no-new-wake-source shell rule.
Install/use it at the harness layer when transcript search is wanted; do not
vendor it, invoke it from the compositor, or make thegn's state depend on it.
...
dead design for the excised proxy path and is not the authority for THE-49.
...
Those claims were rechecked: the branch already contains commit
`8f5eb345` (“MCP proxy hub”), and the implementation is present in
`thegn-core/src/mcp/proxy/` and `thegn-host/src/mcp_proxy/`. The current host
module explicitly calls its v1 path an in-process standalone hub and names
daemon sharing as follow-up [`crates/thegn-host/src/mcp_proxy/mod.rs`](../../../../crates/thegn-host/src/mcp_proxy/mod.rs#L7-L20).
...
4. Keep tokenization, normalized terms, deterministic ranking, limits, and
   source attribution in a new substrate-free `thegn-core` module with unit
   tests. Filesystem, SQLite, and any provider reads stay at host/service
   edges and degrade to a miss with a diagnostic.
