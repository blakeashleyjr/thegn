{
  ...
  outputs = {
    ...
    in {
      ...
      # sprite). Critically it also avoids the `muse` flake input, so a sandbox
      # whose `git` can't fetch that source still gets a working rust devShell.
      # Anything missing is one `nix shell nixpkgs#<tool>` away in-pane (see the
      # shellHook). Selected per-sandbox via `[sandbox] devshell = "sandbox"` →
      # `SUPERZEJ_DEVSHELL` → the repo `.envrc`'s `use flake` ref.
