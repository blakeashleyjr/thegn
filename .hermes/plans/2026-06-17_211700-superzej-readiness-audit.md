# Superzej (szhost) Release Readiness Audit Plan
...
**Goal:** Conduct an unbelievably detailed audit of the native `szhost` terminal multiplexer/IDE to ensure Phase 1 "prime time" release readiness. Identify bugs, performance problems, architectural drift, and UX regressions.

**Architecture:** A single-process Rust host (`superzej-host`) orchestrating a tokio event loop, `portable-pty` panes, `termwiz` diff-flush rendering, and an embedded SQLite state store. The system is entirely event-driven, blocking on `poll_input(None)`, with an invariant of ~0% idle CPU. It replaces the old zellij+WASM architecture.
...
- **Testing & Verification:** 95% line coverage on `superzej-core`, clean CI pipelines, and hermetic end-to-end smoke testing.
...
4. **Coverage:** `just coverage` (Checks `superzej-core` for ≥95% lines, excluding I/O seams).
...
| ------------------ | ------------------------------------------------------------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------- |
| **Performance**    | Cold launch → first frame < 300ms. No blank/flash on startup.             | `just bench` / inspect `run.rs`                          | `szhost::startup` traces show instant cheap model build before git hydration. | Core Team         |
| **Performance**    | PTY channel has bounded backpressure / drain budget.                      | `rg -A 5 "try_recv" crates/superzej-host/src/run.rs`     | Drain budget implemented to prevent PTY starvation.                           | Core Team         |
| **TUI Layout**     | No stale cells survive layout changes.                                    | Inspect `crates/superzej-host/src/run.rs` rendering      | `full_repaint = true` clears logical frame on geometry change.                | Core Team         |
| **TUI Render**     | Multiplexer title bars do not overwrite row 0.                            | Inspect `crates/superzej-host/src/chrome.rs`             | Row 0 offset or `""` prepend logic exists if running inside tmux/zellij.      | Core Team         |
| **UX & Focus**     | Keybinds correctly fall through to PTY only when focused.                 | Inspect `route()` in `crates/superzej-host/src/focus.rs` | `forwards_to_pane` returns true only for Center or when drawer open.          | Core Team         |
...
| **Data Integrity** | Database migrations (to v6) preserve layout and worktrees gracefully.     | Review `crates/superzej-core/src/db.rs`                  | v5 to v6 migration runs atomically, handles malformed JSON safely.            | Core Team         |
...
**Objective:** Verify that `szhost` does not suffer from PTY flood starvation and renders the first frame instantly.
**Verification Steps:**

- Inspect `crates/superzej-host/src/run.rs` around `poll_input` and `try_recv()`.
...
- Inspect `crates/superzej-host/src/compositor.rs` and `run.rs` for `full_repaint`.
- Verify the logical frame clear strategy (`Change::ClearScreen`) is used when `geometry_changed` is true.
- Audit `crates/superzej-host/src/chrome.rs` to ensure tab labels render in `tabbar_content` bounds to prevent far-left flashes.
...
- Verify `SUPERZEJ_LOGIN_SHELL` flag logic avoids expensive login-shell startups by default (`crates/superzej-host/src/panes.rs`).
...
- Audit command palette (`crates/superzej-host/src/palette.rs`) to ensure `Esc` correctly tears down the modal.
...
- Read `README.md` to ensure zellij/WASM references reflect the current native `szhost` architecture correctly.
...
- **Edition 2024 Constraints:** Usage of async traits across crate boundaries (e.g., `superzej-svc`) must be monitored to ensure it doesn't break compilation or require unwanted boxing overhead.
