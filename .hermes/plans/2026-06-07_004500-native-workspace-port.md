...
**Goal:** Port superzej's workspace and worktree loading + navigation logic into the native `szhost` substrate.
**Architecture:** Make `szhost` session loading DB-aware, hydrate the sidebar with actual known workspaces, build real worktree tabs per workspace, and wire native command actions to navigate them.
**Tech Stack:** Rust, `superzej-core::db`, `superzej-host` native components.
...
- Modify: `crates/superzej-host/src/run.rs` (hydration logic)
- Modify: `crates/superzej-host/src/chrome.rs` (FrameModel & sidebar rendering)
...
Run: `cargo test -p superzej-host hydration_worker_loads_real_workspaces_into_sidebar`
Expected: FAIL.

**Step 3: Write minimal implementation**
In `crates/superzej-host/src/run.rs::build_model`:
...
```bash
git add crates/superzej-host/src/run.rs crates/superzej-host/src/chrome.rs
...
**Objective:** The `szhost` session resurrect path currently creates a dummy `{cwd}/home` tab. It needs to read the DB `tab_layout` table for the target session (which represents the workspace), defaulting to creating a `{slug}/home` tab if the DB is empty for that repo.

**Files:**

- Modify: `crates/superzej-host/src/run.rs` (`load_or_seed_session`)
- Modify: `crates/superzej-core/src/db.rs` (if new queries are needed, but `tabs_for_session` exists)
...
**Step 2: Run test to verify failure**
Run: `cargo test -p superzej-host load_or_seed_session_recovers_tabs_from_db_when_present`
Expected: FAIL.
...
**Objective:** When the user selects a workspace in the palette or sidebar, `szhost` needs to swap out the active session's tabs for the new workspace's tabs, persist the old ones, and redraw.

**Files:**

- Modify: `crates/superzej-host/src/run.rs` (action dispatch)
- Modify: `crates/superzej-host/src/keymap.rs` (if new action needed)
...
**Step 2: Run test to verify failure**
Run: `cargo test -p superzej-host action_switch_workspace_reloads_tabs_for_new_target`
Expected: FAIL.
...
**Step 2: Run test to verify failure**
Run: `cargo test -p superzej-host action_new_worktree_adds_tab_and_focuses_it`
Expected: FAIL.
