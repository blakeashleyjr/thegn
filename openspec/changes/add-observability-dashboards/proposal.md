...
Add a Grafana-compatible observability frontend to thegn as an **"Observe"
...
(Prometheus, Loki, SQL, a built-in `host` source over `thegn-metrics`, and a
...
`ratatui-image`, composited through thegn's existing tg-kit `AppTile` contract.

## Impact

- **AH** (resource / system monitoring) — promotes the host metrics into a full
  dashboard surface; the `host` DataSource reuses `thegn-metrics`.
...
thegn already runs a damage-region compositor, a metrics leaf crate, and an
tg-kit app-tab path for sibling TUIs (ratatui 0.30). An observability tile rides
...
also out of scope — Observe ships inside `thegn` (though `gtui-*` crates stay
standalone-capable).
