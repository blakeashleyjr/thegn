...
- [ ] 0.1 `gtui-core`: `Frame`/`Field` model (typed columnar on Polars/Arrow,
      field + frame metadata, time semantics, wide⇄long) — **unit tests** (95% gate).
- [ ] 0.2 `gtui-core`: `DataSource` trait + `Query`/`TimeRange`/`Caps`/`QueryError`.
- [ ] 0.3 `gtui-query`: synthetic test source (random-walk) — **unit tests**.
- [ ] 0.4 `gtui-render`: braille time-series + LTTB downsample + unit-aware axes —
      **unit tests** (downsample bounds, axis formatting).
- [ ] 0.5 `gtui-app`: event loop + global time range; render off-loop via channel +
      waker (assert no polling timeout).

## Phase 1 — MVP [M]

- [ ] 1.1 `gtui-query`: Prometheus (instant + range, step from panel width), Loki
      (LogQL range + tail), `host` source over `superzej-metrics` — fixture-tested parsers.
- [ ] 1.2 Explore mode: split editor/viz, auto-detect viz, session history.
- [ ] 1.3 Core panels: time-series, stat, table, logs + field config + legend toggle.
- [ ] 1.4 Auto-refresh ticker (alive only while active + refresh>0; pause/resume);
      time-range change cancels in-flight queries — **test** cancellation + no idle ticker.
- [ ] 1.5 Native TOML dashboard format + load-from-directory + command palette.
- [ ] 1.6 `gtui-embed`: Observe app-tab via sz-kit + `catch_unwind` panic boundary —
