...
- [ ] 1.1 Extend `LspServerConfig` with optional `extensions: Vec<String>` and
...
- [ ] 1.2 `config_validate`: a non-built-in `lang` key with no `extensions` is
...
- [ ] 1.3 Unit tests: round-trip the new fields, the validation errors, and
      that a legacy override-only entry deserializes identically.
- [ ] 1.4 Document the extended `[[lsp.servers]]` keys in
...
- [ ] 2.1 Express the six built-ins as data: a `builtin_servers()` table of
...
- [ ] 2.2 Merge user entries over built-ins field-wise (override command
...
- [ ] 2.3 Extension → registry-key resolver (`resolve_key(path) ->
...
- [ ] 2.4 Keep `Lang` as the tree-sitter tier: `Lang::from_key(&str)` adapter
...
- [ ] 4.3 Trust gating: `[[lsp.servers]]` entries sourced from a
...
- [ ] 5.1 Move consumers from `Lang`-keyed to registry-key-keyed client
      lookup: Problems (diagnostics), Symbols section outline fetch,
      hover/signature/code-action, Search Everywhere symbol mode,
      go-to-def/refs.
- [ ] 5.2 Per-tier degradation: registry-only languages take the LSP path
...
- [ ] 5.3 `NotAvailable` from the negotiation gate flows into each consumer's
      existing missing-server fallback (no new consumer plumbing) — verify
      with targeted tests per consumer.

## 6. Doctor (thegn-host)

- [ ] 6.1 `thegn doctor` LSP section: every registry entry (built-in +
...
- [ ] 6.2 Unit test the section's resolved-list formatting (pure part) and
      extend the smoke test to assert the section prints.

## 7. Docs + validate

- [ ] 7.1 Update `docs/help/configuration.md` prose for the registry (the
...
- [ ] 7.2 `git add` any new modules before nix-build (flake source allowlist
