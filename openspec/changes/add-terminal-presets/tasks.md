...
- [ ] 3.2 Wire template `preset` into worktree creation (after-create apply;
      template `agent` still wins the remembered agent).
- [ ] 3.3 Keep the render-plan tests green (new tabs/splits ⇒ `Full`).

## 4. CLI + catalog

- [ ] 4.1 `Verb::LaunchPreset` + `required_scope` mapping (exec-level, not
      `open`'s) + `CATALOG` row (Cli implemented; other surfaces per the
      coverage tests — reconcile with `complete-control-surface-coverage`).
- [ ] 4.2 `thegn open --preset`: name validation (miss ⇒ candidates + exit 3),
...
- [ ] 4.3 Smoke-test the CLI path (`test/smoke.sh` addition).

## 5. Help & docs

- [ ] 5.1 Help page update claiming `launch-menu` (extend
