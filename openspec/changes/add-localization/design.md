...
- **Detection:** `sys-locale` reads the host locale; `[ui] language` (default
  `"auto"`) overrides. Resolved **once** in the `thegn::startup` waterfall, before
  first render.
...
(`format!("{:^10}", t!("key"))`). Crate additions go in `thegn-core` so
`thegn-host` is insulated.

## Invariants

Zero runtime I/O (compiled-in), resolves before first frame (sub-300ms preserved),
no idle cost. AI-additive: this localizes only thegn's own chrome, never user
