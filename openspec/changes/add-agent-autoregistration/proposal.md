...
Make thegn discoverable by the external agents already installed on a machine,
...
1. **Auto-registration** — detect installed agent CLIs (Claude Code, Codex,
   Cursor, …) on PATH and register thegn's MCP surface into each agent's config,
...
- **AL (MCP server)** — provides the client-side registration that makes
  thegn's house-tool MCP surface usable from external agents.
- **R (ACP)** — complements the first-party ACP agent by exposing the same surface
  to foreign harnesses via their MCP config.
- Relates to the managed-pi setup (`thegn agent setup`) — the same installer
...
thegn already installs a managed pi agent and exposes a house-tool MCP router
...
strings that say what to do) — fits thegn's bouncer/proxy error seams, turning
"call failed" into an actionable instruction the agent can act on. Both are additive
and keep thegn a good citizen in a multi-agent machine.

## Non-goals

- **Bundling/managing external agents** — thegn registers into agents the user
...
- **Clobbering user config** — registration is idempotent and merge-based; it never
  overwrites unrelated config, and `disable` cleanly removes only thegn's entry.
