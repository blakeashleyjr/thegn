...
## 1. Detection + config merge (superzej-core)
...
- [ ] 1.1 Declarative known-agent table (id → binary, config path, registration
      ...
      superzej's MCP server entry — **unit tests**: merge adds the entry, re-merge
      is a no-op, unrelated entries are preserved, remove deletes only superzej's
      entry, malformed config errors cleanly.

## 2. Error markers (superzej-core)
...
## 3. Register/unregister verbs + marker emission (superzej-host)

- [ ] 3.1 Extend `cmd/agent.rs`: `szhost agent register` detects installed agents
      and registers superzej's MCP surface (CLI-add with config-merge fallback);
...
- [ ] 4.1 Document `szhost agent register`/`unregister`, the config paths targeted,
