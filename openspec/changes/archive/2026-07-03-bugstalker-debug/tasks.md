...
- [x] 1.2 Add `ManagedTool::cargo(name, crate_name, bin_name, version)` (default dir `~/.superzej/tools/<name>`, `bin_rel = bin/<bin_name>`, marker `.version`).
- [x] 1.3 Unit-test: a Cargo tool resolves to `<managed_dir>/bin/<name>` and has no release asset.

## 2. Core: debugger module

- [x] 2.1 Add `crates/superzej-core/src/debug.rs` and export from `lib.rs`.
...
- [x] 4.1 Extend `superzej-host::managed_tool::acquire` with the `Cargo` arm: `cargo install <crate> --version <v> --root <managed_dir>` via `run_setup_cmd` (ensure `cargo` present with a helpful message).
...
## 5. Host: `szhost debug` CLI

- [x] 5.1 New `crates/superzej-host/src/cmd/debug.rs` with a clap `Action` (`Setup{force}`, `Path`, `Run{program, args}`, `Attach{pid}`); register `mod debug` + `Command::Debug` in `main.rs`.
...
- [x] 7.1 `cargo test -p superzej-core` green (new debug + Cargo-source tests).
- [x] 7.2 `cargo clippy -p superzej-core -p superzej-host --all-targets` + `cargo fmt --check` clean; god-file ratchet OK (new code in new modules; `main.rs` command wiring only).
- [x] 7.3 Manual: `szhost debug path` (resolves bs / reports tier), `szhost debug setup` on Linux-x86-64 (installs or reports present), `szhost doctor` lists bs; on a non-supported platform the verbs refuse cleanly. NOTE: full `cargo install` fetch is a smoke/manual seam.
