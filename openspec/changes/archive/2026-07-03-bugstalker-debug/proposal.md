...
thegn has no debugging story. BugStalker (`bs`) is a self-contained Rust/Linux
debugger with its own TUI that speaks standard DAP — the cheapest way to give
thegn a real debugger is to _run `bs` in a pane_, acquired and pinned through
...
- Add a **`Cargo` source** to the managed-tool resolver (`thegn-core`):
...
- Add a **`debugger` capability** in `thegn-core` (`debug.rs`): the pinned
...
- Add a **`thegn debug` CLI subcommand**: `setup [--force]` (ensure `bs` is
  ...
  terminal — run inside a thegn pane to debug within that pane's
  sandbox/placement). All platform-gated with a clear message where unsupported.
- **`thegn doctor`** lists `bs` among managed tools, with the platform note.
...
- `debugger`: how thegn acquires and launches a debugger — the pinned
  BugStalker managed-tool spec, the Linux-x86-64 platform gate, the
  launch/attach session-argv builders, the `thegn debug` verbs, and the
...
- **Code:** `crates/thegn-core/src/managed_tool.rs` (+`Source::Cargo`),
  new `crates/thegn-core/src/debug.rs` (+`lib.rs` export);
  `crates/thegn-host/src/managed_tool.rs` (Cargo acquire arm + generic
  `install()`), new `crates/thegn-host/src/cmd/debug.rs`, `main.rs` command
