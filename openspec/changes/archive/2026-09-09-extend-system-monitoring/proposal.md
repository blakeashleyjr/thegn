...
The monitoring stack was substantial but unspecced, its platform gaps were
invisible, and external metrics accepted only HTTP scrapes. THE-44 needed a
written off-loop/absence contract, probeable platform coverage, bounded
process and disk workflows, and a safe argv-based collector extension.
...
- Delivered code: `thegn-metrics` (no new fields required),
...
- Commit `4d3d8023` landed THE-44. Platform-family classification is in
  `thegn-metrics/src/coverage.rs`; process filtering/tree/signal presentation
  is in `thegn-host/src/monitor/procs_view.rs`; disk projection is in
  `thegn-core/src/disk_fill.rs`; command collection is in
  `thegn-host/src/metrics.rs`.
- Focused tests cover absent-family classification, stable doctor keys,
  process filtering/tree ancestry/cycles, disk-projection honesty gates,
  monitor interactions, collector timeout/cap/failure behavior, and repo
  overlay rejection.
- The shipped implementation remains TUI-only for arbitrary process signals,
  adds no capability-catalog row, and introduces no SQLite schema change.
