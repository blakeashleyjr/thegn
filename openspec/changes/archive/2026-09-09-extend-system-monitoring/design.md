...
## Resolved delivery decisions and deferred scope

- Disk time-to-full uses the shipped least-squares projection with thin-span,
  non-decreasing, non-finite, and noise honesty gates in `disk_fill`.
- Doctor emits the coverage report in both text and structured JSON output.
- Monitor-history record/replay was not implemented and is not part of the
  accepted THE-44 scope; it remains a separate state/retention proposal if
  pursued.
