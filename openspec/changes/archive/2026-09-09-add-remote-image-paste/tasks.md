...
- [x] 4.3 The deterministic hook `e2e_freeze::paste_image_name` shipped. No
      muse scenario drives this flow, so no snapshot recording is required.
- [x] 4.4 Archive reconciliation: the policy, clipboard, local-drop, remote
      script-shape, and permission tests in 4.1–4.3 are the retained evidence.
      Feature commit `4d92dc6c` is in current history; the old reviewer-only
      full-CI reminder is not claimed as having run.
