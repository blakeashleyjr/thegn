...
Feature commit `4d92dc6c` is in current history. Pure naming/limit/drop-script
tests, platform clipboard-candidate tests, local permission/sweep tests, remote
script-shape coverage, and the deterministic `e2e_freeze` name hook evidence
the accepted explicit-action paste path. No muse scenario currently drives the
flow, so snapshot recording is not part of this delivered scope; the proposal,
design, and delta remain accurate.
