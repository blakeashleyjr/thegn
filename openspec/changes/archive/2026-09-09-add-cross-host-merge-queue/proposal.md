...
Feature commit `8776f3c4` is in current history. `merge_remote` bundle/fetch
tests across separate stores, same-store no-op coverage, schema/store tests,
and target-host guards evidence the accepted host-aware queue design. The
proposal/design/delta boundary remains accurate: unreachable transfers defer a
row with a reason rather than dropping or landing it.
