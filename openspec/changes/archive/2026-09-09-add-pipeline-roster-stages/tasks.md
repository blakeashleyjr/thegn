...
- [x] 6.2 Archive reconciliation: the migration, store, wire-schema, CLI, and
      pane-attribution suites in 6.1 are the retained evidence; feature commit
      `6b8d5c8a` and merge `2d2f20d3` are in current history. The old serial
      full-`just ci` reminder is not a remaining product requirement.
