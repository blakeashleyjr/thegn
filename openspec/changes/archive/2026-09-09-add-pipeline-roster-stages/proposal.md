...
Feature commit `6b8d5c8a` and merge `2d2f20d3` are in current history. The v56
additive-migration tests, dispatch read/write and attribution tests, additive
control-schema snapshot, CLI smoke coverage, and pane-exit tests evidence the
accepted stage/parent/session/artifact roster design. `--adopt` intentionally
records an inert intent; consuming that intent and `--resume` are not part of
this archived scope, consistent with the design and deltas below.
