...
## Shipped-scope reconciliation (2026-09-08)

The accepted first cut is CLI discovery and explicit CLI promotion, not a TUI
wizard or palette picker. Feature commit `36da0f15`, hermetic smoke fix
`5ee8a418`, and merge `4f029961` are in current history. Parser/filter/mapping
tests, catalog coverage, doctor coverage, and the missing-client smoke case
evidence the shipped seam. The proposal, design, delta, and tasks below are
normalized to that narrower delivered boundary.
...
- **Headscale as same-seam config**: the local client is control-plane-
  ...
  them.
- **Surfaces**: a `host.discover` capability-catalog row and CLI verb under the
  `thegn host` namespace (`--json` list shape), including explicit
  `--promote <name|fqdn>` into global host config. A TUI wizard/palette picker
  is a possible additive follow-up, not part of this accepted cut. Promotion
  stays explicit and `install_runtime` consent stays `ask`.
...
- **Crates**: `thegn-core` (+`tailnet.rs`, pure), `thegn-svc`
  (+`host_discovery/`), `thegn-host` (CLI discovery/promotion and doctor wiring).
