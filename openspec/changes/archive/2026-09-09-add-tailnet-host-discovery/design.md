...
thegn-core (pure, 95% gate)          thegn-svc (I/O seam)         thegn-host
  ...
  HostCandidate { name, fqdn,          trait HostDiscovery:         --json / --promote
    os, online, tags,                    kind() -> &'static str     doctor probe row
    ssh_advertised, node_id }            discover() -> BoxFuture<
  parse_status_json(&str)                  Result<Vec<HostCandidate>>>
  filter(candidates, &cfg)               probe() -> ProbeReport
```

The diagram records the reusable seam originally considered for both CLI and
TUI consumers. The accepted first cut wires `cmd/host.rs discover` (including
`--promote`) and doctor only; the wizard/palette consumers are not shipped by
this change.
...
- Discovery is **on-demand only** through the CLI in this cut. The CLI process
  executes the bounded discovery call outside the compositor; there is no
  background rescan, polling timer, new damage channel, or wake source.
...
- The CLI verb is documented on the CLI help surface; no TUI action id is
  claimed. Config keys land in `config/config.toml.example` and remain covered
  by the example-config test.
...
- A future TUI picker can consume the same seam and pure promotion mapping, but
  it requires its own bounded proposal and action/help contract.
