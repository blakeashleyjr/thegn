...
Feature commit `563d0697` is in current history. Channel/feature registry and
config-clamp tests, runtime-resolution and doctor tests, clippy with the dev
feature, Nix package definitions, and the recorded manual stable/dev matrix
evidence the accepted two-channel design. This change is a runtime/config
clamp, not compile-time removal of experimental code; the proposal, design,
and deltas remain aligned with that boundary.
