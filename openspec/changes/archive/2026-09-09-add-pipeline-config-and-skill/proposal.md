...
The accepted scope is declarative validation and an agent-consumed conductor
skill, not an in-process scheduler. Feature commit `95b8d748` and merge
`d78fc3f3` are in current history. `config_pipeline.rs`, config-validation and
template tests, the example-config tests, and
`extensions/skills/pipeline/SKILL.md` provide the retained code/test evidence;
the proposal, design, and deltas match that delivered boundary.
