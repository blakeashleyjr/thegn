...
- [x] Archive reconciliation: the scoped core/host MCP tests and strict
      OpenSpec validation above are the retained evidence; feature commit
      `c5909d73` and merge `dc698d44` are in current history. The former
      pre-push aggregate-gate reminder is not a product acceptance item.
