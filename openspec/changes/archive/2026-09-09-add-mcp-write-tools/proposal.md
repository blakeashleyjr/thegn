...
Feature commit `c5909d73` and merge `dc698d44` are in current history. The
argument-schema/router tests, scope-split and catalog coverage tests, host MCP
handler tests, redaction tests, and strict OpenSpec validation evidence the
accepted `sessions.open/input/wait/kill` projection. The explicit
`--allow-session-input` interlock and retained plugin gaps are part of the
shipped design; broader tracker/file mutation remains out of scope.
