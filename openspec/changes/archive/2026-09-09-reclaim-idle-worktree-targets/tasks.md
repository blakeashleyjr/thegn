...
- [x] 8.1 Archive reconciliation: the pure reclaim-policy, disk measurement,
      config overlay, and host-wiring tests above are the retained evidence;
      feature commit `a7db2dad` is in current history and strict OpenSpec
      validation is rerun during reconciliation. The old full-CI land-gate
      reminder is not claimed as having run.
