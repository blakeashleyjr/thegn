...
Feature commit `a7db2dad` is in current history. Pure reclaim-policy tests,
disk-walk measurement tests, config/overlay coverage, host scan wiring tests,
and the measured cold-build/size study evidence the accepted idle and
pressure-eviction policy plus dependency-debuginfo reduction. Shared target
directories and a global gate target remain deliberately rejected, matching
the proposal, design, and deltas.
