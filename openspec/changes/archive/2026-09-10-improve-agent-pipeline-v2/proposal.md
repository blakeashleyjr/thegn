...
Linear: THE-116 (delivery residual; originating issue THE-76)
...
or fires `timeout_secs`. The THE-121 reliability follow-up makes the atomic
claim enforce the declared `concurrency` ceiling; it still does not choose work
or schedule a stage. What it gains is the
...
- **Roadmap**: group **Q** — Q 212 (task→worktree→agent→review→merge pipeline:
  ...
  Lead drives), Q 223 (task history: the durable roster). Linear **THE-116**
  closes the delivery residual from **THE-76**.
