...
- [x] 1.1 `pipeline_run.rs`: `issue_key` / `artifact_path` (whitelist
      ...
      `Spawning | Running`, each miss its own error) +
      `with_fresh_registry`. Every public item unit-tested; nothing impure.
      Harness-aware permission seeding lives in the host's
      `agent_permissions::merged_settings`: malformed/wrongly-shaped files are
      refused, the effective allow list replaces the old list, and unrelated
      keys are preserved.
...
- [x] 2.1 `dispatch verify` — prints the `VerifyReport` (ok / reasons / dirty)
      for one row; CLI-only catalog row `dispatches.verify` (`Verb`,
      `required_scope`, `cli_control_caps()`; no route, no `SURFACE_GAPS`).
- [x] 2.2 Gated `dispatch set-status done` — rows carrying an artifact are
...
- [x] 3.1 `session open --stage <stage>` — renders the stage prompt with
      ...
      session/artifact and moves queued/spawning → `running` atomically; a
      concurrent row verdict is preserved and the orphan session torn down;
      open failure ⇒ row `failed` with its id named.
- [x] 3.2 `session close` (named verb over `sessions.kill`) and
      `session list --live` (exited sessions carry `exited_at_ms` /
      `exit_code` / `final_state`).
- [x] 3.3 Daemon registry freshness — agent resolution refreshes `agents` /
      `tools` / `pipeline` per request via `with_fresh_registry` (D4), merged
      over the boot config.
- [x] 3.4 The daemon-backed `test/smoke.sh` cases now cover failed-open row
      reconciliation and the full stage worker → report → tracked verify →
      Done loop. Rebuild the binary from the final shared source and pass the
      complete smoke before checking this integration gate; no frame changed.
