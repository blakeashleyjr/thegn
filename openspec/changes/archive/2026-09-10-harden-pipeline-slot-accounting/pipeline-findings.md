...
### 31. `session open --stage` bypasses `claim_dispatch` **[fixed]**

The path the Lead actually uses now routes its roster insert through
`claim_dispatch`, so de-duplication and stage capacity are checked in the same
`BEGIN IMMEDIATE` transaction as the insert.

### 32. Row-derived artifact naming defeats dedupe **[fixed]**

Before a row id exists, `session open --stage` has no artifact to claim. The
chunk path is therefore the provisional discriminator: a retry matches the same
chunk after the existing row receives its row-derived artifact, while distinct
`--chunk` claims remain parallel. Once both sides name artifacts, the artifact
dominates so two chunks cannot concurrently own one handoff.
