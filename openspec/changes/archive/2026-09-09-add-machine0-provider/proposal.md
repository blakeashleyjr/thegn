...
Feature commit `6c6ef061` and subsequent provider hardening are in current
history. MCP protocol/helper tests, provider/classifier/factory tests, bridge
and transport tests, clippy, and the recorded live lifecycle smoke provide the
retained evidence for the accepted control-plane, SSH/mosh data-plane,
checkpoint, scale-to-zero, and NixOS-provisioning scope. The proposal, design,
and deltas accurately describe that shipped boundary.
