...
- An optional `landed` queue row (`--track` or equivalent) would change
  `merge list` and expiry semantics. It was not implemented and is not part of
  THE-63; it requires a separate contract if pursued.
- `merge land` on a `ready` row already emits `Landed`; under `remove` it
  ...
  Same cwd hazard, pre-existing, out of scope here.
