...
`thegn land` left a successfully landed worktree at the ungrouped repository
root even though the shipped lifecycle files queue-landed worktrees into the
Merged folder. THE-63 needed a non-destructive land-in-place event that keeps
the command's leave-in-place guarantee while restoring consistent filing.
...
- Commit `1d23deee` landed THE-63. The pure event/decision table is in
  `thegn-core/src/merge_lifecycle.rs`; the two successful `thegn land` arms
  dispatch it from `thegn-host/src/cmd/land.rs`; best-effort folder I/O and
  its lifecycle-folder guard remain in `thegn-host/src/merge_lifecycle.rs`.
- Core tests exhaust every `on_landed` arm and prove `LandedInPlace` never
  yields removal. Host tests prove default filing preserves the worktree and
  branch, `off` unfiles only lifecycle-managed folders, and user folders and
  the home checkout remain untouched.
- Documentation and changelog updates landed in the same commit; the change
  has no schema, control, wake, or render surface.
