...
- [x] 1.1 Add `LifecycleEvent::LandedInPlace` to
...
- [x] 1.2 Extend the exhaustive unit tests: every `on_landed` arm for
...
- [x] 1.3 Update `default_config_enables_full_lifecycle` to also pin the
      `LandedInPlace` default (`expire` ⇒ file into "Merged").

## 2. The land call-site (thegn-host)

- [x] 2.1 In `crates/thegn-host/src/cmd/land.rs`, replace the `Dequeued`
...
- [x] 2.2 Host-side test (in `merge_lifecycle.rs` tests or a `land` test):
...
- [x] 3.1 `docs/help/merge-queue.md`: state that a `thegn land` files the
...
- [x] 3.2 `config/config.toml.example`: extend the `on_landed` /
      `merged_folder` comments with the land-in-place behaviour (no new key).
- [x] 3.3 CHANGELOG entry — behaviour change: `thegn land` now files into
      "Merged" instead of un-filing to the repo root.

## 4. Validation

- [x] 4.1 Commit `1d23deee` records the focused core and host lifecycle tests,
      including the no-removal table invariant, default Merged filing, `off`
      unfiling, user-folder guard, and home-checkout guard. No browser or
      compositor rendering path changed, so no e2e baseline belongs to this
      accepted scope.
- [x] 4.2 Strict OpenSpec validation passed during archive-readiness review.
