...
- Merge `76d85c9c` landed the accepted THE-35 scope. Pure parsing and route
  precedence live in `thegn-core/src/notification_sound.rs` and
  `notification_route.rs`; host snapshot/queue orchestration and fixed-argv
  providers live in `thegn-host/src/notification_sound.rs` and
  `platform/sound.rs`.
- Tests pin reference parsing/rejection, gate and precedence behavior,
  untrusted-repo stripping, pack filename/stem resolution, reload generation
  ordering, command failure reporting, and platform-provider construction.
- The implementation adds no command, control, capability, completion, or
  database surface, matching the negative boundary in the delta.
