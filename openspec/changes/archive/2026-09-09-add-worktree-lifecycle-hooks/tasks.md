...
- [x] 5.1 Smoke coverage: a hook that writes a marker file runs at create and
      destroy in `test/smoke.sh` (hermetic XDG_STATE_HOME).
- [x] 5.2 Verify delta spec scenarios against the implementation.
- [x] 5.3 Reconcile this change with the shipped implementation and validate it
      with `openspec validate add-worktree-lifecycle-hooks --strict`.
...
Hermetic create/destroy coverage now lives in `test/smoke.sh`; strict OpenSpec
validation is the bounded reconciliation gate for this change.
