...
- Commit `0b49a318` landed the shared sidecar, ordering helper, switcher
  integration, reorder gesture, configuration reference, and help prose.
- `thegn-core/src/profile.rs::order` owns load/save/apply behavior under the
  real `XDG_CONFIG_HOME`; `thegn-host/src/palette.rs` builds the complete
  ordered set; `run.rs` swaps and persists the highlighted row only while the
  profile switcher is active.
- Tests cover missing/malformed sidecars, round trips, unknown and duplicate
  names, stable appends, sidecar-over-seed precedence, active markers, and the
  complete configured/on-disk/default profile set.
