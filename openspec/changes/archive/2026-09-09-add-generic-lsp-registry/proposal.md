...
Feature commit `5e4e7c22` and merge `a8d20c4e` are in current history.
Registry/config tests, capability-negotiation tests with the fake-LSP harness,
supervisor/consumer fallback tests, doctor coverage, and scoped clippy/nextest
evidence the accepted arbitrary-server registry. The tree-sitter grammar tier
and semantic graph remain deliberately separate, matching the proposal,
design, and delta.
