...
## 3. Core additions (thegn-core — coverage-gated)
...
## 4. Headless lifecycle (thegn-host)
...
- [x] 5.1 `--json` on `list`/`wt list`, `env list`, `host list`, `ci runs`,
      ...
      `diff --stat --json` was a cuttable stretch item and is excluded from the
      accepted change. The shipped machine-output contract covers the nine surfaces
      in 5.1; a future diff-stat format requires its own proposal.

## 6. Help + completions (thegn-host)
