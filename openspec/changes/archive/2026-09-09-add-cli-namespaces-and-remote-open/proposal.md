...
The accepted scope is the namespace, lifecycle, machine-output, completion,
and open-intent surface implemented by feature commit `c264e3cf` and documented
by `33704764`, both in current history. The retained unit/smoke evidence covers
the shared emitters, DB intent lifecycle, worktree rollback/cleanup, grouped
help, completions, and remote-open resolution. The cut `diff --stat --json`
stretch item is explicitly not part of this change or its delta contract.

## Summary

Restructure the `thegn` CLI around the domain model: a `wt` namespace giving
...
`thegn open <repo>` — a remote control that focuses a running compositor
...
- **D 41/47** (create/delete worktree) — the existing TUI-only lifecycle gains
  CLI parity by reusing the same `thegn_core::worktree` pipeline.
...
parallel-worktrees use case thegn exists for. Machine output is incidental
