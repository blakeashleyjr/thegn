...
Provider feature commit `50e0f31b` and the Fly image/reaper follow-ups are in
current history. Request-shape/parser unit tests, HTTP replay tests, provider
factory/classification tests, workspace clippy/tests, and the recorded live
create/SSH/stop-start/destroy cleanup run evidence the accepted DigitalOcean
and Fly provider scope. The proposal, design, and deltas remain aligned with
that delivered provider-seam boundary.
