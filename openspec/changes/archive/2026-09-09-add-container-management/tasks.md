...
- [x] 5.3 Config decision: no new `[sandbox]` keys. The delivered stats gate
      uses the existing 5s cadence and a `CONTAINER_DF_EVERY_TICKS` constant,
      avoiding a duplicate operator knob and config-overlay blast radius.

## 6. Validation

- [x] 6.1 Archive reconciliation: no deterministic muse flow exercised the
      monitor overlay, so no snapshot update belongs to the accepted scope.
- [x] 6.2 Retain the scoped `just quick thegn-core`/`thegn-host` and
      `cargo nextest run -p thegn-core sandbox_manage` evidence recorded by the
      implementation. Feature commit `47a69d73` is in current history; the old
      reviewer-only full-`just ci` reminder is not claimed as having run.
