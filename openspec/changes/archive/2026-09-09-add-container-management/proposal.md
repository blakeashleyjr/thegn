...
Feature commit `47a69d73` is in current history. The accepted scope is the
ownership-witnessed management API, catalog/scopes, visibility-gated polling,
monitor overlay, and sandbox GC/prune CLI evidenced by `sandbox_manage` unit
tests and the scoped core/host checks. No new sandbox cadence knob or muse
snapshot is part of the shipped contract; the existing cadence constant and
overlay-local actions are deliberate decisions reflected by the design and
deltas.
