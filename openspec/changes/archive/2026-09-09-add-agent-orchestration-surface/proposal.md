...
The accepted scope is the agent-supervisor tool surface and skill described
below; no native scheduler or `fleet` driver is implied. Feature commit
`935b46dd` (merged by `1a5954cd`) is in current history. The implementation and
tests are anchored in the capability catalog/control routes, typed dispatch
store/status tests, session/issue/dispatch CLI smoke cases, and
`extensions/skills/supervise/SKILL.md`. The delta specs remain an accurate
description of the shipped design.
