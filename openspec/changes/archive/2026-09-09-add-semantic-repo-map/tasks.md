...
- [x] 8.2 `git add` the new modules before nix-build (flake source allowlist)
      — done at review time.
- [x] 8.3 Archive reconciliation: store-query, renderer, crawl, CLI smoke,
      catalog/MCP, and search-fallback evidence is recorded above. Feature
      commit `f37e9514`, follow-up `ed08f28b`, and merge `1384e4a3` are in
      current history; the former pre-PR full-CI reminder is not a remaining
      requirement.
