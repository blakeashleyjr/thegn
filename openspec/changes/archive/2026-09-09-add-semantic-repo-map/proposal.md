...
Feature commit `f37e9514`, corrective follow-up `ed08f28b`, and merge
`1384e4a3` are in current history. Store-query and pure-renderer tests,
incremental crawl coverage, hermetic CLI smoke, catalog/MCP scope tests, and
symbol-search fallback tests evidence the accepted tree-sitter-tier repo map.
Prompt injection, LSP workspace symbols, and richer ranking remain explicit
non-goals, consistent with the design and delta.
