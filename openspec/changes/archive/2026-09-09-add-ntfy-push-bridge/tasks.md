...
- [x] 4.1 Smoke: host runs green with push configured against an unreachable
      ...
      secret.
- [x] 4.2 Archive reconciliation: the pure routing/envelope tests, service
      request-shaping tests, smoke case, scoped `just quick`, and targeted
      nextest runs are the retained evidence. Feature commit `a82a22b2` and
      merge `dbbc7627` are in current history; the old reviewer-only full-CI
