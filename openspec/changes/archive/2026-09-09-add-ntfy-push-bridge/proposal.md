...
Feature commit `a82a22b2` and merge `dbbc7627` are in current history. Pure
route/envelope admission tests, ntfy request/stream parsing tests, daemon
catalog-dispatch wiring, probe coverage, and the unreachable-server smoke case
evidence the accepted best-effort publisher and guarded inbox design. The
proposal, security model, and deltas remain accurate; delivery is not promoted
to a durable queue by this change.
