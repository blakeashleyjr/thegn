# Add a credential broker: typed secret refs, SSH custody, host-key policy, audit
...
- **New capability `credentials`** with a typed `SecretRef` in `thegn-core`
  ...
  the type. Legacy issue-account and GitLab CI `Literal` tokens warn in
  `thegn config validate`; `thegn secret migrate` moves those supported fields
  into a `0600` file and rewrites the config to hold only the ref via the
  existing comment-preserving `config_write` path. VPN, snapshot, and MCP refs
  share the resolver but require an explicit config edit rather than claiming
  unsafe generic map-key rewriting.
...
- **One host-key policy table.** A `HostKeyPolicy` enum with four connection
  ...
  justification.
- **Agent-forwarding and pane secret exposure become policy.** `/run/user` is
  removed from every sandbox tier's default mounts; Sealed and SealedTunnel
  additionally drop `SSH_AUTH_SOCK` from environment passthrough. Hardened may
  retain the variable, but its socket is unreachable without an explicit mount.
  `thegn doctor` lists exactly which capabilities each tier would expose.
  `forward_agent` defaults off for user-declared hosts and is forced off for
  managed ephemeral instances; users may explicitly opt in on declared hosts.
- **Signing is a separate execution contract.** The additive
  `[identities.<name>.signing]` schema and pure resolution helper landed while
  this change was implemented, but pane/Git-process injection, automatic-sign
  semantics, precedence evidence, and help prose are owned in full by
  `bind-identity-commit-signing`. This change makes no runtime signing claim.
- **Audit trail without values.** Every broker resolve emits a structured
  ...
  configured refs, their backends and current presence outcomes. Free when no
  subscriber is installed (house instrumentation rule).
- **Hard rules as requirements**: supported legacy issue/CI token literals are
  warned and migratable; no secret value appears in argv, logs, or the SQLite
  DB; subprocesses receive secrets via environment or stdin only (the
  `ssh_shim` stdin-export discipline generalized to a requirement).
...
- **Roadmap**: group **H** items 104/105 (shipped — extended, not reopened);
  **AJ 431** (credential brokerage — agents never see raw keys: this delivers
  ...
  isolation), **757** (layered secret store — generalized). Group J remote work
  consumes the host-key table. Identity-bound commit signing is tracked by the
  focused `bind-identity-commit-signing` successor.
...
- **In-flight changes reconciled**: `add-mcp-proxy-hub` (shares the one
  ...
  `add-decoupled-identities` (the identity primitive whose additive signing
  schema is consumed by the focused signing successor), `add-env-setup-ux`
...
The broker rollout, managed-key selection/custody, all-provider rotation, and
destroy revocation are implemented. The `exec` secret backend remains a
reserved/rejected kind by design. Identity-bound commit-signing execution was
moved without weakening its requirement to `bind-identity-commit-signing`; it
is not a residual acceptance condition of this credential-custody change.

The unused `[credentials] audit_file` field was removed; tracing metadata and
the live presence audit are the complete bounded audit surface.
