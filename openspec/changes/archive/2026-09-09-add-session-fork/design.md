...
`--fork-worktree` on the CLI runs in two steps with distinct failure
...
- A missing cwd/worktree is not redirected to `$HOME`; the ordinary spawn or
  worktree-creation path refuses it and reports the failure.
- Scrollback hand-off uses the existing fixed 2,000-line snapshot bound. No
  additional size knob was added.
- The shipped `fork-session` UI action requests a plain sibling fork. Worktree
  and tab variants remain explicit CLI flags; no unimplemented UI prompt is
  part of this change's accepted scope.
