...
- [x] 5.2 `openspec validate --all --strict --no-interactive` passed during
      archive-readiness review (171 items, 0 failures).
- [x] 5.3 Confirmed the delta requirements are already present in the base
      `sandbox` and `state-db` specs from `507a46a1`; archive this change with
      spec application skipped to preserve subsequent base-spec extensions.
