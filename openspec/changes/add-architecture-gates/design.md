...
- **Crate boundaries via cargo-deny `wrappers`**, not a test: `just deps-audit` already runs `cargo deny check` in ci, and `wrappers` expresses "only these crates may depend on X directly" exactly. Wrapper lists are verified with `cargo tree -e normal -i <crate> --depth 1` before committing. `thegn-core → thegn-media`/`sysinfo` is documented as intended (leaf crates are allowed).
...
- **`let_underscore_must_use` as warn (→ `-D warnings`), `let_underscore_future` as deny** — the dangerous subset fails immediately; the broader `let _ =` discipline is the file-level ratchet.
