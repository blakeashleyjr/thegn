...
- [x] 2.1 `thegn_core::fsperm::{restrict_to_owner, restrict_dir_to_owner}`
      (0600/0700 on unix, protected owner-only DACL rebuilt through the
      PowerShell ACL API on Windows) + unix unit tests.
...
- [ ] 5.2 On-machine (joins the phase-4 checklist): activity dots move under
      a busy pwsh pane; DACL-restricted token file readable by owner only;
