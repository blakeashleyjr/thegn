...
| Writes                            | Always the git CLI, via `util::git_cmd`/`GitLoc` (env-scrubbed, `GIT_OPTIONAL_LOCKS=0`), serialized by the `thegn-git.lock` flock. Reads are bounded by `THEGN_GIT_READ_TIMEOUT_SECS`; general and network writes stay unbounded, while the two automated commit sites use a dedicated 120-second non-interactive deadline.                    | `crates/thegn-core/src/util.rs:314`, `crates/thegn-svc/src/git/background_commit.rs`, `crates/thegn-svc/src/git/plumbing.rs`         |
...
| Signing                           | `[git] override_gpg` applies to automated dirty snapshots; panel commits keep the per-commit `^S` inherit→sign→no-sign cycle (Y 328); fold commits honor `[merge_queue] sign_commits` through `commit-tree -S`. Both automated commit sites use the bounded non-interactive runner; key/format stay delegated to Git's active identity config. | `crates/thegn-svc/src/git/background_commit.rs`, `crates/thegn-svc/src/git/plumbing.rs`, `crates/thegn-host/src/integrate.rs`        |
...
- **Fold/land signing is a policy**: `[merge_queue] sign_commits = false`.
  ...
  - Automated commits use a dedicated runner because the commit message must
    travel over stdin. It writes and explicitly closes that pipe before
    waiting, clears terminal/askpass/display prompt surfaces, and on Unix
    starts a new session so no controlling terminal can be inherited.
  - The commit-only deadline defaults to 120 seconds so remote/provider
    latency remains viable. `THEGN_BACKGROUND_COMMIT_TIMEOUT_SECS` is a narrow
    launcher/test seam, not a general Git-write or user-config timeout. On
    expiry the Unix process group is killed and the direct child is reaped.
    Output is captured to bounded temporary regular files rather than pipe
    readers, so even a non-Unix descendant retaining inherited output handles
    cannot hold the caller open. General `run_stdin` and network writes remain
    unbounded.
