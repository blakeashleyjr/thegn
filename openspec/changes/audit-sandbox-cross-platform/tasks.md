...
- [ ] 1.1 New pure module `thegn-core/src/sandbox_matrix.rs`: `EnforcementRow`
...
- [ ] 1.2 Unit tests to the 95% core gate: every cell of the Linux, macOS, and
...
- [ ] 1.3 Fix the `HostProcess` escape note: drop the Landlock/Seatbelt claim
      (thegn applies neither); reword to "no kernel boundary; resource/lifetime
      scoping only where wrapped".
- [ ] 1.4 `thegn doctor`: render the host's matrix column beside the existing
...
- [ ] 1.5 `docs/help/` sandbox page: document the matrix and what each cell
      means (no new action ids; prose only, keep the help ratchets green).

## 2. Isolation floor

- [ ] 2.1 Config: `[sandbox] isolation_floor` (empty | `shared-kernel` |
      ...
      entries.
- [ ] 2.2 Pure comparison in core: class ordering, `ProviderManaged` outside
      ...
      shared-kernel.
- [ ] 2.3 Launch path: apply the floor after backend/runtime resolution;
...
- [ ] 2.4 Trust clamp: repo overlay may only raise the floor / harden the miss
      policy; denial surfaced through the existing clamp reporting +
      `thegn config explain sandbox.isolation_floor`.
- [ ] 2.5 Ssh placement: compare over what the remote probe proves;
      `Unreachable` counts as a miss (design open question 2 resolved in
      implementation).

## 3. Windows honesty

- [ ] 3.1 Reclassify `Backend::WinJobObject` to the host-process class in
...
- [ ] 3.2 Matrix Windows column: OCI-declined-by-policy and
      limits-deferred cells stated while true (reconcile wording with
      `add-windows-parity`).

## 4. Agent-workload floors

- [ ] 4.1 Extend the agent-task configuration (coordinate with
...
- [ ] 4.2 `agent_run.rs`: wrap the task argv via `sandbox::enter_argv` when
      opted in; floor check per phase 2; keep `wrap_background_argv`
      fail-safe semantics for the resource wrap.
- [ ] 4.3 Failure attribution: a fail-closed floor miss or sandbox setup
...
- [ ] 4.4 Document the recommended agent posture in
