...
- [x] 1.1 Add `crates/superzej-core/src/aggregate.rs` and export from `lib.rs`.
...
- [x] 5.1 `cargo test -p superzej-core` green (new aggregate tests).
- [x] 5.2 `cargo test -p superzej-host` green (new across-section test); `cargo clippy` + `cargo fmt --check` clean; god-file ratchet OK (no growth in run.rs/chrome.rs/keymap.rs).
