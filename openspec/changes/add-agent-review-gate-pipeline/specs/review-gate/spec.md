...
Every pipeline finding SHALL carry a `severity` in {info, warning, error} and an `action` in {auto-fix, ask-user}, and each stage SHALL define an `auto_fix_limit` bounding how many auto-fix findings it may apply automatically.

#### Scenario: Info-level mechanical finding

- **WHEN** a stage produces an `info` finding with action `auto-fix` within its `auto_fix_limit`
- **THEN** the fix is applied automatically without parking

#### Scenario: Blocking finding parks for a decision

- **WHEN** a finding has action `ask-user`, or its stage's `auto_fix_limit` is reached
- **THEN** the run parks and waits for an explicit resolution rather than self-applying

### Requirement: Review auto-fix is disabled by default

The review stage SHALL default its `auto_fix_limit` to 0 so that review findings park for a decision, and MAY be re-enabled per repo or globally via configuration.
...
A parked finding SHALL be resolvable by exactly one of approve (accept as-is), fix (apply the suggested change), or skip (drop it); the resolution surface SHALL be available both in the human review pane and through a structured, non-interactive contract usable by the embedded agent.

#### Scenario: Human resolves in the review pane

- **WHEN** the user selects approve, fix, or skip on a parked finding
- **THEN** the finding is resolved accordingly and the run advances
...
- **WHEN** superzej restarts while a run is parked at a gate
