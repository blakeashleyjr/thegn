...
`xdg_config_home()`, `superzej_dir()`, `sandbox::resolve` env-passthrough,
...
- Reroot: `SUPERZEJ_DIR`, `XDG_STATE_HOME` (DB + logs), and credential vars
...
- `profiles/<p>/{state/superzej.db, logs, activity.json, audit.log, run/,
...
- Profile singleton via advisory `flock` on `profiles/<p>/run/szhost.lock`
  (`LOCK_EX|LOCK_NB`, one-shot — never a poll loop). Launch = spawn a terminal
  running `szhost --profile=X`; best-effort X11 focus via OSC-title marker.
