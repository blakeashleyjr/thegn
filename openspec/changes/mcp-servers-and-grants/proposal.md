...
superzej ships one agent (the managed pi) with hard-coded house tools, but users
can't extend the agent with their own MCP servers, and superzej has no
...
npm:install, glob-scoped). This change brings that pattern to superzej.

## What Changes

- Add a pure **capability-grant model** in `superzej-core` (`grants.rs`): a
...
- **Consume it in the agent layer**: `szhost agent setup` merges the
  ...
  alongside the `superzej-acp` house tools.
- Add a **`szhost mcp` CLI** (`list`, `emit`, `install <name>`) and surface
  declared servers + their grants in **`szhost doctor`**. Acquisition is
  grant-checked: a server whose `source`/launch isn't covered by a matching
  grant is refused with a clear message.

Non-goals (deferred): running MCP servers as long-lived superzej-supervised
...
- `mcp-servers`: user-declared MCP servers — the `[mcp_servers.<name>]` config,
  ...
  injection, the `szhost mcp` CLI, and doctor reporting.

## Impact

- **Code:** new `crates/superzej-core/src/grants.rs` + `crates/superzej-core/src/mcp/config.rs`
  (+ `mcp/mod.rs` `pub mod config;`, `lib.rs` `pub mod grants;`); `config.rs`
  gains a `mcp_servers` map field; new `crates/superzej-host/src/cmd/mcp.rs`
