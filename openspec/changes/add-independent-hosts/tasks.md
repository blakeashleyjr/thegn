...
- [x] 1.1 `trust_class.rs`: the T0–T3 ladder, derivation from probed
...
- [x] 1.2 `host_probe.rs`: `Headroom` + `parse_headroom` (KEY=VALUE) +
...
- [x] 1.3 `host_machine.rs`: `Draining` durable state (tag round-trip,
      absorb-everything totality, resume no-op) — **unit tests**.
- [x] 1.4 `host.rs`: `HostCaps` gains cgroup_v2 / userns / nproc /
      mem_total_kb (serde defaults keep pre-v35 caps_json readable) —
      **unit tests**: probe parse + back-compat round-trip.
- [x] 1.5 Scheduler trust upgrade: `HostSnapshot.trust` +
