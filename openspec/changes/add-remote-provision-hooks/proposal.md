...
Let users bring their own remote infrastructure to superzej's remote "sprite"
...
superzej already models remote placement (`Placement { Local | Ssh | K8s |
...
Given superzej's existing sprite + pool investment, exposing a documented per-task
...
- **A managed cloud offering** — superzej runs the user's own infra via their
