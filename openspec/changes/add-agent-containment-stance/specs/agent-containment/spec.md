...
- **THEN** the process runs on the host and thegn logs that sandboxing was explicitly disabled
...
thegn SHALL NOT transmit usage data, source code, or prompts off the machine. State is kept in local SQLite under `$XDG_STATE_HOME`; the only outbound traffic SHALL be to endpoints the user has explicitly configured (git remotes, GitHub via `gh`/octocrab, and the LLM proxy the user points at).

#### Scenario: No background beacon

- **WHEN** thegn runs normally with no user-configured remote operation in flight
...
- **WHEN** thegn performs a network operation
...
### Requirement: thegn remains a viewer / VCS client

thegn SHALL remain a terminal-native viewer / VCS client and SHALL delegate editing to the user's `$EDITOR`. It SHALL NOT embed an in-app code editor (e.g. Monaco), SHALL NOT embed a browser, and SHALL NOT provide a desktop-automation / computer-use surface.

#### Scenario: Editing hands off to $EDITOR

- **WHEN** the user opens a file to edit from within thegn
- **THEN** thegn launches the configured `$EDITOR` rather than editing the file in an embedded editor

#### Scenario: No embedded browser or computer-use

- **WHEN** an agent or workflow needs web content or GUI automation
- **THEN** thegn provides no embedded browser and no computer-use surface; such capabilities are out of scope
