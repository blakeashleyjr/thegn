...
## 1. Operating-agent definitions (superzej-core)
...
- [ ] 1.1 `operating_agent.rs`: parse a markdown+front-matter definition into
      ...
      `.superzej/agents/*.md` (repo) over the global agents dir by `id` —
...
## 2. Skill definitions (superzej-core)

- [ ] 2.1 `skill.rs`: parse + validate a `SKILL.md` (name, description, optional
      params) resolved from `.superzej/skills/<name>/` + global —
      **unit tests**: valid parse, missing name/description rejected, param parse.

## 3. Tool restriction at the AR/ACP seam (superzej-host)
...
## 4. Skill injection wiring (superzej-host)
