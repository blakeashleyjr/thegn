...
## 1. Authoritative metrics (superzej-proxy / superzej-core)
...
## 2. Compaction detection (superzej-core)
...
## 3. Fleet model + surfaces (superzej-host)
...
- [ ] 3.3 Orphan-port flag: mark a detected `forward.rs` port whose owning process
      has exited.
- [ ] 3.4 `szhost fleet [--json]` subcommand emitting the read-only rollup (the
      `cmd/issue.rs`/`config.rs` `--json` pattern); it MUST NOT issue a model call.

## 4. Docs + validate

- [ ] 4.1 Document the fleet view, its metrics, and `szhost fleet --json` in the
