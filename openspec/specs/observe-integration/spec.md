...
Integration governs how Observe embeds into thegn: it mounts as an app-tab via the tg-kit `AppTile` contract, contains panics so a failing panel never crashes the host, layers its config into thegn's TOML with redacted secrets, and starts fully usable with no configuration against built-in sources while operating offline with no external runtime.
