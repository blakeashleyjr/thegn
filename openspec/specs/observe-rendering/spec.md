...
Rendering turns frames into terminal charts within Observe's cost and responsiveness budget. Series are downsampled to the target width before plotting so cost is bounded regardless of cardinality; a capability-detected renderer picks the best available protocol with a braille/block fallback and truecolor→256→16→mono degradation; and query execution runs off the event loop, pulsing the `TerminalWaker` so rendering never blocks or polls.
