...
The editor seam (`thegn_core::editor`) is how every "open this file in the user's editor" path — panel keys, drawer commands, git after-actions, test/problem/symbol jumps, `thegn config edit` — resolves _which_ editor, _what shell line_ opens the file at a line/column in that program's own syntax, and _where_ it runs (a center pane for terminal editors, a detached window for GUI ones). One ladder, one program table, one placement decision, probed by `thegn doctor`.
