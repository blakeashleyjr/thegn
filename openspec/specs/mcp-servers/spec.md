...
superzej SHALL let users declare MCP servers in config (`[mcp_servers.<name>]`)
with a launch command, arguments, and environment, and MUST emit them as a
standard `mcpServers` settings block that the managed agent consumes. `szhost
...
- **WHEN** `szhost agent setup` runs with declared MCP servers
...
superzej SHALL acquire a declared server's binary through the shared managed-tool
...
- **WHEN** `szhost mcp install <name>` runs for a server whose grants cover its
  source acquisition
...
- **THEN** superzej refuses the install and names the missing capability

### Requirement: Declared servers and grants are inspectable

`szhost mcp list` SHALL list declared servers with their launch spec and grants,
`szhost mcp emit` SHALL print the `mcpServers` settings block, and `szhost
doctor` SHALL report declared servers and their grants.

#### Scenario: list and doctor surface declared servers

- **WHEN** MCP servers are declared and `szhost mcp list` (or `doctor`) runs
