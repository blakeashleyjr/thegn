...
TBD - created by archiving change add-decoupled-identities. Update Purpose after archive.
...
- **WHEN** no `[identities.<name>]` tables exist and nothing sets `identity =`
...
### Requirement: The active identity is switchable without restart

The user SHALL be able to switch the active identity from a switcher, binding it
at the focused workspace or at global scope when no repository is focused. The
binding SHALL take effect for subsequently spawned panes without restarting the
process and MUST NOT introduce any polling. The underlying resolution order MAY
also consume a worktree-scoped binding written by an internal caller, but this
change does not require a worktree-scope selector in the switcher.

#### Scenario: Switching identity rebinds new panes

- **WHEN** the user selects a different identity while a repository workspace is
  focused
- **THEN** panes spawned afterward in that workspace resolve the new identity's
