# provider-native-exec Specification

## Purpose

CLI-free native execution for managed providers (e.g. Sprites over WSS), so agents run in the remote environment without a local provider CLI.
