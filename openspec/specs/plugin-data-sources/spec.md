# plugin-data-sources Specification

## Purpose

How a configured calendar command acts as a seam-specific NDJSON data source:
bounded execution/ingestion, calendar-only manifest negotiation, query context,
and forward-compatible event data. This does not make `DataSource` a generally
accepted contribution in the compositor's plugin host contract.

## Requirements

### Requirement: An external program can act as a calendar data source

thegn SHALL be able to run a configured calendar-account program and read
structured events back, so a calendar integration can be written in any
language without being built into thegn. The program's output SHALL be
newline-delimited JSON messages using the existing plugin API framing. This is
a specialized calendar backend; it SHALL NOT cause the general plugin host to
negotiate `DataSource` contributions.
...
### Requirement: Calendar command ingestion is bounded and diagnosable

Running a calendar command MUST be bounded in time and accepted output.
Exceeding the output limit MUST truncate and report truncation rather than
terminating the program mid-write. A command that exceeds its time limit MUST
have its entire process group terminated, not just the process thegn started,
so a program that spawns children cannot outlive the run. The process is not an
OS sandbox and retains the invoking user's authority; configuration and help
