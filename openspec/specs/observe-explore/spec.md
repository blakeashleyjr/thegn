...
Explore mode is Observe's ad-hoc query surface: a split query-editor and result-viz view that auto-detects frame shape to pick a sensible default visualization. It keeps query history, supports stacked A/B/C queries with per-query show/hide, and offers syntax-aware autocomplete driven by source discovery.
