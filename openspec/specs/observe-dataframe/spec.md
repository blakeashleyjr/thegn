...
The data frame is Observe's unified in-memory result model: query results from every data source normalize into columnar, typed, null-aware frames carrying field and frame metadata. Frames have first-class time semantics with wide/long conversion, and support low-copy slicing, downsampling, and incremental diffing so that streaming appends and high-cardinality results stay cheap to process and render.
