...
- **Help ratchet (`crates/thegn-host/src/help/ratchet_tests.rs`, runs in
  ...
  cheap, and was how the corpus drifted while coverage read ~100%. Four
  pinned-debt allowlists, all shrink-only; three are regenerated by
  `just help-ratchet-update`: `test/help-ratchet.txt` (unclaimed actions),
  `test/help-prose-ratchet.txt` (claimed but unwritten), and
  `test/help-panel-prose-ratchet.txt` (unwritten panel sections); the fourth,
  `test/help-context-ratchet.txt` (unclaimed panel contexts), is frozen —
  seeded empty with no updater, the ratchet refuses additions. The keybindings
