...
- **Fix:** Add a smoke check mirroring the merge-drain block: create a worktree, commit, `"$TG" land "$WT"`, assert the commit is in the target's log and the success line printed; plus one negative (detached HEAD ⇒ clear error, exit non-zero).
- **Status:** ⏳
