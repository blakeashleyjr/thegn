...
thegn today is **one process, one world**: a single global config
(`$XDG_CONFIG_HOME/thegn/config.toml`), a single SQLite DB
(`$XDG_STATE_HOME/thegn/thegn.db`), one set of credentials (whatever the
...
`env_passthrough` from `std::env::var` (`crates/thegn-core/src/sandbox.rs:255`),
`gh::resolve_token` reads `GH_TOKEN`/`GITHUB_TOKEN` from the process env
(`crates/thegn-core/src/gh.rs:75`), and the three path roots —
`xdg_config_home()`, `xdg_state_home()`, `thegn_dir()`
(`crates/thegn-core/src/util.rs:16-37`) — read env on every call. So the
...
### Startup sequence (in `crates/thegn-host/src/main.rs`, before tokio/threads)
...
```
$XDG_STATE_HOME/thegn/                    # real root (unrooted)
  profiles/
    default/                                 # legacy data migrates here
      state/thegn.db                       # per-profile DB (WAL, schema v6+)
      state/logs/
      activity.json  audit.log
      run/            thegn.lock, ssh-* sockets   # per-profile, never shared
...
`~/.thegn/worktrees/` and absolute worktree paths are baked into git's
...
`Config::load_layered` (`crates/thegn-core/src/config.rs:1438`) gains a
...
5. Per-workspace overlay (`[workspace.<slug>]`) → repo-root `.thegn.*` →
   `THEGN_*` env → `--set` (unchanged existing layers).
...
1. **`pane.rs::spawn_with_env` inherits the entire parent env** and only _adds_
   ...
   therefore sees every var thegn inherited — including any `GH_TOKEN`,
...
- **Singleton per profile via advisory `flock`** on
  `profiles/<p>/run/thegn.lock` (`LOCK_EX | LOCK_NB`), held for process
...
- **Switching/launching a profile window** — thegn owns a raw TTY and has no
  ...
  - _Primary (works everywhere incl. Wayland/SSH):_ spawn a new terminal
    emulator running `thegn --profile=X` via a per-profile, configurable launch
    command (`$TERMINAL -e …`).
...
- `crates/thegn-host/src/main.rs` — startup env-rerooting (first statements),
  `--profile` flag, migration trigger, `flock` singleton, profile-launch action.
- `crates/thegn-core/src/util.rs` — `ProfilePaths` (`OnceLock`) typed
  accessor; path helpers read rerooted env.
- `crates/thegn-core/src/config.rs` — two-root layering (shared base +
  profile + subprofile); promote `ProfileConfig` to a full overlay.
- `crates/thegn-host/src/agent.rs` — `launch_spec` assembles profile/subprofile
  credential env (shared seam for agent + shell panes).
- `crates/thegn-host/src/pane.rs` — `spawn_with_env` clear-then-allowlist.
- `crates/thegn-core/src/sandbox.rs` — fix `~/.gitconfig` mount → profile
  gitconfig; verify env-passthrough picks up rerooted creds; document
  file_access/network leaks.
- `crates/thegn-core/src/gh.rs` — token resolution honors `GH_CONFIG_DIR`/
  profile token.
- `crates/thegn-core/src/db.rs` — per-profile DB path via rerooted env;
  comms gets its own DB file.
- `crates/thegn-host/src/run.rs` — `Subsystems` holder; subprofile
...
1. **Path/firewall unit tests** (core, gated 95%): given `THEGN_PROFILE=work`,
...
5. **Subprofile switch**: in comms, switch work↔personal; assert comms panes/DB
   ...
   wakeups introduced (`THEGN_LOG` waterfall / idle-CPU check).
