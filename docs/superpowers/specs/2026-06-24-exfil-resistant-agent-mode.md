...
- **Broker** (host process; holds the API key; the only thing with network) *is*
...
the *container* down — read-only root, dropped caps, `network=none`, pids/mem
caps — while the agent process still runs *inside* it. This exploration is a
