...
`agent` app tab was dropped (`crates/thegn-host/src/apps/mod.rs` registers no
...
> two-layer control plane: `tgproxy` lower plane + ACP upper plane) and the
> behavior-of-record is `openspec/specs/agent/spec.md`. The real, committed agent
> surface lives in `crates/thegn-svc/src/acp/` (`AcpClient`),
> `crates/thegn-core/src/acp/` (protocol data model),
> `extensions/thegn-acp.ts` (the in-pane pi bridge, pinned pi `0.80.2`), and
> `crates/thegn-host/src/{bouncer,relay}.rs` (the sealed-sandbox tool gate +
> model-traffic relay). thegn launches a managed `pi` into a worktree pane and
...
> pi path — model traffic routes through `tgproxy` under a per-worktree virtual
...
thegn's agent subsystem is a **native, embedded coding harness in Rust** —
...
(group **R**: adapt to _external_ harnesses via ACP/adapters). thegn ships its
...
through a Phase 4 autonomous-coding MVP). thegn's job is the **substrate** the
...
- The provider reads `OPENAI_BASE_URL` / `OPENAI_API_KEY` / `OPENAI_MODEL` from
  the environment — which is exactly thegn's proxy seam.

## Decisions

### 1. The `AppTile` lives in the host, not the submodule (for now)

The `agent` tab is implemented as `thegn-host/src/apps/agent.rs` (`AgentUi:
tg_kit::AppTile`), depending on `termite-core` as a path dep. We did **not**
relocate the tile into `termite-agent` (the way `chat` lives in `termite-chat`),
because termite-agent has no `tg-kit` dep and no UI library crate (its TUI lives
...
at `http://{llm_proxy.listen}/v1` (the local `tgproxy`) and **mints a per-worktree
...
thegn's `sandbox.rs` is that missing layer. The host registers its own
...
- Phase A: `crates/thegn-host/src/apps/agent.rs` (`AgentUi` tile), `termite-core`
...
- `cargo build -p thegn-host`; the `agent` tab appears (per `tab_order`,
  `Alt+<n>`) and opens. With no key/proxy, a submit shows the error in-transcript.
- With `[llm_proxy].enabled` + `tgproxy` running and `OPENAI_API_KEY` set, a turn
  crosses the proxy (proxy logs).
- `cargo test -p thegn-host apps::` and `cargo test -p thegn-core --lib
config::tests`.
