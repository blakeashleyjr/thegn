...
superzej currently treats git worktrees and repos as the primary organizational unit. Workspaces hold worktrees. However, developers often need raw terminal access to systems (local machines, remote servers, cloud environments) that aren't tied to a specific repo or git worktree.
...
1. **Database Layer (`crates/superzej-core/src/db.rs`)**
   - New table: `terminals`
...
2. **Sidebar Model (`crates/superzej-host/src/sidebar.rs`)**
...
3. **Session & Process Management (`crates/superzej-host/src/session.rs`)**
...
4. **Execution Layer (`crates/superzej-host/src/pane.rs` & `emulator.rs`)**
...
5. **Action & Palette Integration (`crates/superzej-host/src/chrome.rs`, `center.rs`)**
...
- [ ] Implement remote connections (`ssh`, `mosh`) using native fallback subprocess execution (leveraging `superzej-svc` if applicable, or direct `Command::new`).
