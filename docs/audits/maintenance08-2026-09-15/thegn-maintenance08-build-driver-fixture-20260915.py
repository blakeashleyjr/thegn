...
approval=json.loads(Path('/tmp/thegn-maintenance08-primary-fixture-compile-approval-20260915.json').read_text())
...
unit='thegn-maintenance08-'+phase+'-fixture-20260915'
base=Path('/tmp/'+unit)
...
args=['cargo','clippy','--locked','-p','thegn-svc','--lib','--tests','--','-D','warnings'] if phase=='clippy' else ['cargo','test','--locked','-p','thegn-svc','--features','control-grpc','--lib','--no-run','--message-format=json-render-diagnostics']
if phase=='native':
 prior=json.loads(Path('/tmp/thegn-maintenance08-clippy-fixture-20260915.json').read_text())
