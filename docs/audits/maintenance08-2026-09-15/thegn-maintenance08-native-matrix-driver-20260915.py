...
source=json.loads(Path('/tmp/thegn-maintenance08-primary-compile-approval-final-20260915.json').read_text())['source']
build=json.loads(Path('/tmp/thegn-maintenance08-native-artifact-admission-20260915.json').read_text())
assert build['source']==source and build['exit_code']==0 and len(build['artifacts'])==2
plan=json.loads(Path('/tmp/thegn-maintenance08-native-selection-plan-20260915.json').read_text())
selection=[]
for label in ['svc','host']:
...
manifest=Path('/tmp/thegn-maintenance08-native-selected-20260915.json')
manifest.write_text(json.dumps({'source':source,'selection':selection},indent=2)+'\n')
print(json.dumps({'selected':{label:sum(x['crate']==label for x in selection) for label in ['svc','host']},'manifest':str(manifest)}),flush=True)
...
log=Path('/tmp/thegn-maintenance08-focused-native-20260915.log');receipt=Path('/tmp/thegn-maintenance08-focused-native-receipt-20260915.json')
