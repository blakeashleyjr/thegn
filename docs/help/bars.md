...
merge-queue depth, PR-queue depth, disk usage, metrics targets. With the bar focused,
...
under the cursor, `a` clears all. Merge-queue rows act in place — `l` lands
a gated-green branch, `r` retries a blocked one, `x` removes it, `m` jumps
to the full Work ▸ Merge queue section. Ephemeral confirmations ("Landed",
