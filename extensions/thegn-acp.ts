...
export default function (pi: ExtensionAPI) {
  ...
  // Lower plane: route the model through thegn's proxy. pi flushes provider
  // registrations at startup (before session_start), so this MUST happen here in
  // the factory — not at runtime via providers/set. thegn hands us the proxy
  ...
  // egress at all. THEGN_PROXY_UNIX points at a bind-mounted unix socket the
  ...
  const proxyUnix = process.env.THEGN_PROXY_UNIX;
  if (proxyUnix) {
    installUnixProxyFetch(proxyUnix);
  }

  let proxyModel: string | null = null;
  const proxyBase = process.env.THEGN_PROXY_BASE_URL;
  if (proxyBase) {
    const api = process.env.THEGN_PROXY_API || "anthropic-messages";
    proxyModel = process.env.THEGN_PROXY_MODEL || "model-proxy/standard";
    const apiKey = process.env.THEGN_PROXY_KEY || "sk-thegn";
    pi.registerProvider("thegn-proxy", {
      baseUrl: proxyBase,
      api,
      apiKey,
      models: [{
  ...
  // Send a JSON-RPC *request* to the host (thegn) and await its reply. Used by
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    // thegn hands us the port it already bound. Prefer the --acp-port flag
    // when explicitly set, otherwise the ACP_PORT env var (the reliable path:
    // env crosses thegn's `sh -lc` + sandbox wrapping, an appended flag does
    // not). We MUST bind exactly this port — falling back to 0 (OS-ephemeral)
    // would make thegn connect to the wrong port.
    // Prefer a bind-mounted unix socket (sealed sandbox — crosses the netns
    // without network); otherwise the TCP port (non-sandboxed). thegn sets one.
    ...
    // "The bouncer" (THEGN_BOUNCER=1): override pi's built-in file/shell tools
    // so they route back to thegn over ACP. thegn runs them inside the
    ...
    if (process.env.THEGN_BOUNCER === "1") {
  ...
  function registerBouncerTools() {
    ...
    pi.registerTool({
      name: "read",
      label: "read (thegn)",
      description: "Read a worktree file via thegn (scoped to the sandbox).",
    ...
    pi.registerTool({
      name: "edit",
      label: "edit (thegn)",
      description: "Edit a worktree file via thegn (gated allow/deny).",
      parameters: {
        type: "object",
        properties: {
          path: { type: "string" },
      ...
      async execute(_id, params) {
        try {
          const r = await sendHostRequest("thegn/edit", { path: params.path, edits: params.edits });
    ...
    pi.registerTool({
      name: "write",
      label: "write (thegn)",
      description: "Write a worktree file via thegn (gated allow/deny).",
      parameters: {
        type: "object",
      ...
      async execute(_id, params) {
        try {
          const r = await sendHostRequest("thegn/write", { path: params.path, content: params.content });
  ...
  function handleAcpMessage(line: string, socket: net.Socket, ctx: ExtensionContext) {
    ...
    if (msg.id !== undefined && pendingRequests.has(msg.id)) {
      // It's a response to an RPC request we sent to the client (thegn)
    ...
    if (msg.method === "initialize") {
      ...
      // The host (thegn) is bridging the U layer (tgproxy) over ACP!
      ...
      pi.setModel({ id: models?.[0]?.id || "thegn-default", provider: providerId } as any);
      
      if (msg.id) sendAcp(socket, { jsonrpc: "2.0", id: msg.id, result: {} });
    } else if (msg.method === "mcp/connect") {
      // thegn is providing an MCP connection
  ...
  pi.on("turn_end", (event, ctx) => {
    ...
    // Report context usage back to thegn for live tracking
  ...
  pi.on("agent_end", (event: AgentEndEvent, ctx: ExtensionContext) => {
    ...
    // Lifecycle: notify thegn the agent finished (fires for user-driven turns
    ...
    // If thegn drove this turn via session/prompt, send its final response.
    if (currentPromptId) {
      sendAcp(activeSocket, {
        jsonrpc: "2.0",
        id: currentPromptId,
  ...
  function installUnixProxyFetch(socketPath: string) {
    try {
      ...
      const PLACEHOLDER = "proxy.thegn.internal";
      ...
      console.error("thegn: failed to route the model proxy over the unix socket:", e);
    }
  }

  // Tool ownership is mode-dependent:
  //  - Additive (default): thegn does NOT override pi's built-in bash/read/
  //    edit/write. pi runs them natively (in-process, sandboxed iff its pane is),
  //    keeping pi's inline-diff edit UX; thegn stays additive (observe via
  //    session/update, route the model via the proxy, house tools over MCP).
  //  - Bouncer (THEGN_BOUNCER=1, see `registerBouncerTools`): those four tools
  //    are overridden to route over ACP so thegn runs them inside the sealed
