...
impl Drop for UnixTerminal {
    fn drop(&mut self) {
        // Best-effort cleanup must not unwind, including while another panic is
        // unwinding. A hung-up terminal must not skip later mode/signal cleanup.
        ...
        self.write.modify_other_keys(0).ok();
        self.exit_alternate_screen().ok();
        self.write.flush().ok();
