...
//!
//! [`Theme::prism`] mirrors superzej's default chrome palette
//! (`superzej-core/src/theme.rs`). Embedded, the host converts its live
//! `Palette` into a [`Theme`] (so theme-cycling and user `[theme.colors]`
//! overrides flow through). Standalone, [`Theme::load_superzej_config`] reads
//! the same `config.toml` so an app run on its own still matches the user's
//! superzej look — without this crate depending on `superzej-core`.
...
/// A semantic color slot. Names mirror the superzej `Palette` fields so the
...
impl Theme {
    /// The prism defaults — byte-for-byte the same sRGB as superzej's default
    /// `Palette` (kept in sync by a parity test in `superzej-host`).
    pub fn prism() -> Theme {
        Theme {
            bg0: (11, 14, 22),
            bg1: (16, 20, 31),
    ...
    /// Load the user's superzej theme from `config.toml` (XDG), falling back to
    ...
    pub fn load_superzej_config() -> Theme {
    ...
    fn apply(&mut self, t: &ThemeSection) {
        ...
        // accent recolors both the accent and (by superzej default) the focus.
...
/// `$XDG_CONFIG_HOME/superzej/config.toml`, or `~/.config/superzej/...`.
fn config_path() -> Option<std::path::PathBuf> {
    ...
    Some(base.join("superzej").join("config.toml"))
}
...
// --- the slice of superzej's config.toml we read (serde-tolerant) ---
