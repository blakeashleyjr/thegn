...
//!
//! A shim can also be **orphaned** at [`CHAINED_NAME`] without our hook ever
//! holding the slot: a stray `prek install` reclaims the slot before our
//! installer next runs, and the shim it displaced is left where the allow path
//! execs it. That shape is unreachable by the repair above — it only fires when
//! *our* hook is in the slot, and here a shim is — so it used to be permanent,
//! failing every merge in the checkout. Two halves fix it: [`plan`] reports
//! `drop_chained` so [`install`] deletes the orphan, and the script itself
//! refuses to exec a shim from that position, which covers the window before
//! thegn is next started. [`audit`] reports the arrangement for `thegn doctor`,
//! because the installer only logs its plan at `debug` and this failure is
//! otherwise invisible until someone merges.
...
# A pre-commit framework's shim can end up parked at .thegn-orig — a stray
# `prek install` reclaims the slot before our installer next runs, orphaning
# whatever was there. Exec'ing one is fatal: a shim invoked while it is not the
# installed hook detects migration mode and exits non-zero, so EVERY merge in
# the checkout fails on hook plumbing. It is generated, never a user's work, so
# skip it. (thegn's installer also deletes it at startup; this is the half that
# works before thegn has been restarted.)
is_shim() {
    grep -qE 'hook-impl|generated by prek|generated by pre-commit' "$1" 2>/dev/null
}
# Allow path: delegate to any hook we displaced, else succeed.
pass() { [ -x "$chained" ] && ! is_shim "$chained" && exec "$chained" "$@"; exit 0; }
...
pub struct Plan {
    ...
    /// Delete a framework shim orphaned at [`CHAINED_NAME`] — one the slot has
    /// no use for, because a *different* shim already owns the slot. Nothing
    /// may exec a shim from there, and it is generated, not a user's work.
    pub drop_chained: bool,
...
pub fn plan(slot: Option<&[u8]>, legacy: Option<&[u8]>, chained: Option<&[u8]>) -> Plan {
    ...
    // A shim left at `.thegn-orig` that no branch below is going to consume.
    // `restore_shim` moves it into the slot, and `InstallAction::Chained`
    // overwrites it with the hook it displaces; anything else leaves it sitting
    // where the allow path would exec it, which fails "migration mode" and
    // blocks every merge. Resolved once the action is known, below.
    let orphan_shim = !restore_shim && chained.map(classify) == Some(HookKind::Shim);
    ...
    if effective == Some(HookKind::Shim) {
        ...
        return Plan {
            restore_shim,
            drop_chained: orphan_shim && action != InstallAction::Chained,
...
pub fn install(hooks_dir: &Path) -> std::io::Result<Plan> {
    ...
    // Likewise clear an orphaned shim before anything writes there. Tolerate a
    // concurrent unlink: the point is that the path ends up gone.
    if p.drop_chained
        && let Err(e) = std::fs::remove_file(&chained)
...
/// What is wrong with the arrangement on disk, worst first.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Fault {
    /// A framework shim sits at [`CHAINED_NAME`], where the guard's allow path
    /// execs it — and a shim invoked outside its own slot fails migration mode.
    /// Every `git merge` in this checkout fails on hook plumbing until it goes.
    ShimChained,
    /// The guard is not anywhere it will be invoked, so a sandbox merge against
    /// the canonical checkout is unguarded.
    NotInstalled,
}

impl Fault {
    /// A one-line remedy, for `thegn doctor`.
    pub fn remedy(self) -> &'static str {
        match self {
            Fault::ShimChained => {
                "delete <hooks>/pre-merge-commit.thegn-orig (thegn removes it on next start)"
            }
            Fault::NotInstalled => "start thegn in this repo, or set [git] merge_guard = true",
        }
    }
}

/// A read-only view of the three hook files — what `thegn doctor` reports.
/// Separate from [`plan`] because it must not decide anything: a checkout the
/// installer has not visited yet still has to be describable.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Audit {
    pub slot: Option<HookKind>,
    pub legacy: Option<HookKind>,
    pub chained: Option<HookKind>,
}

impl Audit {
    /// Classify three hook bodies. `None` means the file is absent.
    pub fn of(slot: Option<&[u8]>, legacy: Option<&[u8]>, chained: Option<&[u8]>) -> Audit {
        Audit {
            slot: slot.map(classify),
            legacy: legacy.map(classify),
            chained: chained.map(classify),
        }
    }

    /// Whether our guard sits somewhere git will actually reach it: the slot
    /// itself, or `.legacy` *while a framework shim owns the slot and execs it*.
    /// Our script parked at `.legacy` with no shim above it runs never.
    pub fn guard_runs(&self) -> bool {
        let ours = |k: Option<HookKind>| matches!(k, Some(HookKind::Current | HookKind::StaleOurs));
        ours(self.slot) || (self.slot == Some(HookKind::Shim) && ours(self.legacy))
    }

    /// The worst thing wrong with the arrangement, if anything.
    pub fn fault(&self) -> Option<Fault> {
        if self.chained == Some(HookKind::Shim) {
            return Some(Fault::ShimChained);
        }
        (!self.guard_runs()).then_some(Fault::NotInstalled)
    }
}

/// [`Audit`] the hooks in `hooks_dir`. Errors only on genuine I/O failure.
pub fn audit(hooks_dir: &Path) -> std::io::Result<Audit> {
    Ok(Audit::of(
        read_hook(&hooks_dir.join(HOOK_NAME))?.as_deref(),
        read_hook(&hooks_dir.join(LEGACY_NAME))?.as_deref(),
        read_hook(&hooks_dir.join(CHAINED_NAME))?.as_deref(),
    ))
}
...
"#;
    ...
    #[test]
    fn plan_drops_an_orphan_shim_beside_a_framework_slot() {
        // Regression: the shape a stray `prek install` leaves behind. The
        // framework re-took the slot before our installer next ran, orphaning
        // the shim it had displaced into `.thegn-orig` — and because the slot
        // is a shim, `restore_shim` can never fire again, so the orphan was
        // permanent. The allow path execs it and every merge dies on
        // "migration mode".
        let p = plan(
            Some(b(PREK_SHIM)),
            Some(b(HOOK_SCRIPT)),
            Some(b(PRE_COMMIT_SHIM)),
        );
        assert!(p.drop_chained, "the orphaned shim must be removed");
        assert!(!p.restore_shim, "the slot already has its shim");
        assert_eq!(p.placement, Placement::Legacy);
        assert_eq!(p.action, InstallAction::AlreadyCurrent);
    }

    #[test]
    fn plan_drops_an_orphan_shim_with_no_hook_in_the_slot() {
        let p = plan(None, None, Some(b(PREK_SHIM)));
        assert!(p.drop_chained);
        assert_eq!(p.placement, Placement::Hook);
        assert_eq!(p.action, InstallAction::Wrote);
    }

    #[test]
    fn plan_keeps_a_user_hook_at_chained() {
        // Only a *shim* is disposable there; a real hook is the user's work.
        let p = plan(Some(b(PREK_SHIM)), Some(b(HOOK_SCRIPT)), Some(b(USER_HOOK)));
        assert!(!p.drop_chained);
    }

    #[test]
    fn plan_does_not_drop_a_chained_path_it_is_about_to_write() {
        // `Chained` copies the displaced hook over `.thegn-orig`; deleting it
        // first would be pointless, and deleting it *after* would lose the hook.
        let p = plan(Some(b(PREK_SHIM)), Some(b(USER_HOOK)), Some(b(PREK_SHIM)));
        assert_eq!(p.action, InstallAction::Chained);
        assert!(!p.drop_chained, "the copy overwrites it anyway");
    }

    #[test]
    fn plan_does_not_drop_the_shim_it_is_restoring() {
        let p = plan(Some(b(HOOK_SCRIPT)), None, Some(b(PREK_SHIM)));
        assert!(p.restore_shim);
        assert!(!p.drop_chained, "the rename consumes it");
    }

    // ── audit ───────────────────────────────────────────────────────────────

    #[test]
    fn audit_flags_the_orphan_shim_as_the_worst_fault() {
        let a = Audit::of(Some(b(PREK_SHIM)), Some(b(HOOK_SCRIPT)), Some(b(PREK_SHIM)));
        assert!(a.guard_runs(), "the guard itself is installed correctly");
        assert_eq!(a.fault(), Some(Fault::ShimChained));
    }

    #[test]
    fn audit_is_clean_for_the_healthy_framework_arrangement() {
        let a = Audit::of(Some(b(PREK_SHIM)), Some(b(HOOK_SCRIPT)), None);
        assert!(a.guard_runs());
        assert_eq!(a.fault(), None);
    }

    #[test]
    fn audit_is_clean_when_we_own_the_slot() {
        let a = Audit::of(Some(b(HOOK_SCRIPT)), None, Some(b(USER_HOOK)));
        assert!(a.guard_runs());
        assert_eq!(a.fault(), None);
    }

    #[test]
    fn audit_reports_a_guard_at_legacy_with_no_shim_above_it_as_uninstalled() {
        // `.legacy` is only reached because a framework shim execs it. Under a
        // user's own hook — or nothing — our script there never runs.
        let a = Audit::of(Some(b(USER_HOOK)), Some(b(HOOK_SCRIPT)), None);
        assert!(!a.guard_runs());
        assert_eq!(a.fault(), Some(Fault::NotInstalled));
    }

    #[test]
    fn audit_reports_an_untouched_checkout_as_uninstalled() {
        let a = Audit::of(None, None, None);
        assert!(!a.guard_runs());
        assert_eq!(a.fault(), Some(Fault::NotInstalled));
        assert!(!Fault::NotInstalled.remedy().is_empty());
        assert!(!Fault::ShimChained.remedy().is_empty());
    }
