//! The keybinding registry — the single source of truth for thegn's keys.
//!
//! Every action is declared once in [`BUILTINS`] with its default chord(s), how
//! it's invoked (a `thegn` subcommand, a plugin pipe, or a native zellij
//! action), its menu label, and its short hint. From that one table we:
//!   * generate the `keybinds {}` block spliced into `~/.thegn/zellij.kdl`
//!     (a marked, regenerated region — user edits elsewhere in the file survive);
//!   * drive the Cmd+K menu (labels + chords) and the `thegn keys` surface;
//!   * feed the statusbar's hints (`thegn keys hints`).
//!
...
//! default; the strict check is `thegn keys validate`.
...
pub const BEGIN: &str = "// >>> thegn:keybinds (generated — edits here are overwritten) <<<";
pub const END: &str = "// >>> end thegn:keybinds <<<";
...
impl Plugin {
    ...
    fn url(self) -> String {
        format!("file:~/.local/share/thegn/{}", self.wasm())
    }
}
...
#[derive(Debug, Clone)]
pub enum Invocation {
    /// `Run "thegn" <args…>` in a (optionally floating) command pane.
    Run {
...
pub const BUILTINS: &[Action] = &[
    ...
    Action {
        ...
        // statusbar tracks the palette's id and spawns `thegn menu` itself.
        invocation: Invocation::Pipe {
            plugin: Plugin::Statusbar,
            name: "thegn_toggle_palette",
        },
        scope: Scope::Shared,
        contexts: &[Context::Global],
...
/// owns input, and Ctrl+g/Ctrl+hjkl are deliberate thegn bindings with the
...
/// so the nested block must span lines. `head` is e.g. `Run "thegn" "x"`;
...
fn render_bind(chord: &str, inv: &Invocation) -> String {
    match inv {
        ...
        Invocation::Run {
            ...
            if opts.is_empty() {
                format!("        bind \"{chord}\" {{ Run \"thegn\" {argv}; }}\n")
            } else {
                render_block_bind(chord, &format!("Run \"thegn\" {argv}"), &opts)
            }
        }
        Invocation::Shell {
            run,
...
pub fn render_keybinds_kdl(actions: &[Resolved]) -> String {
    ...
    let (nt_url, nt_name) = actions
        ...
        .unwrap_or_else(|| {
            (
                "file:~/.local/share/thegn/tabbar.wasm".to_string(),
                "thegn_new_tab".to_string(),
            )
        });
...
mod tests {
    ...
    fn render_contains_known_bindings() {
        ...
        assert!(kdl.contains("        bind \"Ctrl Alt s\" {\n            MessagePlugin \"file:~/.local/share/thegn/statusbar.wasm\" {\n                name \"thegn_toggle_sidebar\"\n            }\n        }\n"));
    ...
    fn splice_preserves_surroundings_and_is_idempotent() {
        ...
        assert!(once.contains("theme \"thegn\""), "theme preserved");
    ...
    fn splice_prepends_when_no_keybinds_block() {
        ...
        assert!(out.contains("theme \"thegn\""));
    }
    ...
    fn plugin_wasm_and_url_for_all_variants() {
        for (p, wasm) in [
            ...
            assert_eq!(p.url(), format!("file:~/.local/share/thegn/{wasm}"));
        }
    }
    ...
    fn render_bind_bare_run_and_native_and_tab_scope() {
        ...
        assert!(
            kdl.contains("        bind \"Alt b\" { Run \"thegn\" \"dashboard\"; }\n"),
            "{kdl}"
        );
        // Native single-line.
    ...
    fn render_falls_back_to_default_new_tab_when_absent() {
        ...
        let kdl = render_keybinds_kdl(&resolved);
        assert!(kdl.contains("thegn_new_tab"), "{kdl}");
