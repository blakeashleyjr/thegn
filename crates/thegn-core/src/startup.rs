...
mod tests {
    ...
    fn run_checks_uses_home_env_and_repairs() {
        // Kept from this branch; the HOME_LOCK that used to sit here is NOT —
        // main removed it deliberately (EnvGuard supersedes it, and two mutexes
        // over one resource is what testenv's doc warns against). This guard is a
        // separate concern: symlink repair is unavailable on Windows without
        // Developer Mode, so the test would assert a repair that cannot happen.
        if !can_symlink() {
            return;
        }
