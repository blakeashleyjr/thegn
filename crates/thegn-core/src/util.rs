...
/// thegn's own home — config, worktrees, cache, activity all live under here
/// (`~/.thegn`). `THEGN_DIR` relocates it so a dev/test instance can run on
/// a fully separate root (its own cache, config and worktrees) without touching
/// your daily-driver thegn. Pair it with `XDG_STATE_HOME` to also isolate the
/// DB (see `just start-term`).
pub fn thegn_dir() -> PathBuf {
    std::env::var_os("THEGN_DIR")
        .map(PathBuf::from)
...
pub(crate) fn immutable_sqlite_uri(path: &std::path::Path) -> String {
    #[cfg(unix)]
    let bytes = {
...
/// hooks) inherits a poisoned GIT_DIR/GIT_WORK_TREE. thegn always targets
...
/// Exact env-var names carried from thegn's own process into a freshly
...
/// thegn inherited" leak that both env-bundles (AU) and process-profiles (H)
...
/// - `THEGN_*` — our own non-secret context markers (profile, sandbox flag).
///
/// None of these families carry credentials; secrets ship under distinct names
/// (`*_TOKEN`/`*_KEY`/…) that no family matches.
pub const HOST_ENV_ALLOW_PREFIX: &[&str] = &["LC_", "XDG_", "DBUS_", "NIX_", "THEGN_"];
...
/// (setsid / a backgrounding subshell) could push thegn into the terminal's
/// background process group — after which thegn's next tty write returns EIO
...
/// When thegn (or its test suite, via a pre-commit hook) runs inside a git
...
pub fn git_cmd(dir: &Path) -> Command {
    ...
    // Read-side housekeeping must never touch `.git/index.lock`. thegn hydrates
...
///     write it there, after which every read (thegn's diff panel included)
...
mod tests {
    ...
    fn host_env_filter_keeps_infra_drops_secrets() {
        let input = vec![
            ...
            ("THEGN_PROFILE".to_string(), "work".to_string()),
