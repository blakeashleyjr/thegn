...
impl McpRouter {
    ...
    fn handle_tools_list(&self) -> Result<serde_json::Value, (i32, String)> {
        ...
        let mut tools = if self.git.is_some() {
            vec![
                ...
                json!({ "name": "semantic_diff", "description": "Entity-level (function/struct/...) summary of the diff vs HEAD plus a suggested commit message.",
                    "inputSchema": { "type": "object", "properties": {} } }),
                json!({ "name": "blast_radius", "description": "Inter-entity blast-radius of the diff vs HEAD: for each changed function/method, who depends on it (callers/files), which are untested, and an overall risk band. Reads thegn's persisted semantic graph.",
                    "inputSchema": { "type": "object", "properties": {} } }),
            ]
        } else {
            Vec::new()
        ...
        if self.merge.is_some() {
            tools.extend([
                json!({ "name": "merge_add", "description": "Add this worktree's current branch to thegn's local merge queue.",
                    "inputSchema": { "type": "object", "properties": {} } }),
                json!({ "name": "merge_clear", "description": "Clear thegn's merge queue for this repo.",
                    "inputSchema": { "type": "object", "properties": {} } }),
                json!({ "name": "merge_list", "description": "Show thegn's merge queue for this repo.",
                    "inputSchema": { "type": "object", "properties": {} } }),
            ]);
        }
        let base = json!([
