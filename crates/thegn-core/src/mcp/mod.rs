...
/// agent as house tools. The router lives in `thegn-core` and can't depend on
/// `thegn-svc` where `GitBackend` lives, so this trait inverts the dependency:
/// `thegn-svc` implements it over the real `GitBackend` + `semantic`, and the
...
pub trait HouseGit: Send + Sync {
    ...
    /// message — thegn's structural-diff intelligence.
...
/// Like [`HouseGit`], implemented in `thegn-svc` (over `gh`/`glab` + the git
...
/// (`thegn-host::mcp_merge`) and injects it via [`router::McpRouter::with_merge`].
