...
pub(crate) fn additive_schema(conn: &Connection) {
    ...
    // v62: bounded CI log tails plus an intent marker for deduplicated
    // autofix handoffs. Both are cache-side and additive.
    let _ = conn.execute_batch(
...
pub(crate) fn verify_v62_schema(conn: &Connection) -> Result<()> {
    ...
    for table in ["ci_log_cache", "ci_autofix_dedupe"] {
        ...
        if kind.as_deref() != Some("table") {
            anyhow::bail!("schema v62 migration did not create {table}");
        }
    }
    ...
    let index: Option<i64> = conn
        ...
        .optional()?;
    if index.is_none() {
        anyhow::bail!("schema v62 migration did not create the CI log index");
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn pre_v62_db_gains_ci_log_tables_without_resetting_run_cache() {
