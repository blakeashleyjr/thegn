...
pub const CATALOG: &[Slot] = &[
    ...
    // Hidden rotation custody proofs are generated by the host and deliberately
    // have no discoverable value source. Classify them so adding the safety
    // assertions does not silently turn them into filename completions.
    slot(
        "sprite-proxy",
        "expected_account",
        SourceKind::Reserved(Reserved::Freeform),
