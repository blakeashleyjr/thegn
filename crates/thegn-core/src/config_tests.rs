...
fn disk_config_defaults_and_env_override() {
    ...
    let mut env = MapEnv::default();
    env.0.insert("THEGN_DISK_SCCACHE".into(), "true".into());
    env.0
        .insert("THEGN_DISK_WARN_THRESHOLD_GB".into(), "250".into());
    env.0
        .insert("THEGN_DISK_SHARED_TARGET_DIR".into(), "/tmp/shared".into());
...
fn theme_keys_via_get_set_and_env() {
    ...
    let env = map_env(&[
        ("THEGN_THEME_FOCUS_BORDER", "#010203"),
        ("THEGN_THEME_BORDER", "#040506"),
    ]);
...
fn precedence_default_file_env_flag() {
    ...
    // env overrides file
    let env = map_env(&[("THEGN_BRANCH_PREFIX", "env/"), ("THEGN_PICKER", "fzf")]);
...
fn deprecated_tg_pr_ttl_still_read() {
    ...
    let env = map_env(&[("TG_PR_TTL", "7"), ("THEGN_PR_TTL", "9")]);
...
fn metrics_env_overlay_clamps_runtime_bounds() {
    let env = map_env(&[
        ("THEGN_METRICS_INTERVAL_SECS", "0.2"),
        ("THEGN_METRICS_TIMEOUT_MS", "10"),
        ("THEGN_METRICS_MAX_BODY_BYTES", "0"),
    ]);
...
fn env_overlay_covers_every_knob() {
    let env = map_env(&[
        ...
        ("THEGN_REPO_SCAN_DEPTH", "9"),
        ("THEGN_PROFILE", "vim"),
        ("THEGN_THEME_ACCENT", "#abcdef"),
        ("THEGN_PR_TTL", "11"),
        ("THEGN_WATCH_PR_INTERVAL", "13"),
        ("THEGN_METRICS_INTERVAL_SECS", "3.5"),
        ("THEGN_METRICS_TIMEOUT_MS", "750"),
        ("THEGN_METRICS_MAX_BODY_BYTES", "2048"),
        ("THEGN_LOG_LEVEL", "debug"),
        ("THEGN_LOG_FILE", "true"),
        ("THEGN_LOG_DIR", "/logs"),
...
fn env_bad_values_warn_and_skip() {
    ...
    let env = map_env(&[
        ("THEGN_PR_TTL", "lots"),
        ("THEGN_AUTO_REMOVE_WORKTREE", "maybe"),
        ("THEGN_PICKER", "telescope"),
        ("THEGN_REPO_SCAN_DEPTH", "deep"),
    ]);
    let o = env_overlay(&env);
...
fn accent_and_log_dir_helpers() {
    ...
    assert!(c.log.dir_path().ends_with("thegn/logs"));
    c.log.dir = "~/x".into();
...
fn path_is_under_xdg_config() {
    assert!(Config::path().ends_with("thegn/config.toml"));
}
...
fn env_selection_precedence_repo_over_global_default() {
    // Global default_env picks "g"; a repo .thegn.toml `env = "r"` wins;
...
",
    ...
    std::fs::write(dir.join(".thegn.toml"), "env = \"r\"\n").unwrap();
    let loc = GitLoc::Local(dir.clone());
...
fn process_env_reads_real_vars() {
    ...
    unsafe { std::env::set_var("THEGN_TEST_PENV_xyz", "v1") };
    assert_eq!(ProcessEnv.get("THEGN_TEST_PENV_xyz").as_deref(), Some("v1"));
    assert!(ProcessEnv.get("THEGN_TEST_PENV_absent_qqq").is_none());
    unsafe { std::env::remove_var("THEGN_TEST_PENV_xyz") };
    // blank values are treated as unset.
    unsafe { std::env::set_var("THEGN_TEST_PENV_blank", "   ") };
    assert!(ProcessEnv.get("THEGN_TEST_PENV_blank").is_none());
    unsafe { std::env::remove_var("THEGN_TEST_PENV_blank") };
}
...
fn expand_env_ref_resolves_env_prefix() {
    unsafe { std::env::set_var("THEGN_TEST_EXPAND_TOKEN", "secret") };
    assert_eq!(
        expand_env_ref("env:THEGN_TEST_EXPAND_TOKEN"),
        Some("secret".into())
    );
    unsafe { std::env::remove_var("THEGN_TEST_EXPAND_TOKEN") };
    // Missing var returns None.
    assert_eq!(expand_env_ref("env:THEGN_TEST_EXPAND_TOKEN"), None);
}
...
fn profile_overlay_path_none_for_default_some_for_named() {
    ...
    impl EnvSource for FakeEnv {
        fn get(&self, k: &str) -> Option<String> {
            (k == "THEGN_PROFILE").then(|| self.0.clone()).flatten()
        }
    }
    ...
    assert!(p.ends_with("thegn/profiles/work/config.toml"));
}

#[test]
fn surface_self_log_errors_defaults_off_and_overlay_applies() {
    // Default: thegn's own log errors are not surfaced as user notifications.
