...
fn media_config_defaults_and_enums() {
    ...
    use thegn_media::model::MediaKind;
    assert_eq!(
...
fn theme_color_glyph_env_overrides_apply() {
    ...
    env.0
        .insert("THEGN_THEME_COLOR".to_string(), "16".to_string());
    env.0
        .insert("THEGN_THEME_GLYPHS".to_string(), "ascii".to_string());
    env.0
        .insert("THEGN_THEME_AGENT_GLYPHS".to_string(), "symbol".to_string());
...
fn env_overlay_metrics_rejects_non_finite_floats() {
    let env = map_env(&[("THEGN_METRICS_INTERVAL_SECS", "inf")]);
    assert_eq!(env_overlay(&env).metrics_interval_secs, None);
    let env = map_env(&[("THEGN_METRICS_INTERVAL_SECS", "abc")]);
    assert_eq!(env_overlay(&env).metrics_interval_secs, None);
}
...
fn env_overlay_bad_enum_values_yield_none() {
    let env = map_env(&[
        ...
        ("THEGN_LOG_FORMAT", "bogus"),
        ("THEGN_SANDBOX_BACKEND", "bogus"),
        ("THEGN_SANDBOX_NETWORK", "bogus"),
...
fn env_overlay_log_and_metrics_bad_numbers_skip() {
    let env = map_env(&[
        ("THEGN_LOG_ROTATION_SIZE_MB", "huge"),
        ("THEGN_LOG_MAX_FILES", "lots"),
        ("THEGN_METRICS_TIMEOUT_MS", "soon"),
        ("THEGN_METRICS_MAX_BODY_BYTES", "big"),
        ("THEGN_WATCH_PR_INTERVAL", "later"),
    ]);
    let o = env_overlay(&env);
...
fn load_layered_recovers_on_parse_error_and_still_applies_layers() {
    ...
    let env = map_env(&[("THEGN_BRANCH_PREFIX", "env/")]);
...
fn passthrough_env_remote_drops_host_local_socket_vars() {
    ...
    unsafe {
        std::env::remove_var("TG_TEST_TOK_42");
...
fn passthrough_env_remote_injects_devshell_selector() {
    // Unset → no THEGN_DEVSHELL (host default shell unchanged).
    let plain = SandboxConfig::default();
    assert!(
        !plain
            .passthrough_env_remote()
            .iter()
            .any(|(k, _)| k == "THEGN_DEVSHELL"),
    ...
    assert!(
        sb.passthrough_env_remote()
            .iter()
            .any(|(k, v)| k == "THEGN_DEVSHELL" && v == "sandbox"),
        "devshell attr exported as THEGN_DEVSHELL"
    );
}
...
fn repo_overlay_malformed_file_is_ignored() {
    ...
    std::fs::write(dir.join(".thegn.toml"), "[sandbox\nbroken = =\n").unwrap();
    let cfg = Config::default();
...
fn repo_overlay_parse_error_surfaces_dropped_env_selection() {
    ...
    std::fs::write(dir.join(".thegn.toml"), "env = \"sprites\"\n").unwrap();
    assert!(
