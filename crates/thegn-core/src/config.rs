...
/// `[llm_proxy]` — the AI-traffic chokepoint daemon (`tgproxy`). The shell never
/// hard-depends on this; AI is strictly additive, so the default is disabled.
/// When `enabled`, the host launches `tgproxy` as a pinned daemon and agents
...
pub struct LlmProxyConfig {
    ...
    /// Path to the proxy's routes document (JSON), passed to `tgproxy` as
    /// `TGPROXY_CONFIG`. Empty means no backends are configured yet.
    ...
    /// from `enabled` (which launches `tgproxy`): set this to point the agent at
    /// an already-running proxy without launching our own. This governs the
    /// `THEGN_PROXY_*` vars the pi extension reads — NOT `ANTHROPIC_BASE_URL`
    /// (see `route_claude`).
    ...
    /// `THEGN_PROXY_*`; this switch is specifically for the `ANTHROPIC_*` vars.
    ...
    /// through thegn over a bind-mounted unix-socket ACP channel, and gate
    ...
    /// that can't reach host loopback) uses to reach `tgproxy` — a tunnel/public
    /// endpoint, e.g. `https://proxy.example.ts.net`. When set (and `route_agent`),
    /// thegn injects `ANTHROPIC_BASE_URL` + the per-worktree virtual key into
...
impl LlmProxyConfig {
    ...
    /// model traffic routes through `tgproxy` by default. Empty unless `route_agent`
    /// is on; the loopback URL is then reachable via the reverse tunnel thegn
    /// stands up (empty/`auto` `remote_base_url`) or via an explicit external URL.
    /// Sets the `THEGN_PROXY_*` vars the pi extension reads. `virtual_key`, when
    ...
    pub fn remote_agent_env(&self, virtual_key: Option<&str>) -> Vec<(String, String)> {
        ...
        let mut v = vec![
            ("THEGN_PROXY_BASE_URL".to_string(), url.clone()),
            ("THEGN_PROXY_API".to_string(), self.agent_api.clone()),
            ("THEGN_PROXY_MODEL".to_string(), self.agent_model.clone()),
        ];
        if self.route_claude {
            v.push(("ANTHROPIC_BASE_URL".to_string(), url));
        }
        if let Some(k) = virtual_key.map(str::trim).filter(|k| !k.is_empty()) {
            v.push(("THEGN_PROXY_KEY".to_string(), k.to_string()));
            if self.route_claude {
    ...
    /// sandboxes. Sets the `THEGN_PROXY_*` vars the pi extension reads, and —
    ...
    /// at `http://127.0.0.1:<proxy-port>` (thegn stands the tunnel up); an
    ...
    /// The launch spec for the `tgproxy` daemon — `(program, args, env)` — or
    /// `None` when the proxy is disabled. The host feeds this to its process
    /// supervisor (e.g. as a `restart = "always"` pinned daemon). `TGPROXY_LISTEN`
    /// and `TGPROXY_CONFIG` mirror the standalone env knobs the daemon reads.
    ///
    ...
    /// `listen` (e.g. a separate model-proxy) without thegn trying to bind the
    /// same port and colliding. Run thegn's own tgproxy with `enabled = true`.
    pub fn launch_spec(&self) -> Option<(String, Vec<String>, BTreeMap<String, String>)> {
        if !self.enabled {
            return None;
        }
        let mut env = BTreeMap::new();
        env.insert("TGPROXY_LISTEN".to_string(), self.listen.clone());
        if !self.config_path.is_empty() {
            env.insert("TGPROXY_CONFIG".to_string(), self.config_path.clone());
        }
        env.insert(
            "TGPROXY_FIRST_BYTE_TIMEOUT".to_string(),
            self.first_byte_timeout_secs.to_string(),
        );
        env.insert(
            "TGPROXY_STREAM_IDLE_TIMEOUT".to_string(),
            self.idle_timeout_secs.to_string(),
        );
        env.insert(
            "TGPROXY_STREAM_HEARTBEAT_INTERVAL".to_string(),
            self.heartbeat_secs.to_string(),
        );
        env.insert(
            "TGPROXY_COMPRESS".to_string(),
            if self.token_reduction { "1" } else { "0" }.to_string(),
        );
        env.insert(
            "TGPROXY_COMPRESS_LEVEL".to_string(),
            self.token_reduction_level.as_str().to_string(),
        );
        env.insert(
            "TGPROXY_ROUTING".to_string(),
            self.routing.as_str().to_string(),
        );
        Some(("tgproxy".to_string(), Vec::new(), env))
    }
}
