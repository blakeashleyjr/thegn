...
pub const DEFAULT_BASE_IMAGE: &str = "ghcr.io/thegn/thegn-sandbox:v1";
...
pub struct ImageRef {
    pub name: String,
    /// Empty ⇒ `latest` semantics are left to the runtime; thegn always
...
mod tests {
    ...
    fn managed_tag_is_digest_derived() {
        let d = Digest::parse(D1).unwrap();
        assert_eq!(managed_tag(&d), "localhost/thegn/base:111111111111");
    }
