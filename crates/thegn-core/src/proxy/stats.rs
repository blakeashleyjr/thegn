...
//!
//! The daemon's `/stats` endpoint, the `thegn proxy stats` CLI, and the usage
//! panel's proxy-spend block all render this one rollup — the DB supplies raw
//! [`ModelProxyRequestRow`]s and this module does the math, so the numbers agree
//! everywhere and the logic stays under the core coverage gate.
//!
...
use crate::store::ModelProxyRequestRow;

/// Per-request generation throughput, when the row was measured.
pub fn tokens_per_sec(row: &ModelProxyRequestRow) -> Option<f64> {
...
impl Acc {
    fn push(&mut self, r: &ModelProxyRequestRow) {
        self.requests += 1;
        ...
        self.cache_read_tokens += r.cache_read_tokens.max(0) as u64;
        self.cache_creation_tokens += r.cache_creation_tokens.max(0) as u64;
...
fn scope_of(r: &ModelProxyRequestRow) -> String {
...
pub fn rollup(rows: &[ModelProxyRequestRow]) -> Rollup {
...
mod tests {
    ...
    fn rollup_totals_and_outcomes() {
        let mut r1 = row("a", "ok", 100, 1000, None);
        r1.cache_read_tokens = 20;
        r1.cache_creation_tokens = 5;
        let rows = vec![
            r1,
        ...
        assert_eq!(r.totals.cache_read_tokens, 20);
        assert_eq!(r.totals.cache_creation_tokens, 5);
    ...
    fn last_tokens_per_sec_tracks_newest_measured_row() {
        ...
        new.ts_ms = 2;
        let unmeasured = ModelProxyRequestRow {
            ts_ms: 3,
            outcome: "ok".into(),
            ..Default::default()
        };
