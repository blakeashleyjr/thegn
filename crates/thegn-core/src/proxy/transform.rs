...
//! `estimatedRequestTokens` / `exceedsContextLimit` / `ensureStreamUsage`. The
//! proxy parses the body once into a [`serde_json::Value`] and applies these
//! transforms in place, re-serializing at dispatch.
//!
//! The pre-alpha in-flight tool-output compression (group W) is deliberately
//! **not** restored — token reduction is out of scope for the resurrection; the
//! `compress.rs` module and `CompressPolicy` stay dead.
...
/// Converts a JSON value to an `i64` only for positive integers (matching
