...
//! `.thegn.*` overlay *safe* to honour.
//!
//! thegn's config layers cascade most-specific-wins for *preferences*
...
pub enum TrustLevel {
    ...
    Builtin,
    /// The user's `~/.config/thegn/config.toml`.
    UserGlobal,
    ...
    Workspace,
    /// A repo-root `.thegn.*` overlay — repo-authored, **least trusted**.
    Repo,
    /// `THEGN_*` env + `--set` CLI flags — user-typed at launch (trusted,
...
pub fn resolve_repo_sandbox(
    ...
    // trusted than the repo), so this is an ungated accumulate — which is
    // exactly why it goes through the one ambiguity-refusing selector
    // (THE-515): a refused overlay adds no mounts.
    if let Some(ws) = cfg.workspace_overlay(repo_root).overlay() {
