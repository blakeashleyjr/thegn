...
/// Why a `.claude/settings.local.json` cannot be merged — the file is the
/// harness's own, holding the user's permissions, so thegn **never overwrites
/// a document it does not understand**. The host prefixes the file path.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PermsError {
    /// The existing text is not valid JSON.
    Malformed(String),
    /// A required nesting level is not the shape it must be. Carries which
    /// level (`root document`, `permissions`, `permissions.allow`).
    NotAnObject(&'static str),
}

impl std::fmt::Display for PermsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PermsError::Malformed(e) => write!(
                f,
                "not valid JSON ({e}) — thegn will not overwrite a file it cannot parse"
            ),
            PermsError::NotAnObject(which) => write!(
                f,
                "{which} is not the expected JSON shape — thegn will not overwrite a \
...
/// Merge `allow` patterns into a `.claude/settings.local.json` document,
/// preserving every other key. Returns the file's new full text (pretty JSON,
/// trailing newline).
///
/// - `None` / blank / whitespace-only `existing` starts from `{}`.
/// - Existing entries keep their order; new ones append; duplicates drop
///   (the merge is idempotent — running it twice changes nothing).
/// - Key order in the output follows `serde_json::Value`'s default `Map`
///   behaviour and is not preserved from the input; every *value* survives
///   byte-for-byte.
pub fn merge_claude_allow(existing: Option<&str>, allow: &[String]) -> Result<String, PermsError> {
    let mut root: serde_json::Map<String, serde_json::Value> = match existing {
        Some(text) if !text.trim().is_empty() => {
            let v: serde_json::Value =
                serde_json::from_str(text).map_err(|e| PermsError::Malformed(e.to_string()))?;
            match v {
                serde_json::Value::Object(m) => m,
                _ => return Err(PermsError::NotAnObject("root document")),
            }
        }
    ...
    match root.get("permissions") {
        ...
        Some(_) => return Err(PermsError::NotAnObject("permissions")),
    }
    ...
    let Some(perms) = root.get_mut("permissions").and_then(|p| p.as_object_mut()) else {
        return Err(PermsError::NotAnObject("permissions"));
    };
    match perms.get("allow") {
        None => {}
        Some(serde_json::Value::Array(items)) if items.iter().all(serde_json::Value::is_string) => {
        }
        Some(_) => return Err(PermsError::NotAnObject("permissions.allow")),
    }
    if !perms.contains_key("allow") {
        perms.insert("allow".to_string(), serde_json::Value::Array(Vec::new()));
    }
    let Some(list) = perms.get_mut("allow").and_then(|a| a.as_array_mut()) else {
        return Err(PermsError::NotAnObject("permissions.allow"));
    };
    for pattern in allow {
        let v = serde_json::Value::String(pattern.clone());
        if !list.contains(&v) {
            list.push(v);
        }
    }
    let mut out = serde_json::to_string_pretty(&serde_json::Value::Object(root))
        .map_err(|e| PermsError::Malformed(e.to_string()))?;
    out.push('\n');
    Ok(out)
}
...
///
/// Deliberately **narrow**: the daemon boots with a config snapshot that may
/// carry `--set`/`--config` overrides layered by the CLI, and a wholesale
/// re-load would silently discard them. The only thing that goes stale in
/// practice is a stage's `agent` name (an `[[agents]]` entry renamed after the
/// daemon started) — so refresh exactly the registries agent resolution reads
/// (`agents`, `tools`, `pipeline`) and nothing else.
...
mod tests {
    ...
    // --- claude permission seeding ---------------------------------------------

    fn allow(pats: &[&str]) -> Vec<String> {
        pats.iter().map(|s| s.to_string()).collect()
    }

    #[test]
    fn an_absent_file_becomes_a_minimal_allow_list() {
        let out = merge_claude_allow(None, &allow(&["Read", "Edit"])).unwrap();
        assert_eq!(
            out,
            "{\n  \"permissions\": {\n    \"allow\": [\n      \"Read\",\n      \"Edit\"\n    ]\n  }\n}\n"
        );
        // Blank/whitespace-only text is the same as absent.
        assert_eq!(
            merge_claude_allow(Some("   \n\t"), &allow(&["Read"])).unwrap(),
            merge_claude_allow(None, &allow(&["Read"])).unwrap()
        );
    }
  ...
  "permissions": {
    ...
    "allow": ["Read"]
        ...
        let out = merge_claude_allow(Some(existing), &allow(&["Bash(git status:*)"])).unwrap();
        ...
        assert_eq!(
            v["permissions"]["allow"],
            serde_json::json!(["Read", "Bash(git status:*)"])
        );
    }

    #[test]
    fn an_existing_allow_list_is_unioned_not_replaced() {
        let existing = r#"{"permissions": {"allow": ["Read"]}}"#;
        let out =
            merge_claude_allow(Some(existing), &allow(&["Read", "Bash(git status:*)"])).unwrap();
        let v: serde_json::Value = serde_json::from_str(&out).unwrap();
        assert_eq!(
            v["permissions"]["allow"],
            serde_json::json!(["Read", "Bash(git status:*)"])
    ...
    fn the_merge_is_idempotent() {
        let existing = r#"{"model": "opus", "permissions": {"allow": ["Read"]}}"#;
        let once = merge_claude_allow(Some(existing), &allow(&["Edit", "Read"])).unwrap();
        let twice = merge_claude_allow(Some(&once), &allow(&["Edit", "Read"])).unwrap();
        assert_eq!(once, twice, "byte-identical on the second run");
    }

    #[test]
    fn a_malformed_or_wrongly_shaped_file_is_refused_not_overwritten() {
        // Malformed JSON — the message is serde_json's; only the variant is ours.
        assert!(matches!(
            merge_claude_allow(Some("{permissions: "), &allow(&["Read"])),
            Err(PermsError::Malformed(_))
        ));
        // Root not an object.
        assert_eq!(
            merge_claude_allow(Some("[1, 2]"), &allow(&["Read"])),
            Err(PermsError::NotAnObject("root document"))
        );
        assert_eq!(
            merge_claude_allow(Some("\"just a string\""), &allow(&["Read"])),
            Err(PermsError::NotAnObject("root document"))
        );
        // permissions present but not an object.
        assert_eq!(
            merge_claude_allow(Some("{\"permissions\": [\"Read\"]}"), &allow(&["Read"])),
            Err(PermsError::NotAnObject("permissions"))
        );
        // allow present but not an array of strings.
        assert_eq!(
            merge_claude_allow(
                Some("{\"permissions\": {\"allow\": \"Read\"}}"),
                &allow(&["Read"])
            ),
            Err(PermsError::NotAnObject("permissions.allow"))
        );
        assert_eq!(
            merge_claude_allow(
                Some("{\"permissions\": {\"allow\": [1]}}"),
                &allow(&["Read"])
            ),
            Err(PermsError::NotAnObject("permissions.allow"))
        );
    ...
    #[test]
    fn an_empty_allow_list_still_produces_a_valid_document() {
        let out = merge_claude_allow(None, &[]).unwrap();
        let v: serde_json::Value = serde_json::from_str(&out).unwrap();
    ...
    #[test]
    fn only_the_registries_are_taken_from_the_fresh_config() {
        let mut base = crate::config::Config::default();
        base.keymap_preset = "vscode".into();
        base.agents.push(crate::config::NamedCommand {
            name: "old".into(),
            command: "old --run".into(),
            hints: Vec::new(),
            provider: None,
            resume: false,
        ...
        base.pipeline
            .stages
            .push(crate::config_pipeline::PipelineStage {
                name: "stale-stage".into(),
                agent: "old".into(),
                ..Default::default()
            });

        let mut fresh = base.clone();
        fresh.keymap_preset = "emacs".into();
        fresh.agents.clear();
        fresh.agents.push(crate::config::NamedCommand {
            name: "new".into(),
        ...
        fresh.pipeline.stages.clear();
        fresh
            .pipeline
            .stages
            .push(crate::config_pipeline::PipelineStage {
                name: "fresh-stage".into(),
                agent: "new".into(),
                ..Default::default()
            });

        let merged = with_fresh_registry(&base, &fresh);
        // Registries come from fresh…
        assert_eq!(merged.agents[0].name, "new");
        assert_eq!(merged.pipeline.stages[0].name, "fresh-stage");
        // …everything else survives from base — a wholesale re-load would have
        // discarded the daemon's --set/--config overrides.
        assert_eq!(merged.keymap_preset, "vscode");
    }
}
