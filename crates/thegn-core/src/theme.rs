//! The thegn design system: token palette for the native host's
...
pub fn relative_luminance(rgb: &str) -> f32 {
    ...
    let mut it = rgb
        .split(';')
        .map(|n| n.trim().parse::<f32>().unwrap_or(0.0));
...
mod tests {
    ...
    fn presets_list_has_at_least_twenty_high_quality_themes() {
        assert!(
            PRESETS.len() >= 20,
            "Must have at least 20 themes, found {}",
            PRESETS.len()
        );
        for name in PRESETS {
    ...
    fn ghost_tier_stays_legible_on_standard_surfaces() {
        ...
        for (sname, surf) in surfaces {
            for (fname, fg) in floor {
                ...
                assert!(
                    r >= 3.0,
                    "prism `{fname}` on `{sname}` = {r:.2}:1 (want ≥ 3.0)"
                );
            }
        }
