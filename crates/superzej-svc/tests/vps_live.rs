...
//!
//!   HCLOUD_TOKEN=…        cargo test -p superzej-svc --test vps_live -- --ignored --nocapture live_hetzner
//!   DIGITALOCEAN_TOKEN=…  cargo test -p superzej-svc --test vps_live -- --ignored --nocapture live_digitalocean
//!
...
//! shows it gone → the ledger under $XDG_STATE/superzej/vps is clean.
...
fn ephemeral_key(dir: &Path) -> (std::path::PathBuf, String) {
    ...
    let out = Command::new("ssh-keygen")
        .args(["-t", "ed25519", "-N", "", "-C", "superzej-live-test", "-f"])
        .arg(&key)
        .output()
        .expect("ssh-keygen");
...
fn run_lifecycle(kind: VpsKind, token_env: &str) {
    ...
    // Isolate the registry so the test never touches a real superzej state dir.
    ...
    rt.block_on(async {
        ...
        let result = async {
            ...
            // list() is server-side scoped to superzej-managed instances.
