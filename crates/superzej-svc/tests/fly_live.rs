...
//! prorated IPv4). Run:
//!
//!   FLY_API_TOKEN=… cargo test -p superzej-svc --test fly_live -- --ignored --nocapture
//!
...
fn live_shell_and_docker() {
    ...
    rt.block_on(async {
        ...
        let result = async {
            ...
            let (code, out) = prov
                .run_exec(&name, &["echo".into(), "SZ_FLY_SHELL_OK".into()], None, &[])
                .await?;
            anyhow::ensure!(code == 0 && out.contains("SZ_FLY_SHELL_OK"), "shell echo (exit {code}): {out}");
            ...
            let dscript = "set -e; export DEBIAN_FRONTEND=noninteractive; \
                ...
                docker run --rm hello-world 2>&1 | grep -qi 'hello from docker' && echo SZ_DOCKER_OK || (echo DOCKER_FAIL; tail -8 /tmp/dockerd.log)";
            let (dc, dout) = prov
                .run_exec(&name, &["/bin/sh".into(), "-c".into(), dscript.into()], None, &[])
                .await?;
            anyhow::ensure!(dout.contains("SZ_DOCKER_OK"), "docker run (exit {dc}):\n{dout}");
