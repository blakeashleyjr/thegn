...
pub fn script_from_argv(argv: &[String]) -> String {
    match argv {
        ...
        _ => argv
            .iter()
            .map(|a| superzej_core::util::sh_quote(a))
            .collect::<Vec<_>>()
            .join(" "),
    }
}
...
impl SshShim {
    ...
    pub async fn read(&self, path: &str) -> Result<Vec<u8>> {
        let p = superzej_core::util::sh_quote(path);
    ...
    pub async fn write(&self, path: &str, data: &[u8], mode: &str) -> Result<()> {
        let p = superzej_core::util::sh_quote(path);
        let dir = superzej_core::util::sh_quote(
