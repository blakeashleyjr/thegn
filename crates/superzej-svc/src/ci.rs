...
//! trigger·rerun·cancel onto superzej-core's [`superzej_core::ci`] model, with
...
pub fn resolve_system(loc: &GitLoc, cfg: &CiConfig) -> Option<CiSystem> {
    match cfg.provider {
        ...
        CiProviderKind::Auto => {
            ...
            if !loc.is_remote()
                && let Some(cfg) =
                    superzej_core::ci::detect_ci_configs(std::path::Path::new(&loc.path())).first()
            {
