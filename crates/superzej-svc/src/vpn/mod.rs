...
//!
//! superzej attaches a worktree's sandbox to its OWN overlay/tunnel (Tailscale,
...
//! ordering) lives in `superzej_core::sandbox`, and the host sequences the two.
//!
...
use superzej_core::config::expand_env_ref;
use superzej_core::config::{VpnDnsMode, VpnMode, VpnProviderKind};
use superzej_core::sandbox::{VpnParams, VpnSpec};
...
/// What the provider produced — consumed by `superzej_core::sandbox` to splice
...
/// `superzej_core::sandbox::ensure`); the host runs them off the event loop on a
...
pub trait VpnProvider: Send + Sync {
    ...
    /// `rm -f`'d by `superzej_core::sandbox::teardown`). Best-effort.
...
fn plan_tailscale(
    spec: &VpnSpec,
    t: &superzej_core::config::TailscaleConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
...
fn plan_wireguard(
    w: &superzej_core::config::WireguardConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
...
fn plan_openvpn(
    o: &superzej_core::config::OpenvpnConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
...
fn plan_netbird(
    spec: &VpnSpec,
    n: &superzej_core::config::NetbirdConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
...
fn plan_zerotier(
    z: &superzej_core::config::ZerotierConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
...
fn plan_custom(
    spec: &VpnSpec,
    c: &superzej_core::config::CustomVpnConfig,
    container: &str,
    image: String,
) -> Result<SidecarPlan> {
