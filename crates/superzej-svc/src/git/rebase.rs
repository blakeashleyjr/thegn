...
pub trait RebaseOps: GitBackend {
    ...
    fn rebase_move(
        ...
        let todo = todo_for(loc, &base)?;
        let todo = superzej_core::rebase_todo::move_entry(&todo, sha, up)?;
        rebase_with_todo(loc, &base, &todo, opts)
    }
