...
mod tests {
    ...
    fn cfg(endpoint: &str) -> SnapshotStoreConfig {
        SnapshotStoreConfig {
            ...
            access_key: "env:SZ_TEST_S3_ACCESS".into(),
            secret_key: "env:SZ_TEST_S3_SECRET".into(),
            ..Default::default()
        }
    }
    ...
    fn resolver(s: &str) -> Option<String> {
        match s {
            "env:SZ_TEST_S3_ACCESS" => Some("AKIATEST".into()),
            "env:SZ_TEST_S3_SECRET" => Some("sekrit".into()),
            _ => None,
        }
    }
    ...
    fn custom_endpoint_uses_path_style_and_prefixed_keys() {
        ...
        assert_eq!(
            ...
            "superzej/repo/wt/hetzner/00000000000000000001-abc/bundle"
        );
        let url = s
            .bucket
        ...
        assert!(
            url.path()
                .starts_with("/sz-snaps/superzej/repo/wt/hetzner/id1/tar")
        );
        assert!(
    ...
    fn aws_endpoint_uses_virtual_host_style() {
        ...
        let url = s
            .bucket
            .get_object(Some(&s.creds), "superzej/x")
            .sign(SIGN_TTL);
        assert_eq!(url.host_str(), Some("sz-snaps.s3.auto.amazonaws.com"));
        assert!(url.path().starts_with("/superzej/x"));
    }
