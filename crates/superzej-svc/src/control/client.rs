//! The control-API client — what `szhost` CLI verbs and the compositor's
...
use superzej_core::control_wire::{EventDecoder, EventFrame};
    ...
    loop {
        tokio::select! {
            msg = ws.next() => match msg {
                Some(Ok(Message::Binary(bytes))) => {
                    ...
                    loop {
                        match decoder.next_frame() {
                            ...
                            Ok(None) => break,
                            Err(e) => {
                                tracing::warn!(target: "szhost::control", "attach stream decode error: {e}");
                                return;
                            }
                        }
                    }
                ...
                Some(Err(e)) => {
                    tracing::debug!(target: "szhost::control", "attach websocket error: {e}");
                    return;
                }
            },
            c = ctrl.recv() => match c {
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let mut req = hyper::Request::builder()
        .method(method)
        .uri(path)
        .header(hyper::header::HOST, "superzej-daemon");
    if let Some(t) = token {
