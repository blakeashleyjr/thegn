...
//!
//! Milestone 1 loads routes from a JSON document (file via `TGPROXY_CONFIG`, or
//! inline via `TGPROXY_ROUTES_JSON`) so the daemon is usable on its own for
//! validation. Stage E folds this into thegn's layered `[llm_proxy]` config
...
use thegn_core::config::RoutingStrategy;
use thegn_core::proxy::compress::{Level, Limits};
...
struct RouteDoc {
    ...
    /// `TGPROXY_ROUTING` (then `sequential`).
    #[serde(default)]
...
/// Reads `TGPROXY_LISTEN` (or `META_ROUTER_PORT`), the routes document, and the
...
/// `TGPROXY_COMPRESS_LEVEL` (when `read_env`) → routes-doc `level` → `Off`; an
/// explicit `TGPROXY_COMPRESS=0` forces it off. Bypass list, allow-list, custom
...
fn resolve_compression(doc: Option<&CompressionDoc>, read_env: bool) -> CompressPolicy {
    ...
    let mut level = doc
        ...
        .unwrap_or(Level::Off);
    if read_env {
        if let Ok(v) = std::env::var("TGPROXY_COMPRESS_LEVEL")
            .or_else(|_| std::env::var("MODEL_PROXY_COMPRESS_LEVEL"))
        {
            level = Level::parse(&v);
        }
        match std::env::var("TGPROXY_COMPRESS").ok().as_deref() {
            Some("0") | Some("false") | Some("off") | Some("no") => level = Level::Off,
            Some(_)
                if doc.and_then(|d| d.level.as_deref()).is_none()
                    && std::env::var("TGPROXY_COMPRESS_LEVEL").is_err() =>
            {
                // TGPROXY_COMPRESS=1 with no explicit level → default conservative.
...
/// `MODEL_PROXY_*` names (and `TGPROXY_*` aliases) with the Go defaults.
fn relay_from_env() -> RelayConfig {
    ...
    RelayConfig {
        ...
        heartbeat: secs(
            &[
                "TGPROXY_STREAM_HEARTBEAT_INTERVAL",
...
fn resolve_listen() -> Result<SocketAddr> {
    if let Ok(v) = std::env::var("TGPROXY_LISTEN") {
        return v
            .parse()
            .with_context(|| format!("parse TGPROXY_LISTEN={v}"));
    }
...
fn load_doc() -> Result<Option<ConfigDoc>> {
    if let Ok(path) = std::env::var("TGPROXY_CONFIG") {
        let raw = std::fs::read_to_string(&path).with_context(|| format!("read {path}"))?;
        return Ok(Some(parse_doc(&raw)?));
    }
    if let Ok(inline) = std::env::var("TGPROXY_ROUTES_JSON") {
...
/// The global default routing strategy from `TGPROXY_ROUTING` (then sequential).
fn global_routing_from_env() -> RoutingStrategy {
    std::env::var("TGPROXY_ROUTING")
        .ok()
        .and_then(|s| RoutingStrategy::from_str_validated(&s).ok())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn relay_tunables_from_env() {
        with_env(
            &[
                ("TGPROXY_FIRST_BYTE_TIMEOUT", Some("5")),
                ("TGPROXY_STREAM_IDLE_TIMEOUT", Some("6")),
                ("TGPROXY_STREAM_HEARTBEAT_INTERVAL", Some("7")),
            ],
            || {
        ...
        with_env(
            &[
                ("TGPROXY_FIRST_BYTE_TIMEOUT", None),
                ("TGPROXY_STREAM_IDLE_TIMEOUT", None),
                ("TGPROXY_STREAM_HEARTBEAT_INTERVAL", None),
    ...
    fn global_routing_from_env_parses_and_defaults() {
        with_env(&[("TGPROXY_ROUTING", Some("load_balanced"))], || {
            assert_eq!(global_routing_from_env(), RoutingStrategy::LoadBalanced);
        });
        with_env(&[("TGPROXY_ROUTING", Some("nonsense"))], || {
    ...
    fn resolve_listen_precedence() {
        with_env(&[("TGPROXY_LISTEN", Some("127.0.0.1:9999"))], || {
            assert_eq!(resolve_listen().unwrap(), "127.0.0.1:9999".parse().unwrap());
        });
        with_env(
            &[("TGPROXY_LISTEN", None), ("META_ROUTER_PORT", Some("8080"))],
            || {
                assert_eq!(resolve_listen().unwrap(), "127.0.0.1:8080".parse().unwrap());
            },
        );
        with_env(
            &[("TGPROXY_LISTEN", None), ("META_ROUTER_PORT", None)],
            || {
    ...
    fn compression_level_from_env() {
        ...
        with_env(
            &[
                ("TGPROXY_COMPRESS", Some("1")),
                ("TGPROXY_COMPRESS_LEVEL", None),
                ("MODEL_PROXY_COMPRESS_LEVEL", None),
            ],
            || {
                // TGPROXY_COMPRESS=1 with no explicit level → conservative.
                assert_eq!(
        ...
        with_env(
            &[
                ("TGPROXY_COMPRESS", Some("1")),
                ("TGPROXY_COMPRESS_LEVEL", Some("balanced")),
            ],
            || {
        ...
        with_env(
            &[
                ("TGPROXY_COMPRESS", Some("0")),
                ("TGPROXY_COMPRESS_LEVEL", Some("aggressive")),
            ],
            || {
                // Explicit off wins.
    ...
    fn from_env_reads_inline_routes_and_global_strategy() {
        with_env(
            &[
                ...
                ("TGPROXY_ROUTING", Some("load_balanced")),
                ("TGPROXY_LISTEN", None),
            ...
            || {
                ...
                // Global TGPROXY_ROUTING applies when the route omits its own.
                assert_eq!(cfg.routes[0].strategy, RoutingStrategy::LoadBalanced);
