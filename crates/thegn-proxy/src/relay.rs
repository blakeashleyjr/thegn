...
pub fn parse_sse_data_line(line: &[u8]) -> Option<OpenAiChunk> {
    ...
    if let Some(usage) = v.get("usage").filter(|u| !u.is_null()) {
        chunk.usage = Some(parse_openai_usage(usage));
    }
...
/// Parses an OpenAI `usage` object into [`Usage`], including cache-token counts.
/// Reads both the OpenAI-native shape (`prompt_tokens_details.cached_tokens`) and
/// the Anthropic-translated keys (`cache_read_tokens` / `cache_creation_tokens`)
/// the upstream seam threads through. Shared by the relay sinks and the router's
/// non-streaming usage parse so both surfaces account cache tokens identically.
pub fn parse_openai_usage(usage: &serde_json::Value) -> Usage {
    let u64_at = |v: &serde_json::Value, k: &str| v.get(k).and_then(|x| x.as_u64()).unwrap_or(0);
    let cache_read = {
        let translated = u64_at(usage, "cache_read_tokens");
        if translated > 0 {
            translated
        } else {
            usage
    ...
    Usage {
        ...
        cache_creation_tokens: u64_at(usage, "cache_creation_tokens"),
    }
}
