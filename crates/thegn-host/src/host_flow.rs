...
use thegn_core::host_config::{HostBinding, InstallConsent};
use thegn_core::host_machine::{
    HostEffect, HostEvent as MachineEvent, HostState, MachineCtx, Transition, resume, step,
};
use thegn_core::image::managed_tag;
use thegn_core::inventory::ArtifactKind;
use thegn_core::retry::{ReconnectPolicy, StepBudget, with_retry};
use thegn_core::store::{HostStore, WorkspaceStore};
use thegn_svc::host::{HostRunner, local_caps, runner_for};
...
pub(crate) fn apply_ready(worktree: &str, spec: &mut thegn_core::sandbox::SandboxSpec) {
    let map = READY_SPECS
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
pub(crate) fn failure_reason(f: &HostFailure) -> String {
    let retry = if f.retryable {
        " (retry from System ▸ Hosts or `thegn host provision`)"
    } else {
        ""
    };
        ...
        HostEffect::SeedVolume { spec } => {
            ...
            let digest = ctx
                ...
                .unwrap_or_else(|| {
                    ...
                    thegn_core::image::Digest::from_hex(&"0".repeat(64)).expect("static")
                });
            let res = with_retry(
                "volume seed",
                &StepBudget::new(
...
fn record_image(
    ...
    binding: &HostBinding,
    ctx: &MachineCtx,
    digest: &thegn_core::image::Digest,
    verified_now: bool,
) {
    ...
    let now = unix_now();
    let entry = thegn_core::inventory::InventoryEntry {
    ...
    loop {
        ...
        progress(&[
            ProvisionStepView {
                ...
                detail: Some("provisioning in another thegn process".into()),
            },
            ProvisionStepView {
                label: step_label,
                state: ProvisionState::Active,
...
mod tests {
    use super::*;
    use thegn_core::host::{Arch, HostId, RuntimeInfo, VolumeSpec};
    ...
    fn drive_provisions_then_second_call_is_a_db_only_no_op() {
        ...
        assert_eq!(
            spec.volumes,
            vec![("thegn-nix-store".into(), "/nix".into())]
        );
    ...
    fn failure_reason_names_the_remedy() {
        ...
        assert!(s.contains("thegn host provision"));
