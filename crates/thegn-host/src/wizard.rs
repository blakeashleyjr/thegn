...
//! idle thegn still produces zero events.
...
use thegn_core::{repo, util, worktree};
...
pub struct WizardChoices {
    ...
    /// [`thegn_core::config::Config::resolve_env`]). `"default"` = implicit.
    pub env: String,
    pub sandbox: String,
    pub agent: String,
}
...
impl NewWorktreeWizard {
            ...
            .map(|i| {
                let local = i.key == "default"
                    || cfg
                        .env
                        .get(&i.key)
                        .map(|e| matches!(e.placement, thegn_core::config::PlacementMode::Local))
                        .unwrap_or(true);
                (i.key, i.label, local)
            })
            .collect();
    ...
    /// default selection.
    pub fn apply_template(&mut self, tmpl: &thegn_core::config::WorktreeTemplate) {
...
pub struct WorkerCtx {
    ...
    /// state dir (this shell often runs inside a live thegn).
...
mod tests {
    ...
    fn typing_marks_edited_and_blocks_suggestion() {
        ...
        w.apply_name_suggestion("tg/other-name");
    ...
    fn pristine_field_adopts_suggestion() {
        let cfg = test_cfg();
        let mut w = NewWorktreeWizard::new(std::env::temp_dir(), &cfg);
        w.apply_name_suggestion("tg/calm-otter-3");
        assert_eq!(w.candidate(), "tg/calm-otter-3");
    }
    ...
    fn non_local_host_skips_sandbox_in_focus_ring() {
        ...
        cfg.env.insert(
            ...
            thegn_core::config::EnvConfig {
                placement: thegn_core::config::PlacementMode::Ssh,
                ssh: thegn_core::config::EnvSshConfig {
                    host: "build-box".into(),
                    ..Default::default()
                },
                ..Default::default()
    ...
    fn worker_persists_default_when_ambient_is_provider() {
        use thegn_core::config::{EnvConfig, PlacementMode};
