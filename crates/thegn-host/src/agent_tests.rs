...
fn auto_backend_fallthrough_carries_visible_warning() {
    with_temp_state("auto-fallthrough-warning", || {
        ...
        // whether the box HAS the backend: absent ⇒ "sandbox apple unavailable",
        // present-but-broken ⇒ "sandbox apple failed: …". On Linux/Windows the
        // `container` CLI never exists so only the first is ever seen; on a Mac
        // with it installed the probe finds it and the run is what fails. The
        // invariant is the same either way — the skipped backend is named and
        // the host fallback is visible — so assert that, not one spelling.
        assert!(
            warning.contains("sandbox apple"),
...
fn selected_env_with_no_table_halts_or_degrades_loudly() {
    ...
    with_temp_state("prep-phantom-env", || {
        ...
        //
        // Sandboxing off for this half, so the assertion is about the dropped
        // ENV selection and nothing else. The halt arm above returns before it
        // ever touches a backend (measured: 128ms); this arm does not, and with
        // sandboxing on it resolves a real one — which on a developer box with a
        // container runtime installed means real control-plane work. Measured
        // here at 120s, and past nextest's 5-minute cap on a second run, so the
        // suite's slowest test by two orders of magnitude and a timeout risk.
        // It went unnoticed because a machine with no runtime probes every
        // backend `Absent` in milliseconds.
        let mut cfg = Config::default();
