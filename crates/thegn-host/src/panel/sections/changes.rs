...
pub(crate) fn impact_summary_segs(
    ...
    if let Some(br) = entities.blast.as_ref()
        && cols >= 48
    {
        use thegn_core::semantic_graph::Risk;
...
fn impact_footer(data: &crate::panel::PanelData, ui: &PanelUi, cols: usize) -> Vec<PanelRow> {
    ...
    for (path, changes) in &entities.per_file {
        ...
        for c in changes.iter().take(thegn_core::semantic::ENTITY_ROW_CAP) {
            let (glyph, tok) = match c.touch {
                thegn_core::semantic::Touch::Added => ("+", hue(Hue::Green)),
                thegn_core::semantic::Touch::Modified => ("~", hue(Hue::Amber)),
                thegn_core::semantic::Touch::Removed => ("−", hue(Hue::Red)),
            };
            let mut segs = vec![
                sp(2),
        ...
        if changes.len() > thegn_core::semantic::ENTITY_ROW_CAP {
            rows.push(PanelRow::plain(Line::segs(vec![
                sp(2),
                seg(
                    f(),
                    format!(
                        "… +{} more",
                        changes.len() - thegn_core::semantic::ENTITY_ROW_CAP
                    ),
                ),
            ])));
        }
...
fn impact_counts_segs(
    per_file: &[(String, Vec<thegn_core::semantic::EntityChange>)],
    cols: usize,
) -> Vec<Seg> {
...
mod tests {
    ...
    fn change_row_path_is_legible_on_the_panel() {
        ...
        let c = ChangeRow {
            ...
            dir: "crates/thegn-host/src/".into(),
    ...
    fn clip_dir_left_snaps_to_slash_boundary() {
        // "crates/thegn-host/src/" (25 chars) + "chrome.rs" (9) = 34
        // budget = 20 → dir_budget = 11, take = 10, from = 15 → snap after "host/"
        let d = clip_dir_left("crates/thegn-host/src/", 9, 20);
