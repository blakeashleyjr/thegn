...
pub fn summary(section: Section, model: &crate::chrome::FrameModel) -> Vec<Seg> {
    ...
    match section {
        ...
        Section::PrQueue => {
            ...
            let count = |f: &dyn Fn(&str) -> bool| q.iter().filter(|r| f(&r.status)).count();
            let ready = count(&|s| s == "ready" || s == "merged");
            // `needs_human` and a red blocker are the same call to action here:
            // this PR is not going to move without you.
            let stuck = count(&|s| matches!(s, "needs_human" | "blocked_ci" | "blocked_conflict"));
            let waiting = count(&|s| matches!(s, "blocked_review" | "agent_running" | "merging"));
            let mut v: Vec<crate::seg::Seg> = Vec::new();
            if ready > 0 {
                v.push(seg(hue(Hue::Green), format!("✓{ready}")));
            }
            if stuck > 0 {
                if !v.is_empty() {
                    v.push(seg(g(), " "));
                }
                v.push(seg(hue(Hue::Red), format!("⚑{stuck}")));
            }
            if waiting > 0 {
                if !v.is_empty() {
                    v.push(seg(g(), " "));
                }
                v.push(seg(hue(Hue::Amber), format!("●{waiting}")));
            }
            if v.is_empty() {
                v.push(seg(g2(), "—"));
            }
...
pub fn content(section: Section, ctx: &SectionCtx) -> Vec<PanelRow> {
    match section {
        ...
        Section::PrQueue => pr_queue::content(ctx),
...
mod spec {
    ...
    fn model() -> FrameModel {
        ...
        m.panel.ci_runs = vec![thegn_core::ci::CiRun {
            ...
            jobs: vec![thegn_core::ci::CiJob {
                id: "1".into(),
                name: "build".into(),
                state: thegn_core::ci::CiState::Pass,
                steps: vec![thegn_core::ci::CiStep {
                    name: "compile".into(),
                    state: thegn_core::ci::CiState::Pass,
                    ..Default::default()
                }],
        ...
        m.panel.log = vec![thegn_svc::git::LogRow {
            graph: "*".into(),
            sha: "abc1234".into(),
        ...
        m.panel.notifications = vec![thegn_core::notification::Notification {
            id: 1,
            kind: thegn_core::notification::NotificationKind::AgentDone,
        ...
        m.panel.task_specs = vec![thegn_core::config::Task {
            name: "build".into(),
            command: "cargo build".into(),
            args: vec![],
            cwd: None,
            env: Default::default(),
            kind: thegn_core::config::TaskKind::Build,
            matcher: None,
            scope: None,
        }];
        m.panel.task_last_runs = {
            ...
            h.insert(
                ...
                crate::panel::TaskRunRecord {
                    ...
                    output_tail: "Compiling thegn v0.1.0\nFinished dev profile".into(),
                    running: false,
                },
            );
            h
        ...
        m.panel.hosts = vec![
            crate::host_ui::HostSnapshot {
                ...
                last_probe: Some(thegn_core::util::now() - 60),
        ...
        m.panel.log_lines = vec![
            ...
            .unwrap(),
            thegn_core::log_view::parse_log_line(
                "2026-06-05T12:00:01  ERROR thegn::host  fatal error",
            )
            .unwrap(),
        ];
        m
    ...
    fn sandbox_section_prefers_active_worktree_container() {
        let mut m = model();
        m.active_container_name = "thegn-active-worktree".into();
        m.containers = vec![
            thegn_core::sandbox::ContainerInfo {
                name: "thegn-other-worktree".into(),
                image: "rust:1".into(),
                status: "Up 1h".into(),
                ours: true,
                backend: "podman".into(),
            ...
            thegn_core::sandbox::ContainerInfo {
                name: "thegn-active-worktree".into(),
                image: "rust:1".into(),
                status: "Up 2m".into(),
                ours: true,
                backend: "podman".into(),
    ...
    fn container_info(name: &str, status: &str) -> thegn_core::sandbox::ContainerInfo {
        thegn_core::sandbox::ContainerInfo {
            name: name.into(),
    ...
    fn b1_healthy_container_renders_green_bullet() {
        ...
        m.containers = vec![container_info("thegn-wt-feat", "Up 3 hours")];
    ...
    fn b2_degraded_container_renders_amber_warning() {
        ...
        m.containers = vec![container_info("thegn-wt-feat", "Up 1 hour (degraded)")];
        let rendered = sandbox_rows(&m, PanelWidth::Normal);
    ...
    fn b5_startup_orphans_notice() {
        ...
        m.containers = vec![];
        m.active_sandbox_backend = String::new();
        m.startup_orphans_removed = vec!["thegn-old-wt".into(), "thegn-stale".into()];
        let rendered = sandbox_rows(&m, PanelWidth::Normal);
        ...
        assert!(
            rendered.contains("thegn-old-wt"),
            "expected first name: {rendered}"
        );
        assert!(
            rendered.contains("thegn-stale"),
    ...
    fn b6_timeline_section_in_deep_view() {
        ...
        let events = vec![
            thegn_core::models::ContainerEvent {
                id: 1,
