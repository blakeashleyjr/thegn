...
pub enum Section {
    ...
    /// "New environment…" wizard / `thegn env create`.
    Environments,
...
pub fn resolve_order(cfg: &thegn_core::config::Config) -> Vec<Section> {
...
// Blame parsing moved to `thegn_core::blame` so the semantic layer can group
// it by entity under the core coverage gate; re-exported here for the panel.
pub use thegn_core::blame::{BlameRow, parse_blame_porcelain};
...
pub struct PanelData {
    ...
    pub ci_runs: Vec<thegn_core::ci::CiRun>,
    ...
    /// attention. See [`thegn_core::aggregate`].
    pub across: thegn_core::aggregate::Aggregation,
    ...
    pub entities: Option<thegn_core::semantic::EntitySummary>,
    ...
    pub tracker_issues: Vec<thegn_core::issue::Issue>,
    ...
    /// recent preceding ERROR lines (see `log_view::error_inclusive_tail`), so the
...
pub struct PanelUi {
    ...
    pub hunks: std::collections::HashMap<String, Vec<thegn_svc::git::Hunk>>,
...
pub fn build_change_rows(
    status: &[thegn_svc::git::FileStatus],
    diff: &[thegn_svc::git::DiffEntry],
    ...
    let mut rows: Vec<ChangeRow> = status
        .iter()
        .map(|f| {
            let stage = if thegn_svc::git::is_conflict(f) {
                Stage::Conflict
...
mod tests {
    ...
    fn fstat(staged: char, unstaged: char, path: &str) -> thegn_svc::git::FileStatus {
        thegn_svc::git::FileStatus {
            path: path.into(),
            staged,
            unstaged,
        }
