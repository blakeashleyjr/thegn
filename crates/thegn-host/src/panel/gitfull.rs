...
use thegn_core::rebase_todo::TodoAction;
...
mod tests {
    ...
    use thegn_core::rebase_todo::TodoEntry;
    ...
    fn long_path_wraps_and_all_lines_carry_the_hit() {
        let mut m = model();
        let path = "crates/thegn-host/src/panel/some/deeply/nested/module.rs";
    ...
    fn wrapped_continuation_has_hanging_indent() {
        ...
        m.panel.changes = vec![change(
            "crates/thegn-host/src/panel/some/deeply/nested/module.rs",
        )];
        let frame = build_git_full(&m, &ui(GitView::Files), 120, 40, true);
        let parts = side_text_for_hit(&frame, PanelHit::Row(Section::Changes, 0));
        assert!(parts.len() >= 2);
    ...
    fn count_badge_reflects_items_actually_shown() {
        ...
        m.panel.changes = (0..10)
            .map(|i| {
                change(&format!(
                    "crates/thegn-host/src/panel/deeply/nested/mod_{i}.rs"
                ))
            })
            .collect();
