...
use thegn_core::rebase_todo::{TodoAction, TodoEntry};
...
pub enum GitOp {
    ...
    UndoApply {
        plan: thegn_core::reflog::UndoPlan,
        autostash: bool,
    },
...
fn selection_of(indices: &[(usize, usize)]) -> thegn_core::patch::Selection {
    let mut sel = thegn_core::patch::Selection::default();
    for &(h, l) in indices {
        sel.insert(h, l);
    }
    sel
...
pub fn execute(op: GitOp, loc: &GitLoc, override_gpg: bool) -> GitOpResult {
    ...
    match op {
        ...
        GitOp::UndoPlan { redo } => {
            let marks = match thegn_core::db::Db::open().and_then(|db| db.undo_marks(&loc.path())) {
                Ok(m) => thegn_core::reflog::OurMarks::new(m),
                Err(_) => thegn_core::reflog::OurMarks::default(),
            };
            let plan = if redo {
