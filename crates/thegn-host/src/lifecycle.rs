...
//! thegn's own background polling.
//!
...
//! ranking lives in [`thegn_core::lifecycle::decide`]; this module only gathers
...
use thegn_core::config::{Config, LifecycleConfig};
use thegn_core::db::PoolSpare;
use thegn_core::lifecycle::{
...
pub fn reconcile(session: &crate::session::Session, cfg: &LifecycleConfig) {
    ...
    let now = Instant::now();
    let states = thegn_core::activity::read_states();
    ...
    for g in &session.worktrees {
        ...
        let currently_warm = thegn_svc::bridge::for_loc(&loc).is_some();
...
/// at process start (first observation), so a fresh thegn never hibernates
    ...
    N: Fn() + Clone + Send + 'static,
{
    ...
    // died with a previous thegn session (a restart drops the in-flight thread),
    ...
    for _ in 0..action.create {
        ...
        std::thread::spawn(move || {
            match crate::provision_gate::provision_spare(&cfg, &repo_root, &env_name, |_| {}) {
                Ok(name) => {
                    tracing::debug!(target: "thegn::lifecycle", %name, "warm spare ready");
                    notify();
                }
                Err(e) => thegn_core::msg::warn(&format!("warm pool: spare provision failed: {e}")),
            }
        });
    }
    notify();
...
mod tests {
    ...
    fn recycle_kill_switch_short_circuits_before_any_provider_work() {
        ...
        let spare = thegn_core::db::PoolSpare {
    ...
    fn vps_spares_destroy_instead_of_recycling() {
        ...
        cfg.env.insert(
            ...
            thegn_core::config::EnvConfig {
                placement: thegn_core::config::PlacementMode::Provider,
                provider: thegn_core::config::EnvProviderConfig {
                    provider: "hetzner".into(),
        ...
        let with_cp = thegn_core::db::PoolSpare {
...
mod live_recycle {
    ...
    fn live_spare_checkpoint_capture_and_stale_recycle() {
        ...
        let p = thegn_svc::provider::SpritesProvider::new("", &tok, &name);
        let marker = thegn_core::envplan::EnvPlan::marker_path("/workspace");
        ...
        // full provisioned filesystem. (The .thegn-provisioned marker is
