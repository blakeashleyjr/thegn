...
pub fn keyring_available() -> bool {
    // The explicit off-switch first: a user who disabled the keyring must not
    // pay a probe at all, memoized or otherwise.
    if keyring_disabled() {
        return false;
    }
...
/// stopped `cargo test` from ever completing on that Mac.
///
