...
//! Provisioning injects `THEGN_HOME_NODE` (from [`home_node_id`]) +
//! `THEGN_SANDBOX_AUTH` (from [`mint_token`]) into a machine; the machine's
//! baked `tg-agent` dials home; and the pane path routes a connected sandbox's
//! exec through [`current`] (see `pane::relay_exec`).
//!
//! Opt-in via `THEGN_IROH=1` until a config field lands (config.rs is at its
...
/// Whether the iroh call-home reach is enabled. Opt-in (`THEGN_IROH=1`) until
/// a config field replaces the env toggle.
pub(crate) fn enabled() -> bool {
    matches!(
        std::env::var("THEGN_IROH").as_deref(),
        Ok("1") | Ok("true") | Ok("yes")
    )
}
...
/// The stable home EndpointId string — the value injected as `THEGN_HOME_NODE`.
...
pub(crate) fn ensure_started() {
    ...
    handle.spawn(async {
        let verifier: Arc<dyn TokenVerifier> =
            Arc::new(FnVerifier(|h: &thegn_core::iroh_wire::Hello| {
...
/// return it for injection as `THEGN_SANDBOX_AUTH`. `None` on a DB/CSPRNG error.
...
/// the machine's `tg-agent` needs. `None` ⇒ today's ssh/IPv4-only path.
pub(crate) fn injection_for(sandbox: &str) -> Option<thegn_svc::fly::IrohInject> {
    if !enabled() {
        return None;
    }
    ensure_started();
    let token = mint_token(sandbox)?;
    Some(thegn_svc::fly::IrohInject {
