...
impl SessionActor {
    ...
    pub(crate) async fn run(
        ...
        // Whether ConPTY's opening cursor query has been answered — see the
        // `PaneEvent::Output` arm below. Windows-only because no other platform
        // withholds the child until a terminal replies.
        #[cfg(windows)]
        ...
        let (exit_code, child_exited): (Option<i32>, bool) = loop {
            tokio::select! {
                ev = pane_rx.recv() => match ev {
                    Some(PaneEvent::Output(_, bytes)) => {
                        // Answer ConPTY's cursor-position query, because on a
                        // headless session nobody else will.
                        //
                        // ConPTY opens every session with `ESC[6n` and **stalls
                        // the child until a terminal answers**. The compositor
                        // answers for an attached pane, which is why this never
                        // showed up interactively — but a daemon session with no
                        // client attached is exactly the headless-agent case, and
                        // there the daemon IS the terminal: it owns the
                        // authoritative emulator and the client is only a viewer.
                        // Unanswered, the agent never runs a single command; the
                        // session sits at the handshake forever, looking merely
                        // slow. Measured: the child emits the title/cursor setup
                        // and then nothing at all.
                        //
                        // Once per session, and harmless if a client also
                        // answers — a duplicate reply is ordinary input to a
                        // child that is not reading stdin.
                        #[cfg(windows)]
                        if !answered_dsr {
                            // Carry three bytes so a query split across two
                            // reads is still recognised.
                            dsr_carry.extend_from_slice(&bytes);
                            if dsr_carry.windows(4).any(|w| w == b"\x1b[6n") {
                                // best-effort: the queue is empty at handshake
                                // time, and a pane whose stdin is already wedged
                                // has worse problems than an unanswered query.
                                let _ = stdin_tx.send(b"\x1b[1;1R".to_vec());
                                answered_dsr = true;
                                dsr_carry = Vec::new();
                            } else {
                                let keep = dsr_carry.len().saturating_sub(3);
                                dsr_carry.drain(..keep);
                            }
                        }
