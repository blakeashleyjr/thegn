...
fn failed_pipe_worker_creation_hands_owned_child_to_bounded_reaper() {
    ...
    let mut command = _isolation.git(&fixture.root);
    ...
    assert!(
        matches!(result, Err(Refusal::Unsafe(reason)) if reason.contains("stderr worker unavailable"))
    );
...
fn timeout_returns_without_waiting_for_child_reap() {
    ...
    assert!(matches!(result, Err(Refusal::Unsafe(reason)) if reason.contains("deadline")));
    wait_for_capacity_release(&COUNTER);
}
...
fn inherited_pipe_is_bounded_and_keeps_its_resource_budget() {
    ...
    assert!(
        matches!(result, Err(Refusal::Unsafe(reason)) if reason.contains("reader") && reason.contains("timed out"))
    );
