//! Runtime performance self-profiler (`thegn::perf`).
//!
...
//! the same property the tracing subscriber has when `THEGN_LOG` is unset.
//!
...
/// Enable accounting when `THEGN_PERF=1`/`true` or `THEGN_LOG` selects the
/// `thegn::perf` target. Called from `run::main` right after `log::init` so the
/// state is set before the loop spins up. Cheap and idempotent.
pub fn init() {
    let on = match std::env::var("THEGN_PERF") {
        Ok(v) => {
            let v = v.trim();
            v == "1" || v.eq_ignore_ascii_case("true")
        }
        Err(_) => false,
    } || std::env::var("THEGN_LOG")
        .map(|s| s.contains("thegn::perf"))
        .unwrap_or(false);
    set_enabled(on);
}

/// The rollup cadence (`THEGN_PERF_INTERVAL_MS`, default 10s). The loop emits
/// a `thegn::perf` summary at most this often, piggy-backing on an existing
/// wake — never its own timer thread (that would add a wake source).
pub fn report_interval() -> Duration {
    let ms = std::env::var("THEGN_PERF_INTERVAL_MS")
        .ok()
...
/// storm warning (`THEGN_PERF_WAKE_LIMIT`, default 20).
pub fn wake_storm_limit() -> f64 {
    std::env::var("THEGN_PERF_WAKE_LIMIT")
        .ok()
...
/// only a genuinely runaway pane should trip it (`THEGN_PERF_PTY_LIMIT`,
...
pub fn pty_storm_limit() -> f64 {
    std::env::var("THEGN_PERF_PTY_LIMIT")
        .ok()
        .and_then(|s| s.parse::<f64>().ok())
        .filter(|v| *v > 0.0)
        .unwrap_or(50.0)
...
pub fn frame_budget_us() -> u64 {
    std::env::var("THEGN_FRAME_BUDGET_US")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(16_000)
...
pub fn input_budget_us() -> u64 {
    std::env::var("THEGN_INPUT_BUDGET_US")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(50_000)
...
// ---------------------------------------------------------------------------
// Bench window (THEGN_BENCH_RUN_MS): run the full loop for a fixed window
...
/// If `THEGN_BENCH_RUN_MS` is set, spawn a one-shot thread that, after the
...
pub fn request_stop_after(
    ...
    let ms = std::env::var("THEGN_BENCH_RUN_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|&v| v > 0)?;
...
impl LoopPerf {
    /// Compute rates, emit the `thegn::perf` tracing rollup (+ a wake-storm
