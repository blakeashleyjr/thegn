...
/// failover is off, so thegn refuses to silently open a host shell. `[r]`
...
pub fn reach_picker(cfg: &thegn_core::config::ShareConfig) -> MenuOverlay {
    use thegn_core::config::ShareReach;
    let items = cfg
        .configured_reaches()
        .into_iter()
        .map(|r| {
...
mod tests {
    ...
    fn reach_picker_lists_configured_reaches_and_picks() {
        use thegn_core::config::{ShareConfig, ShareProviderKind, ShareReach};
        let cfg = ShareConfig {
    ...
    fn reach_picker_omits_public_when_disallowed() {
        use thegn_core::config::{ShareConfig, ShareProviderKind};
        let cfg = ShareConfig {
            public: ShareProviderKind::Frp,
            peer: ShareProviderKind::Iroh,
            allow_public: false,
