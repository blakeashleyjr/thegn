...
use thegn_core::host_config::{HostReach, InstallConsent};
    ...
    cfg.host
        ...
        .map(|(name, hc)| {
            ...
            let image = if hc.image.trim().is_empty() {
                thegn_core::image::ImageRef::default_base().name_tag()
            } else {
                hc.image.clone()
            };
    ...
    match key {
        ...
        Char('c') => {
            *active_menu = Some(crate::menu::confirm_menu(
                ...
                format!(
                    "Record install consent for {name}: thegn may bootstrap a \
...
mod tests {
    ...
    use thegn_core::host::{HostCaps, HostFailure, HostStep};

    fn cfg_with_host(name: &str) -> Config {
        let mut cfg = Config::default();
        cfg.host.insert(
            name.into(),
            thegn_core::host_config::HostConfig {
    ...
    fn snapshots_cover_every_config_host_even_without_db_rows() {
        ...
        assert!(s.image.contains("thegn-sandbox"), "{}", s.image);
    ...
    fn snapshots_join_db_state_caps_inventory_and_events() {
        ...
        db.host_event(&id, "deliver", "delivered 11111111 via oci", 92)
            .unwrap();
        let digest = thegn_core::image::Digest::from_hex(&"1".repeat(64)).unwrap();
        db.host_inventory_put(&thegn_core::inventory::InventoryEntry {
            key: thegn_core::inventory::InventoryKey {
                host: id.clone(),
                kind: thegn_core::inventory::ArtifactKind::Image,
                digest,
                arch: thegn_core::host::Arch::Amd64,
            },
