...
fn parse_composite(
    ...
    let warn = |msg: &str| thegn_core::msg::warn(&format!("[[actions]] {name}: {msg}; skipped"));
...
impl KeyMap {
    pub fn new() -> Self {
        Self::with_config(thegn_core::config::Config::default())
    }

    pub fn with_config(config: thegn_core::config::Config) -> Self {
        Self {
    ...
    pub fn config(&self) -> &thegn_core::config::Config {
        &self.config
    }
...
/// create/close action); see `crates/thegn-core/src/keymap.rs`.
...
pub fn default_keymap_with_config(cfg: &thegn_core::config::Config) -> KeyMap {
...
/// [`Config::effective_keybinds`](thegn_core::config::Config::effective_keybinds)
    ...
    for (program, remaps) in &cfg.program_remap {
        for (from, to) in remaps {
            match (single_key(from), parse_chord(to)) {
                ...
                (Err(e), _) | (_, Err(e)) => thegn_core::msg::warn(&format!(
...
/// Apply one keybind layer (a [`KeybindConfig`](thegn_core::config::KeybindConfig)) onto `map`: the flat table
/// rebinds across all modes, the nested tables rebind their named mode only.
fn apply_keybind_layer(map: &mut KeyMap, layer: &thegn_core::config::KeybindConfig) {
...
pub fn startup_mode(cfg: &thegn_core::config::Config) -> Mode {
...
fn parse_mode(s: &str) -> Option<Mode> {
    match s.trim().to_ascii_lowercase().replace('_', "-").as_str() {
        ...
        other => {
            thegn_core::msg::warn(&format!("profile: unknown default_mode {other:?}; ignored"));
            None
        }
    }
}
...
fn apply_override(
    ...
    let action = if let Some(idx) = custom_idx {
        ...
        thegn_core::msg::warn(&format!("host keymap: unknown action {id:?}; ignored"));
        return;
    };
...
mod tests {
    ...
    fn keymap_preset_default_leaves_ide_chords_unbound() {
        let cfg = thegn_core::config::Config::default(); // "default"
        let mut map = default_keymap_for(&cfg, None, None);
    ...
    fn user_keybind_overrides_preset() {
        let mut cfg = thegn_core::config::Config {
            keymap_preset: "vscode".into(),
            ..Default::default()
        };
    ...
    fn profile_default_mode_overrides_name_heuristic() {
        ...
        profiles.insert(
            "custom".to_string(),
            thegn_core::config::ProfileConfig {
    ...
    fn global_keybind_beats_profile_layer() {
        ...
        let mut prof = thegn_core::config::ProfileConfig::default();
    ...
    fn program_overlay_binds_action_for_focused_program() {
        let mut cfg = thegn_core::config::Config::default();
        let mut binds = thegn_core::config::KeybindConfig::default();
    ...
    fn cfg_with_composite(action: &str, params: &[(&str, &str)]) -> thegn_core::config::Config {
        let mut cfg = thegn_core::config::Config::default();
        cfg.actions.push(thegn_core::config::CustomAction {
            name: "composite".into(),
            key: "Alt N".into(),
            run: None,
    ...
    fn file_worktree_folders_collects_and_dedups() {
        let mk = |name: &str, key: &str, folder: &str| thegn_core::config::CustomAction {
            name: name.into(),
            key: key.into(),
            run: None,
            action: Some("file-worktree".into()),
