...
//! ratcheted `agent.rs`.
//!
//! A launched agent authenticates to `tgproxy` with a minted virtual key whose
...
/// routes through `tgproxy` scoped to `worktree:<path>`. Returns the bearer
...
pub fn mint_agent_proxy_key(worktree: &str, upstream: Option<&str>) -> Option<String> {
    let slug = thegn_core::util::slugify(worktree);
...
pub fn agent_proxy_key_id(worktree: &str) -> String {
    format!("szk-{}", thegn_core::util::slugify(worktree))
}
...
pub fn revoke_agent_proxy_key(key: &str) {
    if let Ok(db) = Db::open() {
        let _ = db.revoke_proxy_virtual_key(key, thegn_core::util::now());
    }
}
