...
fn sources() -> Vec<(String, String)> {
    ...
    files
        .into_iter()
        .filter_map(|p| {
            // Forward slashes, always: the keys are matched against
            // `test/caret-glyph-ratchet.txt`, which is checked in with POSIX
            // separators. Without this, every allowlisted file in a subdirectory
            // (`panel/sections/misc.rs`) reads as `panel\sections\misc.rs` on
            // Windows, matches nothing, and the ratchet fails there only.
            let key = p
                .strip_prefix(&root)
                .ok()?
                .to_string_lossy()
