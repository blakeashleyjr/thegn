...
fn mk_term(name: &str, conn: &str, kind: &str) -> thegn_core::models::TerminalRow {
...
fn ring_navigates_back_to_top_workspace() {
    // Two DB-backed workspaces: thegn at index 0 (the top) and washu at index 1.
    // The session is currently in washu. Stepping Up must land on thegn with a
    // real repo_path — never a live fallback, which would silently block the switch.
    // This is the exact scenario: "Shift+Alt+Up does not select thegn (the top one)."
    ...
    // thegn is the top workspace with a real path (not a live fallback).
    assert_eq!(
        ring,
        vec![
            RingStop::Workspace {
    ...
    // From washu (cur=1), Prev (Up) steps back to thegn (cur=0).
...
fn forgetting_closed_worktree_registry_prevents_restart_readoption() {
    ...
    let db = thegn_core::db::Db::open_at(&root.join("state/thegn.db")).unwrap();
    ...
    assert!(
        db.worktree_for_tab(&thegn_core::db::session(), "app/feat")
            .unwrap()
            .is_none()
    );
...
fn palette_worktree_switch_persists_active_tab_for_target_workspace() {
    ...
    let row = |name: &str, ord: i64| thegn_core::models::TabGroupRow {
        name: name.into(),
        kind: "branch".into(),
...
fn removing_workspace_drops_its_rows_without_a_hydration() {
    ...
    crate::handlers::workspace_remove::forget_workspace_in_model(&mut model, "lib", "/tmp/lib");
...
fn new_workspace_discovery_finds_repos_under_repo_roots() {
    ...
    let repos = thegn_core::repo::discover_repos(&cfg);
    assert!(
...
fn legacy_nul_byte_does_not_trigger_summon_workspace() {
    ...
    // Ctrl+Space, NOT SummonWorkspace. This documents why thegn relies on
...
fn workspace_switch_does_not_duplicate_sidebar_workspaces() {
    ...
    let slugs: Vec<_> = model
        ...
        .map(|(s, _, _, _)| s.as_str())
        .collect();
    assert_eq!(slugs, vec!["thegn", "washu"]);
