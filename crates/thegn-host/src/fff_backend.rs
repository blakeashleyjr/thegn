...
//! the host talks to a small, thegn-shaped surface instead of fff's stateful
...
//! loop.
//!
//! Design notes tying into thegn's invariants:
//!
...
//!   `$XDG_STATE_HOME/thegn/fff/` and is strictly best-effort: if it fails to
...
fn compute_ignored(root: &Path) -> Vec<String> {
    ...
    let out = thegn_core::util::git_cmd(root)
        .args([
            "ls-files",
            "--others",
            "--ignored",
...
fn fff_dir() -> PathBuf {
    thegn_core::util::xdg_state_home().join("thegn").join("fff")
}
...
/// A content (grep) match, in thegn's shape.
