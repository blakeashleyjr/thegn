...
//! event loop, so an idle thegn still produces zero events.
...
use thegn_core::host_config::{HostConfig, HostReach};
...
impl TerminalWizard {
    ...
    pub fn new(cfg: &Config, taken: &[String]) -> Self {
        ...
        let taken_set = thegn_core::worktree::BranchSet::from_names(taken.iter().cloned());
        let generated_slug =
            thegn_core::worktree::dedupe(&thegn_core::worktree::random_pair(), &taken_set);
        TerminalWizard {
            focus: Field::Name,
            name: String::new(),
            name_edited: false,
...
mod tests {
    ...
    fn cfg_with_hosts() -> Config {
        use thegn_core::host_config::HostConfig;
