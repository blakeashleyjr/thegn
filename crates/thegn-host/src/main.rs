//! thegn-host — the native terminal compositor.
//!
...
#[command(
    name = "thegn",
    version,
    about = "thegn — terminal-native worktree IDE"
)]
pub struct Cli {
    ...
    /// to `THEGN_PROFILE`; absent / `default` keeps today's shared paths.
...
pub enum Command {
    ...
    /// Tail or query the thegn log file (plugin/script API).
    Logs {
        #[command(subcommand)]
...
fn main() -> anyhow::Result<()> {
    ...
    // single-threaded). thegn targets git explicitly with `-C <dir>`, so it
...
fn run_subcommand(cli: &Cli, command: Command) -> anyhow::Result<()> {
    ...
    match command {
        ...
        Command::Bridge => {
            ...
            thegn_svc::bridge::serve(std::io::stdin().lock(), std::io::stdout());
            Ok(())
        }
        Command::BridgeRevtunnel { port } => {
            ...
            rt.block_on(async move {
                ...
                thegn_svc::revtunnel::run_sandbox(stream, listener).await
            })?;
            Ok(())
        }
        ...
        Command::SandboxArgv { worktree } => {
            ...
            match crate::agent::launch_spec(&cfg, &wt, None, "shell") {
                ...
                Err(e) => {
                    thegn_core::msg::die(&format!("launch_spec failed: {e:#}"));
                }
            }
            Ok(())
        }
