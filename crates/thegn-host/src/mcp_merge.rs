//! `HouseMerge` implementation — exposes thegn's local merge queue to the
...
//! implementing the `thegn_core::mcp::HouseMerge` trait the core `McpRouter`
...
pub fn handle_house_request(
    ...
    match Db::open() {
        Ok(db) => {
            ...
            let mut router = thegn_core::mcp::router::McpRouter::new(Arc::new(db), Arc::new(bus))
                .with_git(
                    Arc::new(thegn_svc::mcp_git::HouseGitImpl),
                    worktree.to_string(),
                )
                .with_forge(
                    Arc::new(thegn_svc::mcp_git::HouseGitImpl),
...
impl thegn_core::mcp::HouseMerge for HouseMergeImpl {
