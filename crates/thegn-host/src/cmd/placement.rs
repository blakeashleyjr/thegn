//! `thegn placement <action>` — inspect the placement engine: the per-host
...
fn list(cfg: &Config, json: bool) -> Result<()> {
    ...
    for (name, hc) in &cfg.host {
        if hc.reach == thegn_core::host_config::HostReach::Cloud {
        ...
        let ownership = cap
            .as_ref()
            .map(|c| c.ownership)
            .unwrap_or(thegn_core::capacity::HostOwnership::Independent);
        let trust = match row.as_ref().and_then(|r| r.caps.as_ref()) {
            Some(caps) => {
                thegn_core::trust_class::effective_class(caps, ownership, hc.trust_egress_enforced)
                    .to_string()
            }
