//! `thegn proxy <action>` — inspect and manage the LLM proxy (`tgproxy`):
...
use thegn_core::store::{ProxyStore, budget_period_len_ms};
use thegn_core::util;
...
fn status(cfg: &Config) -> Result<()> {
    ...
    let health = match get_json(&format!("{base}/health")) {
        ...
        Err(_) => {
            ...
            outln!("  start it with `thegn proxy serve` or `[llm_proxy] enabled = true`");
            return Ok(());
        }
    };
    outln!(
...
/// Runs `tgproxy` in the foreground with the resolved `[llm_proxy]` env —
/// standalone mode, independent of `enabled` (which only gates the host's
/// auto-launch). The daemon binary ships as a sibling of `thegn`.
...
fn serve(cfg: &Config) -> Result<()> {
    ...
    let (program, args, env) = proxy_cfg
        .launch_spec()
        .context("build tgproxy launch spec")?;
    ...
    if !status.success() {
        bail!("tgproxy exited with {status}");
    }
    Ok(())
}
