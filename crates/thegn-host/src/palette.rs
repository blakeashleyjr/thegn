...
/// score (`thegn_core::frecency` — frequency × recency decay, so a place
...
pub fn order_by_frecency(
    ...
    order_by_frecency_at(items, usage, thegn_core::util::now())
}
...
fn order_by_frecency_at(
    ...
    let rank: HashMap<&str, f64> = usage
        .iter()
        .map(|(k, c, l)| (k.as_str(), thegn_core::frecency::score(*c, *l, now)))
        .collect();
...
pub(crate) fn configured_file_worktree_folders(cfg: &thegn_core::config::Config) -> Vec<String> {
...
/// passed straight to [`thegn_core::config::Config::resolve_env`]. The label
...
pub(crate) fn build_env_palette(
    ...
    use thegn_core::config::PlacementMode as P;
    let def = {
    ...
    // map of what thegn can run on, not just what is already configured.
...
/// starts an interactive login. See [`thegn_core::account`].
...
/// bundle is marked. See [`thegn_core::bundle`].
...
pub(crate) fn build_profile_palette(cfg: &thegn_core::config::Config) -> Vec<PaletteItem> {
    ...
    let pdir = thegn_core::util::xdg_config_home()
        .join("thegn")
        .join("profiles");
...
mod tests {
    ...
    fn build_bundle_palette_lists_bundles_clear_and_marks_active() {
        use thegn_core::config::{Bundle, Config};
        use thegn_core::db::Db;
        let db = Db::open_memory().unwrap();
        let mut cfg = Config::default();
        cfg.bundle.insert("work".into(), Bundle::default());
        cfg.bundle.insert("personal".into(), Bundle::default());
        thegn_core::bundle::set_active(&db, thegn_core::bundle::Bind::Global, "/wt", None, "work")
            .unwrap();
    ...
    fn build_profile_palette_includes_default_and_configured_marks_active() {
        use thegn_core::config::{Config, ProfileConfig};
        let mut cfg = Config::default();
    ...
    fn build_env_palette_offers_setup_rows_for_unconfigured_providers() {
        use thegn_core::config::{Config, EnvConfig, EnvProviderConfig, PlacementMode};
        let mut cfg = Config::default();
        cfg.env.insert(
            "sprite".into(),
            EnvConfig {
    ...
    fn cfg_with_file_worktree(folders: &[&str]) -> thegn_core::config::Config {
        let mut cfg = thegn_core::config::Config::default();
