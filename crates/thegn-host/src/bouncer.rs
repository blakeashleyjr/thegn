...
//! thegn over a bind-mounted unix-socket ACP channel, and gate the
...
use thegn_core::config::Network;
use thegn_core::sandbox::{Backend, Mount};
...
pub enum ApprovalKind {
    ...
    Shell,
    /// `thegn/edit` — an in-place edit of a worktree file.
    Edit,
    /// `thegn/write` — a full-content write of a worktree file.
    Write,
}
...
pub fn gated_kind(inbound: &AcpInbound) -> Option<(ApprovalKind, String)> {
    match inbound {
        ...
        AcpInbound::ThegnEditRequest { path, .. } => Some((ApprovalKind::Edit, path.clone())),
        AcpInbound::ThegnWriteRequest { path, .. } => Some((ApprovalKind::Write, path.clone())),
        _ => None,
    }
}
...
pub struct AgentEnvPlan {
    ...
    /// Host path thegn connects to for ACP. `Some` ⇒ unix-socket transport
    /// (bouncer + sandbox); `None` ⇒ the caller uses the TCP port.
    pub acp_socket: Option<PathBuf>,
    /// Host path thegn must serve the model-proxy relay on (full seal only).
...
pub fn agent_env_plan(
    ...
    if lp.route_agent {
        plan.vars
            .push(("THEGN_PROXY_API".into(), lp.agent_api.clone()));
        plan.vars
            .push(("THEGN_PROXY_MODEL".into(), lp.agent_model.clone()));
        if let Some(k) = proxy_key {
            plan.vars.push(("THEGN_PROXY_KEY".into(), k.to_string()));
        }
        match proxy_reach(&lp.listen, sandbox) {
            ProxyReach::Url(url) => {
                plan.vars.push(("THEGN_PROXY_BASE_URL".into(), url));
            }
            ProxyReach::Unix => {
                ...
                plan.vars.push((
                    "THEGN_PROXY_BASE_URL".into(),
                    "http://proxy.thegn.internal".into(),
                ));
                plan.vars.push(("THEGN_PROXY_UNIX".into(), sock_s.clone()));
    ...
    if lp.bouncer && sandboxed {
        ...
        let sock_s = sock.to_string_lossy().into_owned();
        plan.vars.push(("THEGN_BOUNCER".into(), "1".into()));
...
mod tests {
    ...
    fn gated_kind_gates_shell_edit_write_but_not_reads() {
        ...
        assert_eq!(
            gated_kind(&AcpInbound::ThegnEditRequest {
                id: Id::Number(1),
                path: "a.rs".into(),
        ...
        assert_eq!(
            gated_kind(&AcpInbound::ThegnWriteRequest {
                id: Id::Number(1),
                path: "b.rs".into(),
                content: "x".into(),
            })
    ...
    fn agent_env_plan_host_pane_has_no_bouncer_or_mounts() {
        ...
        assert!(plan.vars.iter().all(|(k, _)| k != "THEGN_BOUNCER"));
    ...
    fn agent_env_plan_nat_sandbox_enables_bouncer_over_gateway() {
        ...
        assert!(plan.vars.iter().any(
            |(k, v)| k == "THEGN_PROXY_BASE_URL" && v == "http://host.containers.internal:8383"
        ));
        // Bouncer override is on, with a unix-socket ACP channel + its mount.
        assert!(
            plan.vars
                .iter()
                .any(|(k, v)| k == "THEGN_BOUNCER" && v == "1")
        );
    ...
    fn agent_env_plan_sealed_sandbox_relays_proxy_over_unix() {
        ...
        assert!(plan.vars.iter().any(|(k, _)| k == "THEGN_PROXY_UNIX"));
        ...
        assert!(plan.vars.iter().any(|(k, _)| k == "THEGN_BOUNCER"));
    ...
    fn agent_env_plan_respects_route_agent_off() {
        ...
        assert!(plan.vars.iter().all(|(k, _)| !k.starts_with("THEGN_PROXY")));
