...
use thegn_core::config::{Config, Pin, PinCorner, PinLocation, PinRestart};
...
impl PinSupervisor {
    ...
    pub fn eager_pins<'a>(cfg: &'a Config, workspace: Option<&str>) -> Vec<&'a Pin> {
        use thegn_core::config::PinStart;
...
mod tests {
    use super::*;
    use thegn_core::config::{PinScope, PinStart};
    ...
    fn pin(name: &str, loc: PinLocation) -> Pin {
        Pin {
            ...
            label: None,
            ratio: None,
            corner: thegn_core::config::PinCorner::BottomRight,
            corner_width: None,
            corner_height: None,
        }
    }
