...
pub(crate) fn compute_entity_summary(
    ...
    diff_entries: &[thegn_svc::git::DiffEntry],
) -> Option<thegn_core::semantic::EntitySummary> {
    use thegn_core::semantic::{EntitySummary, Lang, entities_for_diff};
    ...
    for f in thegn_core::patch::parse_patch(&diff) {
...
mod tests {
    ...
    fn compute_entity_summary_over_a_real_repo() {
        ...
        let loc = thegn_core::remote::GitLoc::for_worktree(&dir);
        // A non-empty diff_entries list (only its length gates the call).
        let entries = vec![thegn_svc::git::DiffEntry {
            path: "lib.rs".into(),
            added: 1,
            deleted: 1,
        }];
