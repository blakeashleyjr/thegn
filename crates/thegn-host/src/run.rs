...
pub async fn main(cli: crate::Cli) -> Result<()> {
    ...
    // that scroll for the life of the session (restored on teardown).
    {
        ...
        let mouse_ok =
            thegn_core::termcaps::detect(&thegn_core::termcaps::TermEnv::from_env()).mouse;
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn begin_worktree_preset(
    root: std::path::PathBuf,
        ...
        while let Ok(ev) = create_rx.try_recv() {
            ...
            match ev {
                ...
                wizard::CreateEvent::Done {
                    ...
                    crate::handlers::tracker::on_worktree_created(
                        &mut pending_issue_link,
                        generation,
                        &payload.path,
                    );
                    refresh_tab_model(&mut model, &session, &mut sb);
            ...
            Ok(Some(InputEvent::Key(k))) => {
                    ...
                    && !k.modifiers.contains(Modifiers::ALT)
                {
                                ...
                                panel_ui.row_mode = true;
                                {
                                    match panel_ui.open {
                                        ...
                                        Section::Issues => {
                                            ...
                                            // the cursor row.
                                            crate::handlers::tracker::toggle_link(
                                                &mut crate::handlers::tracker::TrackerCtx {
                                                    model: &mut model,
                                                    panel_ui: &mut panel_ui,
                                                    session: &session,
                                                    cfg: keymap.config(),
                                                    live_cfg: &current_config,
                                                    waker: &waker,
                                                    create_gen: &mut create_gen,
                                                    create_tx: &create_tx,
                                                    inflight: &mut inflight,
                                                    wizard_ui: &mut wizard_ui,
                                                    pending_issue_link: &mut pending_issue_link,
                                                    model_tx: &model_tx,
                                                    hydration_gen: &mut hydration_gen,
                                                },
                                            );
                                        }
                    ...
                    let handled = match (panel_ui.open, k.key) {
                        ...
                        (Section::Mine | Section::Issues, KeyCode::Char('b')) => {
                            crate::handlers::tracker::branch_from_issue(
                                &mut crate::handlers::tracker::TrackerCtx {
                                    model: &mut model,
                                    panel_ui: &mut panel_ui,
                                    session: &session,
                        ...
                        // -- issues: open URL, refresh, self-assign, dispatch --
                        (Section::Issues, KeyCode::Char(c @ ('o' | 'r' | 'a' | 'D'))) => {
                            crate::handlers::tracker::issues_key(
                                c,
                                &mut crate::handlers::tracker::TrackerCtx {
                                    model: &mut model,
                                    panel_ui: &mut panel_ui,
                ...
                match dispatch {
                    crate::sequence::MatchResult::Matched(action) => {
                        ...
                        which_key.clear();
                        which_key_filter = None;
                        // Alt+arrow (`Nav*`) is one seamless motion: it walks the
                        // same spatial focus graph as Ctrl+arrow, and only when a
                        // move dead-ends at the outer edge does it fall through to
                        // the tab/worktree switch Alt historically owned. Resolve
                        // that here by rewriting the Nav action into either the
                        // matching `Focus*` action (the move lands — hand off to the
                        // full, coalesced focus gesture below) or the tab/worktree
                        // action (a true dead-end). This keeps `Nav*` out of the
                        // match entirely — it reuses both existing code paths.
                        ...
                        | Action::NavDown = action
                        {
                            ...
                            action = match (action, dead_end) {
                                (Action::NavLeft, true) => Action::PrevTab,
                                (Action::NavRight, true) => Action::NextTab,
                                (Action::NavUp, true) => Action::PrevWorktree,
                                (_, true) => Action::NextWorktree,
                                (Action::NavLeft, false) => Action::FocusLeft,
                                (Action::NavRight, false) => Action::FocusRight,
                                (Action::NavUp, false) => Action::FocusUp,
                                (_, false) => Action::FocusDown,
                            };
                        }
                        match action {
                            ...
                            | Action::NavUp
                            | Action::NavDown => {
                                debug_assert!(false, "Nav* rewritten before dispatch");
                            }
                            Action::FocusLeft
                            | Action::FocusRight
                            | Action::FocusUp
