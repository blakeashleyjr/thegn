...
impl ProcessObservation {
    fn if_owned<T>(&self, operation: impl FnOnce() -> T) -> Option<T> {
        if self.lost { None } else { Some(operation()) }
    }
    fn observe<T>(&mut self, operation: impl FnOnce() -> std::io::Result<T>) -> std::io::Result<T> {
...
impl OwnedChild {
    fn spawn(command: &mut Command, root: Arc<tempfile::TempDir>) -> Self {
        let slot = {
    ...
    fn terminate(&mut self, budget: Duration) -> bool {
