...
/// the `tg-host` label so two thegn hosts sharing one cloud account never
...
pub fn host_label() -> String {
    ...
    thegn_core::util::short_hash(&seed, 10)
}
...
impl VpsProvider {
    ...
    async fn ensure_ssh_key(&self) -> Result<i64> {
        ...
        // "thegn-managed": vendor key names must be unique, so a fixed name
        ...
        let created = self
            .post_json(
                ...
                &shaper.ssh_key_body(&format!("thegn-managed-{fp}"), self.spec.pubkey.trim()),
            )
            .await?;
        shaper
    ...
    pub async fn resolve_ip(&self, name: &str) -> Result<String> {
        ...
        let _ = registry::write(&registry::VpsRecord {
            // best-effort: registry is a cache; write failure just means no resurrection row
            name: name.to_string(),
            provider: self.spec.kind.as_str().into(),
            state: "ready".into(),
            instance_id: inst.id,
            ip: ip.clone(),
            created_at: inst.created.unwrap_or_else(thegn_core::util::now),
        });
    ...
    async fn wait_reachable(&self, name: &str, ip: &str) -> Result<()> {
        ...
        let _ = tokio::time::timeout(
            // best-effort: cloud-init wait is advisory (see comment above)
    ...
    /// the pre-snapshot quiesce for `thegn env image bake`.
