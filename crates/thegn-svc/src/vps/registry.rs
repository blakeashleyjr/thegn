//! File-based VPS instance registry under `$XDG_STATE/thegn/vps/` — one JSON
...
//! doubles as the IP cache the `thegn vps-ssh` attach bridge reads (no API
...
pub fn dir() -> PathBuf {
    thegn_core::util::thegn_dir().join("vps")
}
