...
//! public IPv4 → create the machine → poll it to `started`. thegn reaches it
//! over plain ssh on that IPv4 (see [`super::FlyProvider`]), reusing the VPS
//! `ssh_shim` — no vendor CLI.
//!
//! Scoping is by **machine metadata** (`managed-by=thegn`, `tg-host=<hash>`),
...
pub const MANAGED_VAL: &str = "thegn";
pub const HOST_KEY: &str = "tg-host";
...
/// The create-app body. Fly app names are globally unique, so thegn derives a
...
/// the machine stays up as an ssh box (the same model thegn gives a VPS).
/// Verified live. `thegn env image-bake` can later fold this into an image.
...
/// thegn reaches it like a VPS: `authorized_key` rides in via a Machines
...
/// - **prebaked** (`prebaked = true`): a baked thegn image (see
///   `nix/fly-sandbox-image.nix`) whose OWN entrypoint runs sshd and already has
///   the toolchain — thegn must NOT override the init, so the machine boots
...
pub fn create_machine_body(
    ...
    if let Some(inject) = iroh {
        // iroh call-home: the baked `tg-agent` reads these three `THEGN_*` env
        ...
        config["env"] = serde_json::json!({
            thegn_core::iroh_wire::HOME_NODE_ENV: inject.home_node,
            thegn_core::iroh_wire::SANDBOX_AUTH_ENV: inject.sandbox_auth,
...
pub struct FlyMachine {
    ...
    /// thegn reaches the machine over the app's public IPv4, not this; kept
...
mod tests {
    ...
    fn iroh_injection_emits_env_with_wire_keys() {
        ...
        assert_eq!(
            b["config"]["env"][thegn_core::iroh_wire::HOME_NODE_ENV],
            "home-endpoint-id"
        );
        assert_eq!(
            b["config"]["env"][thegn_core::iroh_wire::SANDBOX_AUTH_ENV],
            "auth-token-xyz"
        );
        assert_eq!(
    ...
    fn list_keeps_only_managed_machines() {
        let list = serde_json::json!([
            { "id": "a", "name": "mine", "state": "started",
              "config": { "metadata": { "managed-by": "thegn" } } },
