...
//!   [`machines::SSHD_INIT`] brings up sshd with thegn's managed key on a
...
pub struct FlySpec {
    ...
    /// the three `THEGN_*` env vars the baked `tg-agent` reads on boot to dial
...
impl FlyProvider {
    ...
    /// All thegn-managed Fly sandboxes this host created (ledger view).
