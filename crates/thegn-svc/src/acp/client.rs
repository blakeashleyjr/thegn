...
/// bespoke review-pane edit/write methods (`thegn/edit`, `thegn/write`).
...
pub fn parse_client_request(req: Request) -> Option<AcpInbound> {
    ...
    match req.method.as_str() {
        ...
        "thegn/edit" => Some(AcpInbound::ThegnEditRequest {
            id: req.id,
            path: str_field("path"),
            edits: params.get("edits").cloned().unwrap_or(Value::Null),
        }),
        "thegn/write" => Some(AcpInbound::ThegnWriteRequest {
            id: req.id,
            path: str_field("path"),
...
impl AcpClient {
    ...
    /// model. Used to route through `tgproxy` with a per-worktree virtual key
...
mod tests {
    ...
    fn parses_fs_read_edit_write_and_ignores_unknown() {
        ...
        assert!(matches!(
            ...
            Some(AcpInbound::ThegnEditRequest { .. })
        ));
        assert!(matches!(
            parse_client_request(req(
                "thegn/write",
            ...
            Some(AcpInbound::ThegnWriteRequest { .. })
        ));
