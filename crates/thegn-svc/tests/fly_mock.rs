...
//!
//! Run: `cargo test -p thegn-svc --test fly_mock`.
...
fn handle(stream: TcpStream, rec: Arc<Mutex<Vec<Recorded>>>) {
    ...
    let managed = r#"{"managed-by":"thegn","tg-host":"h1"}"#;
...
fn create_list_destroy_with_ledger() {
    ...
    assert_eq!(cbody["config"]["metadata"]["managed-by"], "thegn");
    assert!(
    ...
    // process-wide `THEGN_DIR` (this file keeps a single #[test] because the
