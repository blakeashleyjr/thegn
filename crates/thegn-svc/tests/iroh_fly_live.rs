...
//! `#[ignore]` — needs a token + network + a baked image containing `tg-agent`.
//!
//! Unlike `fly_live.rs` (which ssh's into a dedicated IPv4), this proves the new
//! model: a Fly machine boots the baked `tg-agent`, which **dials home** to a
...
//!
//! Run (org must own the baked image; image must contain `tg-agent` on PATH and an
...
//!     cargo test -p thegn-svc --test iroh_fly_live -- --ignored --nocapture
...
use thegn_core::iroh_wire::Hello;
use thegn_svc::fly::{FlyProvider, FlySpec, IrohInject};
use thegn_svc::iroh_reach::{FnVerifier, IrohHome, TokenVerifier};
use thegn_svc::provider::{ExecFrame, ExecSpec, RemoteProvider};
...
#[tokio::test]
#[ignore = "live: needs FLY_API_TOKEN + FLY_IMAGE (baked tg-agent) + network; creates a real Fly machine"]
async fn machine_dials_home_and_serves_shell_over_iroh() {
    ...
    assert!(
        ...
        "FLY_IMAGE must be a baked image ref (image:<registry>/<app>:<tag>) containing tg-agent"
    );
    ...
    // 2) Provision a Fly machine that boots tg-agent with the injected env.
    let spec = FlySpec {
        api_base: String::new(),
    ...
    eprintln!("[{name}] creating machine (boots tg-agent → dials home)…");
    ...
    let result = async {
            ...
            .open_exec(
                ...
                ExecSpec {
                    argv: vec![
                        "/bin/sh".into(),
                        "-lc".into(),
                        "echo TG_IROH_REMOTE_OK; exit 0".into(),
                    ],
                    tty: true,
                    cols: 80,
                    rows: 24,
        ...
        anyhow::ensure!(
            text.contains("TG_IROH_REMOTE_OK"),
