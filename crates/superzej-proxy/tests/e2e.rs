...
async fn upstream_anthropic_backend_translates_both_ways() {
    ...
    let resp = superzej_proxy::upstream::call_backend(
        &reqwest::Client::new(),
        &b,
...
async fn upstream_network_error_is_err() {
    ...
    let res = superzej_proxy::upstream::call_backend(
        &reqwest::Client::new(),
        &b,
        br#"{"model":"m","messages":[]}"#,
    )
    ...
    let stream = superzej_proxy::upstream::open_openai_stream(
        &reqwest::Client::new(),
        &b,
        br#"{"model":"m"}"#,
    )
...
use superzej_proxy::relay::{self, OpenAiSink, Peek, RelayConfig};
...
async fn wait_for_rows(
    db: &SharedDb,
    n: usize,
    pred: impl Fn(&[superzej_core::db::ProxyRequestRow]) -> bool,
) -> Vec<superzej_core::db::ProxyRequestRow> {
    for _ in 0..50 {
        let rows = db
            .lock()
            .unwrap()
...
async fn model_alias_selects_route() {
    ...
    let cfg = superzej_proxy::config::parse_config(&doc).unwrap();
