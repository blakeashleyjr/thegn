//! superzej-proxy — the LLM proxy daemon (`szproxy`).
//!
//! The async I/O shell around `superzej_core::proxy`'s pure routing logic. It is
...
pub async fn run(config: ProxyConfig) -> Result<()> {
    let db = Arc::new(Mutex::new(Db::open().context("open superzej.db")?));
...
pub async fn serve(config: ProxyConfig, db: shared::SharedDb) -> Result<()> {
    ...
    tracing::info!(%listen, "szproxy listening");
...
async fn shutdown_signal() {
    ...
    tracing::info!("szproxy shutting down");
}
