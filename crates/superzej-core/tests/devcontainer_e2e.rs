...
use superzej_core::config::{SandboxBackend, SandboxConfig, SandboxProfile};
use superzej_core::devcontainer::{self, SubstCtx};
use superzej_core::devcontainer_overlay;
use superzej_core::remote::GitLoc;
use superzej_core::sandbox::{self, container_name};
...
fn e1_build_dockerfile_builds_and_runs() {
    ...
    let name = "superzej-dc-e2e-build";
    force_rm(name);
    ...
    assert!(sb.image.starts_with("superzej-dc-"), "image={}", sb.image);
...
fn e3_compose_up_and_exec() {
    ...
    if !superzej_core::util::have("docker") {
        return;
    }
    let wt = worktree_with(&[
        (
    ...
    let name = "superzej-dc-e2e-compose";

    let sb = overlaid_config(&wt);
    let cs = superzej_core::sandbox_compose::ComposeSpec::decode(sb.compose.as_ref().unwrap());
