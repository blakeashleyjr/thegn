...
//! API) lives in `superzej-svc::projection`. A [`ProjectionSpec`] is assembled
...
/// Pure data; the bring-up/teardown lives in `superzej-svc::projection`.
#[derive(Debug, Clone)]
pub struct ProjectionSpec {
    ...
    /// A stable local mountpoint under `<superzej_dir>/mounts` (the sshfs/sync
    /// target, and the cwd a pane should use once projected).
    pub mountpoint: String,
}

/// A stable local mountpoint for a `host:remote_path` pair, under
/// `<superzej_dir>/mounts/<slug>`. Deterministic so mount and unmount agree
