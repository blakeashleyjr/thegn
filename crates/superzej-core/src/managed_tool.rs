...
//!
//! superzej owns a handful of external tools (today the `pi` coding agent under
//! `~/.superzej/pi`; soon a debug adapter and user-declared MCP servers). Rather
...
//! side-effecting fetch lives in `superzej-host` (core carries no HTTP client),
...
/// place new tools under `~/.superzej/tools/<name>`; the managed pi keeps its
/// legacy layout for byte-for-byte compatibility with existing installs.
#[derive(Debug, Clone, PartialEq, Eq)]
struct Layout {
    /// Absolute managed dir, or `None` to derive `~/.superzej/tools/<name>`.
    dir: Option<PathBuf>,
    bin_rel: PathBuf,
    marker_name: String,
}
...
impl ManagedTool {
    ...
    /// (`~/.superzej/tools/<name>` by default, or the pinned legacy dir).
    pub fn managed_dir(&self) -> PathBuf {
        self.layout
            .dir
            .clone()
            .unwrap_or_else(|| util::superzej_dir().join("tools").join(&self.name))
    }
    ...
    /// testable core (no env, no real `~/.superzej`).
...
mod tests {
    ...
    fn path_computation_and_layout_override() {
        ...
        let legacy = PathBuf::from("/home/u/.superzej/pi");
        let pi = tool().with_layout(
            legacy.clone(),
            "node_modules/.bin/pi",
            ".superzej-pi-version",
        );
        ...
        assert_eq!(pi.version_marker(), legacy.join(".superzej-pi-version"));
    }
