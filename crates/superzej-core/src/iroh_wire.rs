...
//! that feed bytes through it live in `superzej-svc::iroh` (home side) and the
//! `superzej-agent` binary (sandbox side).
//!
...
pub const SANDBOX_AUTH_ENV: &str = "SUPERZEJ_SANDBOX_AUTH";
/// Env var naming which sandbox the agent serves (the home registry key).
pub const SANDBOX_ID_ENV: &str = "SUPERZEJ_SANDBOX_ID";
