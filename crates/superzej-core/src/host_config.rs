...
//! on. Hosts are **global config only**: the repo overlay (`.superzej.toml`)
...
config_enum! {
    /// `[host.<n>] reach` — how superzej reaches this host.
...
mod tests {
    ...
    fn cloud_host_binds_with_cloud_id() {
        let cfg = cfg_from(
            ...
            template = "superzej-base"
            "#,
        );
        let b = cfg.host_binding("sprites-us").unwrap();
        assert_eq!(b.id, HostId::cloud("sprites", "superzej-base"));
    ...
    fn repo_overlay_cannot_define_hosts() {
        ...
        // `[host.*]` smuggled into .superzej.toml can never reach Config.host:
