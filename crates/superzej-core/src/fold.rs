...
//! adapter over `superzej_svc::git::PlumbingOps` (merge-tree + commit-tree), then
...
/// than reusing svc's `MergeTreeOutcome`) so `superzej-core` needn't depend on
/// `superzej-svc`; the host adapter converts between the two.
