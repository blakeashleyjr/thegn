...
mod tests {
    ...
    fn parse_extracts_path_and_filters_noise() {
        ...
        let json = r#"{
            "variables": {
                ...
                "SUPERZEJ_YAZI_BIN": {"type": "exported", "value": "/nix/store/yz/bin/yazi"},
    ...
    fn cache_key_none_without_flake_and_stable_with() {
        let dir = std::env::temp_dir().join(format!("sj-devenv-key-{}", std::process::id()));
