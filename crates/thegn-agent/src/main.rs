//! `tg-agent` — the lean in-sandbox dialer.
//!
//! Baked into a thegn workspace image and launched as PID 1. On boot it reads
//! two env vars injected at provision time — `THEGN_HOME_NODE` (the
//! compositor's stable iroh EndpointId, the dial target) and
//! `THEGN_SANDBOX_AUTH` (this sandbox's minted, short-lived auth token) — then
...
//!
//! This is the "call-home" inversion of the old dumbpipe-ticket model: thegn
//! is the stable node, every sandbox dials it. The var names deliberately avoid a
//! `*_TOKEN` suffix so they clear thegn's `THEGN_*` host allowlist and the
...
//! in-process against `thegn-svc::iroh` before wiring provisioning).
...
use thegn_agent::serve;
use thegn_core::iroh_wire::{ALPN, HOME_NODE_ENV, Hello, SANDBOX_AUTH_ENV, SANDBOX_ID_ENV};
...
async fn main() -> Result<()> {
    ...
    eprintln!("tg-agent: node={} dialing home {}", endpoint.id(), home_id);
