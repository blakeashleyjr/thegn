...
//!
//! A home endpoint (`thegn-svc::iroh_reach`) and a sandbox agent (this crate's
...
//! live test (`thegn-svc/tests/fly_live.rs`) mirrors end-to-end.

use std::sync::Arc;
use std::time::Duration;

use thegn_agent::serve;
use thegn_core::iroh_wire::{ALPN, Hello};
...
async fn shell_command_runs_over_iroh_pty() {
    ...
    let spec = ExecSpec {
        argv: vec![
            "/bin/sh".into(),
            "-lc".into(),
            "echo TG_IROH_PTY_OK; exit 7".into(),
        ],
        tty: true,
        cols: 80,
        rows: 24,
