//! `tg-kit` — the thegn embedding contract.
//!
//! Sibling TUIs plug into the thegn compositor as top-level
...
//! termwiz, nor `thegn-core`, so a standalone app can link it without
//! dragging in thegn's stack.
//!
...
//!   mirror thegn's chrome palette and a tolerant config loader so a
//!   standalone binary picks up the user's thegn theme.
...
//! `tg_kit::ratatui` (never a direct `ratatui` dep at a different version), so
