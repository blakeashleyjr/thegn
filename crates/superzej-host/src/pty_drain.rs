...
pub(crate) struct DrainCtx<'a> {
    ...
    pub keymap_config: &'a superzej_core::config::Config,
    pub current_config: &'a superzej_core::config::Config,
    ...
    if let Some((gi, ti, sole)) = owner {
            ...
            .filter(|s| !s.is_empty());
        {
            ...
            if !wt.is_empty() {
                ...
                let policy = superzej_core::event_bus::ProcessExitPolicy::parse(
                    &ctx.current_config.notifications.process_exit,
                );
                let outcome =
                    superzej_core::event_bus::classify_process_exit(exit_code, is_shell, policy);
                ...
                tokio::task::spawn_blocking(move || {
                    ...
                    let event = superzej_core::event_bus::Event::ProcessExited {
                        worktree: wt.clone(),
