...
use superzej_core::models::TerminalRow;
...
pub(crate) fn sidebar_terminals(db: &Db) -> Vec<TerminalRow> {
    match db.terminals() {
        Ok(t) => t,
        Err(e) => {
            tracing::warn!(target: "szhost::hydrate", error = %e, "terminals() read failed; sidebar Terminals section will be empty");
            Vec::new()
        }
    }
}
...
pub(crate) fn active_backend(db: &Db, path: &str, cfg_backend: SandboxBackend) -> String {
    match db.worktree_sandbox(path) {
        ...
        Ok(_) => superzej_core::sandbox::Backend::from_config(cfg_backend)
            .filter(|b| *b != superzej_core::sandbox::Backend::None)
            .map(|b| b.label().to_string())
            .unwrap_or_default(),
        Err(e) => {
            tracing::warn!(target: "szhost::hydrate", error = %e, "worktree_sandbox() read failed; location chip may be blank");
            String::new()
        }
    }
}
