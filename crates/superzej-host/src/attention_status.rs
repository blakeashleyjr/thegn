...
//! into one [`superzej_core::attention::AttentionScore`] per worktree path,
//! plus a **hysteresis-stable** display order for the sidebar's Attention sort
//! (see [`superzej_core::attention::stable_order`]: only a tier or membership
...
/// the branching lives in `superzej_core::attention::score`.
pub(crate) fn collect_attention(
    ...
    let activity = superzej_core::activity::read_entries();
    ...
    for (worktree, json, _fetched_at) in db.list_pr_cache().unwrap_or_default() {
        if let Ok(mut st) = serde_json::from_str::<superzej_core::github::PrStatus>(&json) {
    ...
    for path in meta.keys() {
            ...
            && let Some(latest) = runs.first()
        {
            ...
            ci_running = matches!(
                latest.state,
                superzej_core::ci::CiState::Running | superzej_core::ci::CiState::Pending
            );
        }
...
mod tests {
    ...
    fn scores_ranks_and_rollups_from_db_signals() {
        ...
        db.put_notification("log_error", "log:szhost", "boom", "")
            .unwrap();
        ...
        use superzej_core::attention::AttentionTier as T;
    ...
    fn merge_queue_row_scores_when_parseable() {
        ...
        assert_eq!(
            ...
            superzej_core::attention::AttentionTier::Blocked
        );
