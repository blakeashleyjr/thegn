...
impl PrView {
    ...
    pub fn open(
        ...
        let (owner, repo) = superzej_core::github::owner_repo_from_url(&pr.url).unwrap_or_default();
        PrView {
            number: pr.number,
            url: pr.url.clone(),
            owner,
    ...
    fn checks_body(&self) -> Vec<(Line, bool)> {
        ...
        for (i, c) in self.checks.iter().enumerate() {
            ...
            let (glyph, tone) = match c.state {
                CheckState::Pass => ("✓", Tok::Hue(superzej_core::theme::Hue::Green)),
                CheckState::Fail => ("✗", Tok::Hue(superzej_core::theme::Hue::Red)),
                CheckState::Pending => ("•", Tok::Hue(superzej_core::theme::Hue::Amber)),
            };
            ...
            match row {
                ...
                ConvRow::Thread(i) => {
                    ...
                    out.push((
                        Line::segs(vec![
                            seg(Tok::Slot(S::Faint), sel_marker(selected)),
                            seg(Tok::Hue(superzej_core::theme::Hue::Blue), "▚ "),
                    ...
                    out.push((
                        Line::split(
                            ...
                            vec![
                                ...
                                seg(Tok::Hue(superzej_core::theme::Hue::Red), format!("-{dels}")),
                            ],
                        ),
                        selected,
                    ));
...
fn review_tone(state: &str) -> Tok {
    match state.to_uppercase().as_str() {
        "APPROVED" => Tok::Hue(superzej_core::theme::Hue::Green),
        "CHANGES_REQUESTED" => Tok::Hue(superzej_core::theme::Hue::Red),
        _ => Tok::Slot(S::Dim),
    }
}
...
fn diff_line(dl: &DiffLine, selected: bool, cols: usize) -> Line {
    let (marker, tone) = match dl.kind {
        DiffLineKind::Add => ("+", Tok::Hue(superzej_core::theme::Hue::Green)),
        DiffLineKind::Del => ("-", Tok::Hue(superzej_core::theme::Hue::Red)),
    ...
    fn files_expand_and_line_comment() {
        ...
        v.diff = Some(PrDiff {
            files: vec![DiffFile {
                ...
                hunks: vec![superzej_core::github::DiffHunk {
                    header: "@@ -1,2 +1,3 @@".into(),
                    lines: vec![
                        DiffLine {
