//! `szhost vps-ssh <name> [--] [cmd…]` — the self-bridge that gives VPS envs a
...
use superzej_svc::vps::{registry, ssh_shim};
...
fn resolve_ip(cfg: &Config, name: &str) -> Result<String> {
    ...
    Err(anyhow!(
        "vps-ssh: no registry record or reachable API for instance {name:?}; \
         provision it first (`superzej env provision`)"
    ))
}
...
pub fn run(cfg: &Config, name: &str, cmd: &[String]) -> Result<()> {
    ...
    let shim = ssh_shim::SshShim {
        name: name.to_string(),
        ip,
        user: superzej_svc::vps::VPS_USER.into(),
        key_path: key,
    };
