...
mod tests {
    ...
    fn workspace_input_accepts_existing_directory_workspace() {
        ...
        let missing_dir = std::env::temp_dir().join(format!(
            "superzej-test-ws-missing-{}-{}",
            std::process::id(),
            now_secs()
        ));

        let missing_path =
            create_workspace_from_input(missing_dir.to_str().unwrap(), &mut session, &db)
                .expect("missing directory workspaces should be created and accepted");

        assert_eq!(missing_path, missing_dir);
        ...
        assert_eq!(db.workspaces().unwrap()[0].kind, "repo");
    }
