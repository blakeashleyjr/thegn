...
async fn event_loop<T: Terminal>(
    ...
    loop {
        ...
        // The accordion's width (`e` cycles Normal → Half → Full) drives the
        // chrome geometry. Central detector so every toggle path (keyboard,
        // mouse Expand hit) is covered without per-site wiring.
