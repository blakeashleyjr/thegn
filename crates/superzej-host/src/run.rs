        ...
        if let Some(parent) = config_path.parent() {
            ...
            if let Ok(mut watcher) = recommended_watcher(move |res: notify::Result<Event>| {
                    ...
                    && last_send.elapsed() > std::time::Duration::from_millis(500)
                {
                    ...
                    .map(|mut c| {
                        superzej_core::host_config::merge_db_hosts(&mut c);
                        c
                    });
...
pub(crate) fn delete_groups(
    ...
    superzej_core::host_config::merge_db_hosts(&mut del_cfg);
...
async fn event_loop<T: Terminal>(
    ...
    loop {
        ...
        // Apply the last `superzej open` focus intent: same switch path as a
        ...
        if let Some(row) = pending_focus
            && let Ok(fi) = serde_json::from_str::<superzej_core::models::FocusIntent>(&row.payload)
        {
