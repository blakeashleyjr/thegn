...
//!    as a tiny per-worktree file under `~/.superzej/drawer/` so it survives
...
/// flag on the next restart). The only staleness is a second concurrent szhost
...
/// (headless/tests).
pub(crate) fn request_spawn(cfg: &superzej_core::config::Config, dir: &Path) {
...
fn resolve_launch(cfg: &superzej_core::config::Config, dir: &Path) -> Result<DrawerLaunch, String> {
    if !dir.is_dir() {
