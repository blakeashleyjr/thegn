...
/// The brand logo segs for the current width's rung: `◆ superzej v0.0.0` (full),
/// `◆ superzej` (compact), or nothing (hidden). A leading pad is the caller's.
fn brand_segs(model: &FrameModel, brand_cols: usize) -> Vec<Seg> {
    ...
    let mut v = vec![seg(accent, diamond), seg(Tok::Slot(S::Text), "superzej")];
