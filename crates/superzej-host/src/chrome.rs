...
fn row_bg(
    ...
    let header = matches!(row.kind, RowKind::Workspace | RowKind::TerminalHost | RowKind::Folder);
    if i == cursor {
        Tok::Slot(S::Panel2)
    } else if row.active {
        Tok::SelAccent
    ...
    match row.kind {
        RowKind::Workspace | RowKind::TerminalHost => {
            ...
            if row.kind == RowKind::TerminalHost {
                ...
                let local = row
                    ...
                    .map(str::is_empty)
                    .unwrap_or(true);
                l.push(seg(Tok::Slot(S::Dim), if local { "\u{1f4bb} " } else { "\u{1f310} " }));
            } else if row.dir {
        ...
        RowKind::Worktree => {
            ...
            if let Some(g) = row.git {
                ...
                if g.ahead > 0 {
                    push_sp(&mut right);
                    right.push(seg(Tok::Slot(S::Dim), format!("{}{}", gl.arrow_up, g.ahead))); // ↑N
                }
                if g.behind > 0 {
                    push_sp(&mut right);
                    right.push(seg(Tok::Slot(S::Dim), format!("{}{}", gl.arrow_down, g.behind)));
                    // ↓N
                }
            }
...
fn compose_rail_line(row: &crate::sidebar::SidebarRow) -> crate::seg::Line {
    ...
    match row.kind {
        RowKind::Worktree => {
            ...
            let initial: String =
                row.label.chars().next().map(|c| c.to_string()).unwrap_or_default();
