...
//!
//! - instance labeled ours (`sz-host = <this host>`) with NO ledger record and
...
use superzej_core::store::PoolStore;
use superzej_svc::vps::{self, registry};
...
pub fn tick(cfg: &Config) {
    ...
    let envs: Vec<(String, superzej_core::config::EnvProviderConfig)> = cfg
        .env
        .iter()
        .filter(|(_, e)| superzej_core::config::vps_provider_kind(&e.provider.provider))
...
fn reap(envs: &[(String, superzej_core::config::EnvProviderConfig)]) {
    ...
    for (env_name, pc) in envs {
        ...
        let instances = match crate::agent::block_on_provider(|| async {
            ...
            Ok(list) => list,
            Err(e) => {
                tracing::debug!(target: "szhost::lifecycle", error = %e, "vps reap: list failed");
                continue;
            }
        };
        ...
        for inst in instances
            .iter()
            .filter(|i| i.labels.get("sz-host").map(String::as_str) == Some(ours.as_str()))
        {
