    ...
    N: Fn() + Clone + Send + 'static,
{
    ...
    for _ in 0..action.create {
        ...
        std::thread::spawn(move || {
            // Log the active provisioning phase so a *stuck* background spare is
            // diagnosable from `SUPERZEJ_LOG` (it has no loading screen of its own).
            let breadcrumb = |views: &[crate::agent::ProvisionStepView]| {
                if let Some(active) = views
                    .iter()
                    .find(|v| v.state == crate::agent::ProvisionState::Active)
                {
                    tracing::debug!(target: "szhost::lifecycle", step = %active.label, "warm spare provisioning");
                }
            };
            match crate::provision_gate::provision_spare(&cfg, &repo_root, &env_name, breadcrumb) {
                Ok(name) => {
