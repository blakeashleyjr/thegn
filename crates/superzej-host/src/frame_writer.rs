...
//!
//! `sync` mode (the termwiz debug renderer, or `SUPERZEJ_SYNC_WRITER=1` as the
...
impl FrameWriter {
    ...
    /// BufferedTerminal (not us), and `SUPERZEJ_SYNC_WRITER=1` is the async
    /// writer's kill-switch/A-B lever.
    pub(crate) fn want_sync(use_termwiz_renderer: bool) -> bool {
        use_termwiz_renderer || std::env::var_os("SUPERZEJ_SYNC_WRITER").is_some_and(|v| v == "1")
    }
