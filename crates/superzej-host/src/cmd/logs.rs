//! `szhost logs` — plugin/script API for reading the szhost log file.
//!
...
pub enum Action {
    /// Tail the szhost log (like `tail -n N`, but with filtering).
    Tail {
...
pub fn run(cfg: &superzej_core::config::Config, action: Action) -> anyhow::Result<()> {
    match action {
        Action::Tail {
            lines,
            level,
            ...
            let log_path = superzej_core::log_view::log_file_path(&cfg.log);
            if path {
            ...
            if !log_path.exists() {
                eprintln!(
                    "log file not found: {} (set SUPERZEJ_LOG to enable logging)",
                    log_path.display()
                );
                return Ok(());
            }
