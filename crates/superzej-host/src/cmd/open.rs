//! `superzej open <repo>` — remote-control (or launch) the compositor onto a
//! workspace.
//!
...
//! normal interactive launch, whose startup resolution (SUPERZEJ_SESSION →
...
use superzej_core::store::{IntentStore, WorkspaceStore};
...
pub fn run(cfg: &Config, repo_arg: &str, no_launch: bool) -> Result<OpenOutcome> {
    ...
    if superzej_core::profile::instance_running() {
        let payload = serde_json::to_string(&superzej_core::models::FocusIntent {
            repo: target.clone(),
        })?;
...
fn resolve(cfg: &Config, db: &Db, arg: &str) -> Result<String> {
    let expanded = superzej_core::util::expand_tilde(arg);
