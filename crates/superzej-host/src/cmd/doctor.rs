...
fn master_status(placement: &superzej_core::placement::Placement) -> String {
    let superzej_core::placement::Placement::Ssh(p) = placement else {
        return String::new();
    };
    let sock = superzej_core::remote::control_path(&p.host, p.port);
    if !sock.exists() {
