...
//! plain-thread watchdog, so if szhost exits the agent is orphaned (it keeps
...
pub(crate) struct DrainCtx<'a> {
    ...
    pub event_bus: &'a superzej_core::event_bus::EventBus,
...
/// agent — CLI symmetry with `superzej integrate`).
...
/// (CLI symmetry with `superzej merge drain`).
...
pub(crate) fn apply_step(
        ...
        .find(|r| r.worktree == worktree)
    {
        ...
        None => {
            ...
            panel.merge_queue.push(superzej_core::db::MergeQueueRow {
...
fn notify_queue(ctx: &mut DrainCtx, kind: NotificationKind, worktree: &str, message: String) {
    ...
    if dec.desktop {
        let n = superzej_core::notification::Notification {
            id: 0,
            kind,
        ...
        ctx.event_bus.publish_with_notification(
            &superzej_core::event_bus::Event::NotificationReceived { notification: n },
        );
    }
...
/// `superzej merge land`, recording the outcome on the queue row. Returns the
/// terminal `DriveMsg` (a `Done` clears the inflight flag).
fn land_ready(cfg: &superzej_core::config::Config, wt: &str) -> DriveMsg {
...
mod tests {
    ...
    fn row(worktree: &str, status: &str) -> superzej_core::db::MergeQueueRow {
