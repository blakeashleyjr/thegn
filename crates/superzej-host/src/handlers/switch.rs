...
pub(crate) fn refresh_tab_model_switch(
    ...
    // Escape hatch + A/B lever: `SUPERZEJ_SWITCH_FULL_REBUILD=1` routes every
    ...
    if *FORCE_FULL
        .get_or_init(|| std::env::var_os("SUPERZEJ_SWITCH_FULL_REBUILD").is_some_and(|v| v == "1"))
    {
    ...
    model.active_container_name = superzej_core::sandbox::container_name(
        session
            .active_group()
            .map(|g| g.path.as_str())
            .unwrap_or(""),
