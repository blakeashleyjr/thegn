...
//!
//! Only the superzej-managed pi agent speaks ACP: its command references
//! `~/.superzej/pi`, so superzej seeds it the `superzej-acp` extension that binds
...
//! (plus a spurious "N error(s) in szhost.log" badge). This module is the gate:
...
/// Whether `agent_name` is the superzej-managed pi — the only agent that loads
/// the `superzej-acp` extension and therefore answers on the ACP channel. Keyed
/// on the same `.superzej/pi` command marker the sandbox provisioner uses to
/// decide it must install the managed pi (see `agent::…` `managed_pi`).
pub(crate) fn speaks_acp(cfg: &Config, agent_name: &str) -> bool {
    cfg.agents
        .iter()
        .find(|a| a.name == agent_name)
        .map(|a| a.command.contains(".superzej/pi"))
        .unwrap_or(false)
}

/// Resolve the ACP channel + proxy side effects for a launched pane, pushing any
/// pane env (`ACP_PORT`, `SUPERZEJ_PROXY_*`) onto `env`.
///
...
mod tests {
    use super::*;
    use superzej_core::config::{Config, NamedCommand};
    ...
    const MANAGED_PI: &str = "PI_CODING_AGENT_DIR=\"$HOME/.superzej/pi/agent\" exec \"$HOME/.superzej/pi/node_modules/.bin/pi\"";
    ...
    fn managed_pi_tcp_reserves_port_and_pushes_env() {
        ...
        assert!(
            !env.iter().any(|(k, _)| k.starts_with("SUPERZEJ_PROXY_")),
