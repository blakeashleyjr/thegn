...
use superzej_core::config::DaemonConfig;
use superzej_core::control_wire::EventFrame;
use superzej_core::db::Db;
use superzej_svc::control::client::{AttachControl, AttachStream, ControlAddr, ControlClient};
use superzej_svc::control::{OpenSpec, SessionInfo};
use superzej_svc::provider::{ExecControl, ExecFrame, ExecSession, ExecSpec};
...
pub(crate) async fn ensure_daemon(dcfg: &DaemonConfig) -> Result<ControlClient> {
    ...
    let discovered = tokio::task::spawn_blocking({
        ...
        move || {
            let db = Db::open().ok()?;
            superzej_svc::control::client::discover(&db, &scope, now_ms())
        }
    })
    .await
    .ok()
    ...
    let mut cmd = superzej_core::util::detached(&exe.to_string_lossy());
