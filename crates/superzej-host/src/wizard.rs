...
pub fn run_worker(
    ...
    let prep = |backend: &str, wt: &Path| -> crate::agent::SandboxOutcome {
    ...
    let choices = loop {
        match cmds.recv() {
            ...
            Ok(WizardCmd::SandboxChosen(backend)) => {
                ...
                let outcome = prep(&backend, &path);
                step(
                    CreateStep::SandboxPrep,
                    StepState::Done,
                    Some(outcome.backend_label.clone()),
                );
            ...
            if want == branch {
                ...
                if renamed {
                        ...
                        && outcome.spec.is_some()
                    {
                        ...
                        step(CreateStep::SandboxPrep, StepState::Running, None);
                        let redo = prep(&backend, &path);
                        step(
                            CreateStep::SandboxPrep,
                            StepState::Done,
                            Some(redo.backend_label.clone()),
                        );
                        *outcome = redo;
                    }
                    step(
    ...
    let (_, sandbox) = match prepped {
        ...
        None => {
            step(CreateStep::SandboxPrep, StepState::Running, None);
            let outcome = prep(&choices.sandbox, &path);
            step(
                CreateStep::SandboxPrep,
                StepState::Done,
                Some(outcome.backend_label.clone()),
