...
//!
//! `ptrace_scope=1` blocks attaching `perf`/`gdb` to an already-running szhost,
//! so instead the process profiles *itself*: send `SIGUSR2` once to start, again
//! to stop and write a flamegraph to `$XDG_STATE_HOME/superzej/profiles/`. This
...
mod imp {
            ...
            .build()
        {
            Ok(g) => {
                *guard_slot().lock().unwrap() = Some(g);
                tracing::info!(target: "szhost::perf", "profiler started (SIGUSR2 again to dump)");
            }
            Err(e) => tracing::warn!(target: "szhost::perf", error = %e, "profiler start failed"),
        }
    }
    ...
    fn stop_and_dump() {
        ...
        let report = match guard.report().build() {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!(target: "szhost::perf", error = %e, "profiler report failed");
                return;
            }
        };
        let dir = superzej_core::util::superzej_dir().join("profiles");
        ...
        match std::fs::File::create(&path).and_then(|f| {
            ...
            Err(e) => tracing::warn!(target: "szhost::perf", error = %e, "flamegraph write failed"),
        }
    }
}
