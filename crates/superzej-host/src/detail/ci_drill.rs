...
//! `RunCommand(["ci","view",<id>])` that spawned a one-shot `szhost ci view`
...
pub struct CiDetailPayload {
    pub run: superzej_core::ci::CiRun,
...
pub(crate) fn ci_glyph_marker(state: superzej_core::ci::CiState) -> (&'static str, Tok) {
...
pub(crate) fn ci_state_word(state: superzej_core::ci::CiState) -> &'static str {
    use superzej_core::ci::CiState;
    match state {
...
fn ci_drill_title(run: &superzej_core::ci::CiRun) -> String {
...
fn ci_header_section(run: &superzej_core::ci::CiRun) -> Section {
    ...
    if let Some(secs) = run.duration_secs(superzej_core::util::now()) {
...
impl DetailOverlay {
    ...
    /// dropped.
    pub(crate) fn enter_ci_view(&mut self, run: &superzej_core::ci::CiRun) {
    ...
    pub(crate) fn set_ci_detail(&mut self, run: &superzej_core::ci::CiRun, log_tail: Vec<String>) {
        ...
        if !log_tail.is_empty() {
            ...
            let rows: Vec<Vec<Cell>> = log_tail
                .iter()
                .map(|line| {
                    let tone = if superzej_core::ci::line_is_failure(line) {
                        Tok::Hue(Hue::Red)
                    } else {
                        Tok::Slot(S::Dim)
                    };
...
mod tests {
    ...
    fn ci_rows_are_enriched_and_drill_fills_in_place() {
        use superzej_core::ci::{CiJob, CiRun, CiState, CiStep};
        let run = CiRun {
            id: "42".into(),
            name: "CI".into(),
            title: "fix: DNS".into(),
