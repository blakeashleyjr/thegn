...
//!
//! This is the autopilot on top of the pure fold engine ([`superzej_core::fold`])
...
//! canonical checkout — the agent only makes its branch clean; superzej does the
...
use superzej_core::config::{ConflictHandoff, MergeQueueConfig};
...
fn run_agent(
    ...
    cmd.arg("-lc")
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .env("SUPERZEJ_WORKTREE", worktree)
        .env("SUPERZEJ_BRANCH", branch)
        .env("SUPERZEJ_MERGE_PROMPT", &prompt)
        .env("SUPERZEJ_MERGE_TARGET", target)
        .process_group(0);
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            tracing::warn!(target: "szhost::merge", error = %e, "merge queue: agent failed to spawn");
            return false;
        }
    };
    ...
    if timed_out.load(Ordering::Relaxed) {
        tracing::warn!(target: "szhost::merge", branch, "merge queue: agent timed out");
        return false;
    }
