...
//!
//! Executes the pure decision from [`superzej_core::merge_lifecycle`]: file a
...
use superzej_core::merge_lifecycle::{LifecycleAction, LifecycleEvent, decide};
...
fn remove_landed(db: &Db, repo_root: &Path, worktree: &str, branch: &str, delete_branch: bool) {
    superzej_core::worktree::remove(repo_root, Path::new(worktree), branch, delete_branch);
...
mod tests {
    ...
    use superzej_core::config::OnLanded;
