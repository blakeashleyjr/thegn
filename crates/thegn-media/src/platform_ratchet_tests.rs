//! Platform-cfg ratchet for `thegn-media` — a per-OS leaf crate by design, so the
//! pinned files in `test/platform-cfg-media-ratchet.txt` *are* the platform
...
fn platform_cfgs_are_pinned() {
    file_ratchet(
        ...
        "platform-cfg-media-ratchet.txt",
        &[],
